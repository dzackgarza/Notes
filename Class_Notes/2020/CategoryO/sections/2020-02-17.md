...
a. If $M$ is a highest weight module of highest weight $\mu$ and $\mu \not< \lambda$, then $\ext_\OO(M(\lambda), M = 0$. 
...
*Proof:*
Given an extension $0 \to M \mapsvia{f} E \mapsvia{g} M(\lambda) \to 0$, we want to show it splits.
