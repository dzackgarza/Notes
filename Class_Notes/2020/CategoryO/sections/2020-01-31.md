...
: 	$\chi_\lambda$ is called the *central (or infinitesimal) character*, and $\hat\mcz(\lieg)$ denotes the set of all central characters.
    ...
    Central characters are in one-to-one correspondence with maximal ideals of $\mcz(\lieg)$, where $\chi \iff \ker \chi$ and $\CC[x_1, \cdots, x_n] \iff \generators{x_1 - a_1, \cdots, x_n - a_n}$ where $[a_1, \cdots] \in \CC^n$.
