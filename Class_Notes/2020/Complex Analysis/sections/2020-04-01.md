...
:   We argue by contradiction and suppose $f'(z_0) =0$ for some $z_0 \in U$.
    ...
    Since $\abs G < \abs F$ on a small disc $D_\eps(z_0)$ and $F$ has at least two zeros in $D_\eps(z_0)$, Rouche's theorem implies that $f(z) - f(z_0) - w$ has at least two zeros in this disc.
