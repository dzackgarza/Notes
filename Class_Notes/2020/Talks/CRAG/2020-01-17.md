...
i.e. any solutions to $f = 0$ over $k^{r+1}$ can be found by first choosing a point $u = (u_0, \cdots, u_r)$ in the variety cut out by $L$, so $L(u) = \sum a_i u_i = 0$, then picking an $n_i$the root $r_i \in k$ of each $u_i$ to obtain some $r = (r_0, \cdots, r_r) \in k^{r+1}$.
Then $u_i = r_i^{n_i}$ implies that $0 = \sum a_i u_i = \sum a_i r_i^{n_i}$, so $r$ is a solution to $f$.
