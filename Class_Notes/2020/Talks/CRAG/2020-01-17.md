...
Schubert cells exhibit structure as a CW complex, and $H^{2d}(\Gr(k, \CC^{n+k})) \cong \ZZ^\ell$ where $\ell$ is the number of solutions of $\sum_{j=1}^k x_j = d$ with $x_j$ weakly increasing, i.e. $x_1 \leq x_2 \leq \cdots x_k$.
