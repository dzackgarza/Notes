...
- closed: $d\omega = 0$
- nondegenerate $\omega_p: T_p M \tensor T_p M \to \RR$; $\omega_p(\vector v,\vector w) = 0~~\forall \vector w \implies \vector v= 0$.
...
,$$ 
or equivalently $\omega^n \neq 0$ (c.f. Mike Hutchings).

> Motivation: There is Hamiltonian $H$ in the background, we want this to induce a vector field and thus a flow.
