...
Definition:
Recall that for any smooth function $H: M\to N$, the *derivative* of $H$ at $p\in M$ is defined by $dH_p: T_pM \to T_p N$ which we define using the derivation definition of tangent vectors: given a derivation $v\in T_p M: C^\infty(M) \to \RR$, we send it to the derivation $w_v \in T_{q}M = C^\infty(M) \to \RR$ where $w_v$ actson on $f\in C^\infty(M)$ by $w_v \actson f = 

*Remark:*
If $(M, \omega)$ is a symplectic manifold, $H$ induces a unique vector field $X_H = \theset{X_{H_t}}$ characterized by the property $\iota_{X_{H_t}\omega = -dH_t$.
