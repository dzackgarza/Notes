...
> However, not every compact 3-manifold $M$ admits a *fillable* contact structure (roughly: admits a symplectici manifold $X$ with $\bd X = M$ -- need framing to be realizable as a *Legendrian* framing.
...
**Definition (Surgery):**
Given a manifold $M^n$ where $n=p+q$, then $p\dash$surgery on $M$ is the $S(M)$, result of cutting out $S^p \cross D^q$ and gluing back in $D^{p+1} \cross S^{q-1}$.

> Note: $\bd(S^p\cross D^q) = S^p \cross S^{q-1} = \bd(\mathbf{D^{p+1} \cross S^{q-1}})$

Formally, fixing any $\phi: S^p \cross D^q \to M$, we have
\begin{align*}
\mathcal{S}_p(M) \definedas M \setminus (\im \phi)^\circ \disjoint_\phi (D^{p+1} \cross S^{q-1})
.\end{align*}

**Definition (Handle Attachment)**
Given a manifold $(M^n, \bd M^n)$ with boundary, attaching a $k\dash$handle to $M$, denoted $H_k(M)$, is given by $M$ with new boundary $\mathcal{S}_k(\bd M)$.
