...
\begin{align*}
S(M) \definedas M \setminus (\im \phi)^\circ \disjoint_\phi (D^{p+1} \cross S^{q-1})
.\end{align*}

**Definition (Handle Attachment)**
Given a manifold $(M^n, \bd M^n)$ with boundary, attaching a $k\dash$handle to $M$, denoted $H_k(M)$, is given by an embedding $\phi: S^p \cross D^q \to M$ where
\begin{align*}
