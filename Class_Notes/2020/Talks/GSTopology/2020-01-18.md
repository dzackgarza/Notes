...
If we have $H: M\cross I \to \RR$, we'll refer to this as a *time-dependent Hamiltonian*, i.e. $H_t: M \to \RR$ given by $H_t(p) = H(p, t)$ is a Hamiltonian.

Remark:
If $M$ is a symplectic manifold, $H$ induces a unique vector field $X_H = \theset{X_{H_t}}$
