...
Let $\Gamma_{p, q} = S^p \cross D^q$, then the above says $\bd \Gamma_{p, q} = S^p \cross S^q = \bd \Gamma_{p+1, q-1}$

So fix any embedding 
$$\phi: \Gamma_{p, q} \to M$$

Note that this restricts to some map 
$$\tilde \phi: \bd \Gamma_{p, q} \to M$$
