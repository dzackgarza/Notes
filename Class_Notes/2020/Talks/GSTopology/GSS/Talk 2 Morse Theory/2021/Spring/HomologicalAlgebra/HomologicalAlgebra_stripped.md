...
::: {.remark}
Last time: semisimple Lie algebras. For today, let \( {\mathfrak{g}}\in {\mathsf{Lie}{\hbox{-}}\mathsf{Alg}} \) be finite dimensional over \( k \) a field with \( \operatorname{ch}(k) = 0 \). We saw that \( {\mathfrak{g}} \) is semisimple iff \( {\mathfrak{g}}\cong \bigoplus {\mathfrak{g}}_i \) with each \( {\mathfrak{g}} \) simple, which implies \( [{\mathfrak{g}}_i {\mathfrak{g}}_i] = {\mathfrak{g}}_i \). As a corollary, \( H_1({\mathfrak{g}}; k) = 0 = H^1({\mathfrak{g}}; k) \) by considering the abelianization of \( {\mathfrak{g}} \). We'll now discuss a corollary.
:::

::: {.lemma title="?"}
If \( {\mathfrak{g}} \) is semisimple, then for all \( M\in {\mathfrak{g}{\hbox{-}}\mathsf{Mod}} \) finite dimensional, \( H^1({\mathfrak{g}}; M) = 0 \).
:::

::: {.remark}
Recall that these were outer derivations, so this implies that every derivation is inner. In fact, this result is an iff by Weibel exercise 7.8.4.
:::

::: {.proof title="$\\implies$"}
Induct on \( n \coloneqq\dim_k M \). For \( n=1 \), \( M=k \) is the trivial \( {\mathfrak{g}}{\hbox{-}} \)module, so applying the previous corollary shows that \( H^1 \) vanishes. Now for \( n>1 \), if \( M \) is simple we can use the previous theorem. Otherwise, \( M \) has a proper nonzero \( {\mathfrak{g}}{\hbox{-}} \)submodule \( L \leq M \), and inductively \( H^1({\mathfrak{g}}; L) = 0 \). We also have \( M/L \) of smaller dimension than \( M \), so \( H^1({\mathfrak{g}}; M/L) = 0 \) as well. So take the SES
\[
0 \to L \to M \to M/L \to 0
,\]
...
\begin{tikzcd}
    \cdots \\
    \\
    {H^1({\mathfrak{g}}; L) = 0} && \textcolor{rgb,255:red,214;green,92;blue,92}{H^1({\mathfrak{g}}; M)} & {} & {H^1({\mathfrak{g}}; M/L) = 0} \\
    \\
    && \cdots && {H^0({\mathfrak{g}}; L)}
...
Hence \( H^1({\mathfrak{g}}; M) = 0 \).
:::

::: {.theorem title="Weyl's Theorem"}
If \( {\mathfrak{g}} \) is semisimple, then every finite-dimensional \( {\mathfrak{g}}{\hbox{-}} \)module is completely reducible, i.e. it decomposes as a direct sum of irreducible submodules.
:::

::: {.proof title="?"}
Essentially an inductive argument. Suppose the result is false and choose a minimal counterexample: a finite dimensional \( {\mathfrak{g}}{\hbox{-}} \)module \( M_1 \) that is not completely reducible where every smaller dimensional module *is* complete reducible. Simple modules are completely reducible, since they're direct summand of themselves, so \( M_1 \) is not simple. Thus we can choose \( M_0 \leq M_1 \) a nonzero proper submodule, and inductively \( M_0, M_1/M_0 \) are completely reducible. So the following SES can not be split:
\[
0 \to M_0 \to M_1 \to M_2 \coloneqq M_1/M_0 \to 0
.\]
If it were, then \( M_1 \cong M_0 \oplus M_2 \) would be reducible, a contradiction. So there is a nontrivial \( \operatorname{Ext}^1_{{\mathcal{U}(\mathfrak{g}) }}(M_2, M_0) \), since this classifies extensions, the split extension is trivial, and non-split extensions yield nontrivial elements in this group. By exercise 7.3.5, we can use a nice trick:
\[
0\neq \operatorname{Ext}^1_{{\mathcal{U}(\mathfrak{g}) }}(M_2, M_0) \cong H^1( {\mathfrak{g}}, \mathop{\mathrm{Hom}}_{{\mathsf{k}{\hbox{-}}\mathsf{Mod}}}(M_2, M_0) )
.\]
...
If \( M \in {\mathfrak{g}{\hbox{-}}\mathsf{Mod}} \) is finite dimensional and \( {\mathfrak{g}} \) is semisimple, then \( H^2({\mathfrak{g}}; M) = 0 \).
:::

::: {.remark}
Question from DZG: what do we know about \( H^*({\mathfrak{sl}}_2({\mathbb{C}}), M) \)? By exercise 7.7.3, it turns out that \( H^3({\mathfrak{sl}}_2({\mathbb{C}}), k) = k \). Thus

1.  There is no "Whitehead's Third Lemma" for \( H^3 \),
2.  We know \( H^*({\mathfrak{sl}}_2; M) \) for all \( M \in \mathsf{{\mathfrak{sl}}_2}{\hbox{-}}\mathsf{Mod} \): Since \( \dim {\mathfrak{sl}}_2 = 3 \) (using that \( \dim {\mathfrak{gl}}_2 = 4 \) and we impose one linear condition), we have \( H^{>3} = 0 \). We also know it's 0 for \( n=3 \) if \( M \) is simple and not equal to \( k \), and we know what it is for \( k \). For non-simple modules, if \( M \) is finite dimensional, we have \( M \cong \bigoplus M_i \) with each \( M_i \) simple by Weyl's theorem, and \( H^n({\mathfrak{g}}, {-}) \) commutes with direct sums.
:::

::: {.theorem title="Levi"}
Let \( {\mathfrak{g}}\in {\mathsf{Lie}{\hbox{-}}\mathsf{Alg}} \) be finite-dimensional, not necessarily semisimple. Then there exists a semisimple Lie-subalgebra \( {\mathfrak{l}} \) called the **Levi factor** (or **Levi subalgebra**) such that \( {\mathfrak{g}}\cong \mathop{\mathrm{rad}}{\mathfrak{g}}\rtimes{\mathfrak{l}} \). [^15]

This is defined as \( \mathop{\mathrm{rad}}{\mathfrak{g}}\times{\mathfrak{l}} \) as the underlying \( k{\hbox{-}} \)module with bracket
\[
[ (r, x), (s, y) ] = ([xs]-[yr], [xy])
.\]
:::
...
::: {.remark}
Note that any \( r\in \mathop{\mathrm{rad}}{\mathfrak{g}} \) corresponds to \( (r, 0) \), and bracketing it with anything causes \( [xs]=0 \) and more importantly \( [xy]=0 \), so this lands back in \( \mathop{\mathrm{rad}}{\mathfrak{g}} \).
:::

::: {.definition title="Reductive Lie Algebras"}
A Lie algebra \( {\mathfrak{g}} \) is **reductive** if \( {\mathfrak{g}} \) is completely reducible as a \( {\mathfrak{g}}{\hbox{-}} \)module by the adjoint action.
:::

::: {.remark}
Note that submodules under the adjoint action are ideals, which are \( k{\hbox{-}} \)submodules closed under bracketing with everything. In this situation, \( {\mathfrak{g}} \) is a direct sum of simple (or possibly 1-dimensional trivial) \( {\mathfrak{g}}{\hbox{-}} \)module. Moreover, \( {\mathfrak{g}}\cong Z({\mathfrak{g}}) \oplus [{\mathfrak{g}}{\mathfrak{g}}] \), and in fact \( Z({\mathfrak{g}}) \cong \bigoplus_{i=1}^{m} k \) for some \( m \). The second part \( [{\mathfrak{g}}{\mathfrak{g}}] \) is a direct sum of simple ideals, and in characteristic zero, this is the Levi decomposition. So \( [{\mathfrak{g}}{\mathfrak{g}}] = {\mathfrak{l}} \) and \( \mathop{\mathrm{rad}}{\mathfrak{g}}= {\mathfrak{g}} \), and thus \( {\mathfrak{g}} \) is the sum of an abelian (the radical \( Z({\mathfrak{g}}) \)) and a semisimple (the Levi factor \( [{\mathfrak{g}}{\mathfrak{g}}] \)) Lie algebra.
:::

::: {.remark}
Note that there are 3 characterizations here:

1.  \( [{\mathfrak{g}}{\mathfrak{g}}] = {\mathfrak{l}} \)
2.  \( \mathop{\mathrm{rad}}{\mathfrak{g}}= Z({\mathfrak{g}}) \)
3.  \( {\mathfrak{g}} \) is the sum of an abelian and semisimple algebra.

In characteristic zero, theses are all equivalent to \( {\mathfrak{g}} \) being reductive.
:::

::: {.example title="The prototypical reductive Lie algebra"}
Take \( {\mathfrak{gl}}_m = k \oplus {\mathfrak{sl}}_m \), where \( k \) denotes the scalar matrices \( Z({\mathfrak{g}}) \). Noting that \( \operatorname{Tr}([xy]) = \operatorname{Tr}(xy-yx) = 0 \) since the trace doesn't depend on multiplicative order, \( {\mathfrak{sl}}_m \) are trace zero matrices and thus form a simple algebra.
:::
...
::: {.remark}
Many results about simple Lie algebras extend naturally to reductive algebras. Technique: to prove things about semisimple Lie algebras, look for smaller reductive subalgebras and use induction. These show up in **parabolic subalgebras**, which have a reductive Levi factor.
:::
...
Finite dimensional representations of reductive algebras need not be completely reducible. Take \( {\mathfrak{g}}\coloneqq{\mathbb{C}} \) with basis \( x \), a 1-dimensional abelian Lie algebra (sometimes called a 1-dimensional torus), and take \( M \coloneqq{\mathbb{C}}^2 = {\mathbb{C}}\left\langle{ v, w }\right\rangle \) and define an action of \( {\mathfrak{g}}\curvearrowright M \) by the matrix
\[
A\coloneqq
\begin{bmatrix}
0 & 1 
...
\end{bmatrix}
,\]
so \( x\cdot v = 0v + 0w \) and \( x\cdot w = 1v + 0w \). This preserves the bracket trivially, and the commutator of any matrix with itself is always zero since it's equal to \( A^2 - A^2 \). Note that \( {\mathbb{C}}v \subseteq M \) is a \( {\mathfrak{g}}{\hbox{-}} \)invariant subspace, but there is no \( {\mathfrak{g}}{\hbox{-}} \)invariant complement: taking any other vector \( \mathbf{y}\coloneqq av + bw \), one can compute
\[
x\cdot \mathbf{y} \coloneqq x(av+bw) = bv
,\]
and this can only be of the form \( \lambda \mathbf{y} \) if \( b=0 \) and \( a=b \), which forces \( a=0 \). This is certainly a reductive Lie algebra, since \( Z({\mathfrak{g}}) = {\mathfrak{g}} \) and the semisimple part \( [{\mathfrak{g}}{\mathfrak{g}}] = 0 \), but this is not completely reducible.
:::

::: {.question}
Can you find a finite dimensional representation of \( {\mathfrak{gl}}(2, {\mathbb{C}}) \) that is not completely reducible? The previous example is cheating a little since it's a torus and there's no semisimple part, but what if that part is nontrivial? Here the 1-dimensional center doesn't have to act in a completely reducible manner.
:::
...
[^15]: Note that the angle points to the ideal in the semidirect product, and \( \mathop{\mathrm{rad}}{\mathfrak{g}} \) is the largest solvable ideal.
