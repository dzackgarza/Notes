...
- Now have $\theset{s_{n_j}'}_{n_j \in \NN} \to s'$ and $\theset{s_n}_{n\in \NN} \to s$: since the latter sequence is convergent, every subsequence converges to the same limit, so $\theset{s_{n_j}}_{n_j \in \NN} \to s$.
