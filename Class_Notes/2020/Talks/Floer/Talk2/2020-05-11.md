...
- Again using failed injectivity, i.e.
  $$
  v(s, t) = v(s', t) \hspace{4em} (\implies v(s, t) - v(s', t) = 0)
  $$
  we have 
  $$
  s_{n_j}' \neq s_{n_j} \qtext{and} v(s_{n_j}, t_{n_j}) = v(s_{n_j}', t_{n_j}) (\implies v(s_{n_j}, t_{n_j}) - v(s_{n_j}', t_{n_j}) = 0 ~~\forall n_j)
  $$

- But by the last step, we do have equality in the limit, $s= s'$, thus
\begin{align*}
