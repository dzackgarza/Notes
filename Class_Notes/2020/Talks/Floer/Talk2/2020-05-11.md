...
- Produces a sequence $\theset{s_n'}_{n\in \NN}$, want to show it is bounded.
  ...
  \begin{align*}
  v(s, t) 
&= \lim_{n_k \to \infty} v(s_{n_k}', t_{n_k}') \qtext{using continuity of } v\\
  &= v(\pm \infty, t) \\
  &\definedas x^\pm (t)
  .\end{align*}
  - Why? By definition, precisely we extended $v$ by setting $v(\pm \infty, t) = x^{\pm}(t)$.
  - But condition 2 says $v(s, t) \neq x^\pm(t)$, so this contradicts $(s, t) \in R(v)$.
