...
2. Prove the continuation principle that was used in Proposition 8.1.4
  3. Idea: A holomorphic function with all derivatives vanishing on a line is identically zero on $\CC$
...
- (Minor technical point) Now have $\theset{s_{n_j}'}_{n_j \in \NN} \to s'$ and $\theset{s_n}_{n\in \NN} \to s$: since the latter sequence is convergent, every subsequence converges to the same limit, so $\theset{s_{n_j}}_{n_j \in \NN} \to s$.
- Again using failed injectivity, we have $$s_{n_j}' \neq s_{n_j} \qtext{and} v(s_{n_j}, t_{n_j}) = v(s_{n_j}', t_{n_j})$$
- By the last step, we do have 
