...
\left\{\left(u, H=H_{0}+h\right) | h \in \mathcal{C}_{\varepsilon}^{\infty}\left(H_{0}\right) \text { and } u \in \mathcal{M}(x, y, J, H)\right\}
.\end{align*}
If $(u, H) \in \mathcal{Z}(x, y)$ then 
\begin{align*}
\Gamma: W^{1, p}\left(\mathbf{R} \times S^{1} ; \mathbf{R}^{2 n}\right) \times \mathcal{C}_{\varepsilon}^{\infty}\left(H_{0}\right) & \longrightarrow L^{p}\left(\mathbf{R} \times S^{1} ; \mathbf{R}^{2 n}\right) \\
(Y, h) & \longmapsto\left(d \mathcal{F}^{H}\right)_{u}(Y)+\operatorname{grad}_{u} h
\end{align*}
(where $H = H_0 + h$ and $\mcf^H$ is the corresponding Floer operator) admits a continuous right-inverse (and in particular is surjective).
