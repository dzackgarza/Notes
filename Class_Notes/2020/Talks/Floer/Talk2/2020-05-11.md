...
\begin{align*}
  \dd{v}{s} + J_1(v) \dd{v}{t} &= 0 \\ \\  v(s, t+1) &= \phi(v(s, t)) \\ \\ C(v)=C(u),&~R(v) = R(u)
.\end{align*}
...
:   Let $v$ be a smooth solution of the Cauchy-Riemann equation with $v(s, t+1) = \varphi(v(s, t))$ and $\dd{v}{s}{ \neq 0}$.

    \
  
    Then $R(v) \subseteq \RR^2$ is open and dense.
