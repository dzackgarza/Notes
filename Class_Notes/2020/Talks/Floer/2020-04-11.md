...
> Recall what $\mcp^{1, p}(x, y)$ is here.
> 
...
\begin{center}
\begin{tikzcd}
\mathcal{F}: \mathcal{P}^{1, p}(x, y) \ar[r, "\mathcal{F}"]
& L^{p}\left(\mathbb{R} \times S^{1} ; T W\right) \ar[r]
& L^{p}\left(\mathbb{R} \times S^{1} ; \mathbb{R}^{m}\right) \\
u \ar[rr] & &\frac{\partial u}{\partial s}+J(u)\left(\frac{\partial u}{\partial t}-X_{t}(u)\right)
\end{tikzcd}
\end{center}
