...
:   Omitted. Sketch:
    ...
    - Take $\sum \dd{y_i}{t} Z_i$ and expand in the $Z_i$ basis (roughly) to write $\dd{y_i}{t} = \sum b_{ij} y_j$ for some coefficients $b_{ij}$.
    - Collect terms into a matrix/operator $B^\mp$ for $x,y$ respectively 
    - Look at $O_0$, the zero-order part of $(d\mcf)_u$ at $s = \mp \infty$ respectively
