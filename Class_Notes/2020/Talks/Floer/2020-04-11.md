...
Trivialize the symplectic fiber bundle $\tilde u ^* TW$ to obtain an orthonormal basis $\theset{Z_i}_{i=1}^{2n} \subset T_{u(s, t)} W$ depending smoothly on $(s, t) \in S^2$, where $\lim_{s\to \infty} Z_i$ exists for each $i$.
