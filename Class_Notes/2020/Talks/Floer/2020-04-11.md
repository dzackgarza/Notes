...
- By the assumption, Trivialize $\tilde u ^* TW$ to obtain an orthonormal unitary frame $\theset{Z_i}_{i=1}^{2n} \subset T_{u(s, t)} W$ where
  - It depends smoothly on $(s, t) \in S^2$, 
  - $\lim_{s\to \infty} Z_i$ exists for each $i$.
  - $$\dd{}{s}, \quad \frac{\del^2}{\del s^2}, \quad \frac{\del}{\del s~\del t} \quad \actson Z_i \converges{s\to \pm\infty}\to 0\quad \text{for each } i$$
