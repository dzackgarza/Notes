...
Where $\theset {\eps_k} \subset\RR$ is chosen such that $\mcc_\eps^\infty \injects \mcc^\infty(W \cross S^1)$ is dense for the $C^1$ topology, and the $\Psi_i: B_i \to \bar{B(0, 1)}$ is a fixed finite sequence of diffeomorphisms where $\union_i B_i^\circ = W \cross S^1$.
