...
:   \hfill
    ...
    - Apply Stone-Weierstrass: For $A\leq C^\infty(X; \RR)$ a subalgebra with $X$ compact Hausdorff and $A$ containing a nonzero constant function, $A$ is dense iff it separates points (for all $x,y$ there exists $f\in A$ such that $f(x) \neq f(y)$) 
      - Apply to $A = \QQ[x_1, \cdots, x_m]$ the subalgebra of polynomial functions
