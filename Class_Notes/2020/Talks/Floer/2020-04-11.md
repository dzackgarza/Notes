...
:   Omitted -- $S$ is exactly $O_0$ from before:
    ...
    - The term in blue vanishes as $s\to \pm \infty$ using the fact that $u$ is a solution and $\dd{u}{s} \to 0$ uniformly (as do its derivatives?)
    - Suffices to show the remaining part is symmetric in the limit, i.e. write as $A(y_1, \cdots, y_{2n} = \cdots$ and show $A_{ij} = A_{ji}$ using inner product calculations
