...
Goal: given a fixed Hamiltonian $H\in C^\infty(W, \RR)$, perturb it (without modifying the periodic orbits) so that $\mcm(x, y)$ are manifolds of the expected dimension.
...
- Let $h(x, t) \in C_\eps^\infty{H}$ denote a perturbation of $H$.
- Fix $(\eps_n) \subset \RR^{>0}$ a sequence of real numbers, which we will choose carefully later
- For a fixed $x, t$, define $\abs{d^k h(x, t)} = \max \theset {d^\alpha h(x, t) \suchthat \abs{\alpha} = k}$, the maximum over all sets of multi-indices $\alpha$ of length $k$.

  > Note: I interpret this as $d^{\alpha_1, \alpha_2, \cdots}h = \dd{}
