...
- Extract the part that is linear in $Y$:
  \begin{align*}
  (d \mathcal{F})_{u}(Y)
& = {\color{red}\frac{\partial Y}{\partial s}} +(d J)_{u}(Y) \frac{\partial u}{\partial t}+J(u) \frac{\partial Y}{\partial t}-(d J)_{u}(Y) X_{t}-J(u)\left(d X_{t}\right)_{u}(Y)
  .\end{align*}
