...
> Note: I don't know what this pairing is. The top Chern class is the Euler class (obstructs nowhere zero sections) and are defined inductively, so $c_1 \in H^1(TW)$?
> Assumption is satisfies when all maps $S^2 \to W$ lift to $B^3$ $\iff \pi_2(W) = 0$.
