...
- Using the assumption, trivialize $\tilde u ^* TW$ to obtain an orthonormal unitary frame $$\theset{Z_i}_{i=1}^{2n} \subset T_{u(s, t)} W$$ where
...
- Use this frame to define a chart around $u \in \mcp^{1, p}(x, y)$ given by
\begin{align*}
