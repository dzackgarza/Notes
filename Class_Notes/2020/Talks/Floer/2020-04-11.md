...
- Using the chart $\iota$ defined by $\theset{Z_i}$ to write $Y = \sum_{i=1}^{2n} y_i Z_i$ and thus
  ...
  \begin{align*}
  O_0 &= \sum_{i=1}^{2n} \dd{y_i}{s} Z_i + \dd{y_i}{t} J(u) Z_i \\ \\
  O_1 &= \sum_{i=1}^{2n} y_i \qty{ \dd{Z_i}{s} + J(u) \dd{Z_i}{t} + (dJ)_u (Z_i) \dd{u}{t}  - J(u) (dX_t)_u Z_i - (dJ)_u (Z_i) X_t }
  .\end{align*}

> Note: this may not exactly be correct, the wording is ambiguous:
>
![](figures/image_2020-04-15-20-31-48.png)\

- Study $O_1$ first, which (claim) reduce to 
  \begin{align*}
  ...
  - The secon equality follows from the assumption that the $Z_i$ are symplectic and orthonormal.
...
- Note that since we've computed in charts, we have actually computed the differential of $\mcf_u$ in the following diagram
  \begin{center}
  \begin{tikzcd}
  W^{1, p}(\RR \cross S^1; \RR^{2n})  \ar[r, "\iota"] & \mathcal{P}^{1, p}(x, y) \ar[r, "\mathcal{F}"] \ar[rr, dotted, "\tilde \mcf", bend left]
  & L^{p}\left(\mathbb{R} \times S^{1} ; T W\right) \ar[r]
  & L^{p}\left(\mathbb{R} \times S^{1} ; \mathbb{R}^{m}\right) \\
   & u \ar[rr, "\tilde \mcf"] & &\frac{\partial u}{\partial s}+J(u)\left(\frac{\partial u}{\partial t}-X_{t}(u)\right) \\
