...
Start by trying to construct a subspace $\mcc_\eps^{\infty}(H) \subset \mcc^\infty(W\cross S^1; \RR)$, the space of perturbations of $H$, and show it is dense.
...
Note that in order to actually compute these derivatives,we need to fix 

Then
\begin{align*}
\norm{h}_{\vector \eps} &= \sum_{k \geq 0} \eps{k} \sup _{(x, t) \in W \times S^{1}} \sup_{i, z\in B(0, 1)} \abs{ d^k (h\circ \Psi_i\inv)(z)}
.\end{align*}
