...
:   For any $J, Y, v$ with 
    ...
    - $\im J \subset \endo(\RR^m)$
    - $\im Y, \im v \subset \RR^m$,
...
Proof
:   Differentiate the map $x \mapsto J(x)\cdot v(x)$:
    \begin{align*}
    J(x + Y) v(x + y) &= \qty{J(x) + (dJ)_x (Y) } ~\cdot~ \qty{v(x) + (dv)_x(Y)} \\
    &= J(x) \cdot v(x) + {\color{blue} J(x) \cdot (dv)_X(Y) + (dJ)_x(Y) \cdot v(x) + (dJ)_x(Y)\cdot (dv)_x(Y)
    .\end{align*}
