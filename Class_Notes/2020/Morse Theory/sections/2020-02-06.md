...
Otherwise, if it doesn't leave $U$, then there exists an $\eps>0$ such that $u(x_1, \phi) < 0 $ for all $S = \thset{(x,p) \in h(U) \suchthat \phi< \eps}$.
Thus there exists a $-\alpha<0$ such that $u<-\alpha$ on $S$.
