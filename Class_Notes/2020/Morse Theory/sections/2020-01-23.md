...
So $S_L = \Psi_{-\eps}\inv( W^s(p) \intersect M_{-\eps} )$, and we can define $\phi_L$ by the following composition:
...
Similarly, one can show $W^u(p) \cong D^{n-\lambda}$ and define a $\phi_R D^{\lambda} \cross S^{n-\lambda - 1} \injects M_1$.
