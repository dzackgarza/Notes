...
Corollary
: $i^*$ and $i_*$ are mutually inverse, order-preserving bijections $\spec(S\inv R) \to^{\iota^*} \from_{\iota_*} \theset{\mfp \in \spec(R) \suchthat \mfp\intersect S = \emptyset}$.

Lemma
: For $I\normal R$, $S$ a multiplicatively closed set, let $f: R\surjects R/I$ be the quotient map and $\bar{S} \definedas q(S)$. 
  Then $S\inv R/IS\inv R \mapsvia{\cong} \bar{S}\inv (R/I)$ by the map $\frac a s + IS\inv R \maps to \frac{a + I}{s + I}$.
...
Construction 2:
Define $S\inv M \definedas S\inv R \tensor_R M$, where $\iota: M \to S\inv M$ where $m\maps to 1\tensor m$.
