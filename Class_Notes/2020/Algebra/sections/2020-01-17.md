...
\begin{center}
\begin{tikzpicture}[baseline= (a).base, scale=0.6, every node/.style={scale=0.6}]
...
\draw[blue] (-4.5,1) .. controls (-4.5,0) .. (-2.25,-.75) .. controls(0, 0) .. (0, 1);
\draw[red] (2,-1.5) .. controls(2,.5) .. (2.4, .5) .. controls(2.60,.5) .. (2.6, -1.5);
\end{tikzpicture}
\end{center}
