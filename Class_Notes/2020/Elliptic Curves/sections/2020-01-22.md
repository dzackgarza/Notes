...
This says that $K\units/K^{\times n} = \chi(g_{L/K})$ where $L$ is the maximal abelian extension of $K$ of exponent dividing $n$, and $\chi(\wait) = \hom(\wait, \ZZ/n\ZZ)$, i.e. these are Pontrayagin duals.
