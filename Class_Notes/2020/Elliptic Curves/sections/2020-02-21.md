...
Theorem (Cartan)
: Let $K$ be a local field, $Q\injects K$ dense (so $K = \RR, \QQ_p$).
  Then if $G_1, G_2$ are $K\dash$analytic, and $\phi \in \hom_{\text{TopGrp}}(G_1, G_2)$, then $\phi \in \hom_{k\dash\text{analytic}}(G_1, G_2)$.
