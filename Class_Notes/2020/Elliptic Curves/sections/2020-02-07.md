...
c. For all divisors $D_1, D_2 \in \div V$, there is approximate linearity $h_{V, D_1 + D_2} = h_{V, D_1} + h_{V, D_2} = O(1)$.

d. If $D_1 \sim D_2$, then $h_{V, D_1} = h_{V, D_2} + O(1)$.

e. Let $D\subset \div V$ be effective and $B$ the base locus
