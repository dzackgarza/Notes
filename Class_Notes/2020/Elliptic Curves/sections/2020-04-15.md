...
A quick opaque way is to define $\mch$ as the $p\dash$adic upper half plane for some "connected" rigid analytic space, then $\mch(K) = \PP^1(\bar K) \setminus \PP^1(K)$.
There is a faithful action $\PGL(2, K)$ on $\mch(K)$ by linear fractional transformations.
