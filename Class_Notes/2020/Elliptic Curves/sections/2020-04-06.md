...
The succinct answer is that $\phi^d_N = \phi_N \tensor \chi_d$, where $\chi_d: g_k \to \theset{\pm 1}$ where $\sigma \mapsto 1$ iff $\restrictionof{\sigma}{K(\sqrt d)}$ is trivial and -1 otherwise.
