...
Now let $T^i = G^i \intersect A(k)[\tors]$, then $\# A(k)[\tors]' \leq \# \Phi(\FF_Q) \# A^0(\FF_q)$, and since we know everything about $A^0(\FF_q)$, we have
...
This generalizes the potentially good reduction case, since p.g.r. implies AR.
