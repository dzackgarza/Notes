...
If $A/\FF_q$ is an abelian variety of dimension $\beta$, we have the bound $\# A(\FF_q) \leq \floor{ q + 2\sqrt q + 1  }^\beta$, if $T_{\FF_q}$ is a torus of dimension $u$, then $\# T(\FF_q) \leq \qty{q+1}^u$.
Note that if $\dim T = 1$, then $T$ is isomorphic to either $\GG_m$ or $\ker( \GG_m/\FF_{q^2} \to \GG_m/\FF_q  )$.
