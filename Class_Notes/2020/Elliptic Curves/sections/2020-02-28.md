...
We want to generalize this: let $R$ be a valuation ring, then there exists a finite type smooth (not necessarily proper) $R\dash$model $A/R$ of $A/k$, the Néron model, such that $A(R) = A(K)$ and $0 \to A'(R) \to A(R) = A(k) \mapsvia{r} A(\FF_q) \to 0$.
