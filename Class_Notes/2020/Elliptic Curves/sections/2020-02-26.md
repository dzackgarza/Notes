...
Proof (Sketch)
: For $K$ a $p\dash$adic field, the absolute galois extension has galois group $\hat \ZZ$, then extend to $\bar K$ with galois group $H$ to obtain a SES
  $$
  1 \to H \to g_K \to \hat \ZZ \to 0
  $$
  Since $\hat \ZZ$ is a free profinite group, thus a projective object in that category of profinite groups, so this sequence splits.
