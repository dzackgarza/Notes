...
\addbibresource{/tmp/ci-uzs7M6uSZu/ModuliSpaces.bib}
...
\toprule
\endhead
title: ``Moduli Spaces''\tabularnewline
subtitle: ``University of Georgia, Spring 2020''\tabularnewline
author:\tabularnewline
- name: D. Zack Garza\tabularnewline
affiliation: University of Georgia\tabularnewline
email: dzackgarza@gmail.com\tabularnewline
date: Fall 2020\tabularnewline
author-note: "These are notes live-tex'd from a course in Moduli Spaces
taught by Ben Bakker at the University of Georgia in Spring
2020.\tabularnewline
Any errors or inaccuracies are almost certainly my own."\tabularnewline
cover-image: ``figures/cover.png''\tabularnewline
todos: true\tabularnewline
book: true\tabularnewline
\bottomrule
\end{longtable}
...
\emph{Problem}: \(F''\) is not a good moduli functor, as it is not
representable. Consider \(\operatorname{Spec}k[\varepsilon]\):
...
If \(F\) is representable, then it is actually the Zariski tangent
space.
...
\begin{align*} h_{{\mathbb{P}}^n_{\mathbb{Z}}}: {\mathbb{P}}_{\mathbb{Z}}^n {\operatorname{Sch}}^\operatorname{op}&\to {\operatorname{Set}}\\ s &\mapsto {\mathbb{P}}^n_{\mathbb{Z}}(s) = \left\{{ {\mathcal{O}}_s^{n+1} \to L \to 0}\right\} .\end{align*}
...
\begin{description}
\tightlist
\item[Example (Good to keep in mind)]
The twisted cubic:
\end{description}
...
\includegraphics{figures/2020-01-28-12:36.png}\\
...
\includegraphics{figures/2020-01-28-12:46.png}\\
...
\includegraphics{figures/2020-01-28-13:36.png}\\
...
\includegraphics{figures/2020-01-28-13:47.png}\\
...
\textbf{Claim:} There is a flat family \(Z\subset {\mathbb{P}}^3_S\)
...
Suppose otherwise. Then we have a family over \(H^\operatorname{red}\)
...
Consider \({\operatorname{Pic}}\), fix \(X/k\), start with a line bundle
...
\hypertarget{schelessingers-criterion}{%
...
Question: can we extend this to \(X'/{\mathbb{A}}\)? We have
...
This covers the obstruction, so now we need to show that the set of
...
Claim: Such a minimal \(I\) exists, i.e.~if \(I, I'\) satisfy the two
...
If it's the case that for all \(A' \to A\) small and
...
We conclude that the fibers are naturally torsors for \(\Der_k(R, M)\)
...
Hodge theory is now used: by Deligne-Illusie, for \(X\xrightarrow{f} S\)
...
Claim: Taking the associated cohomology sequence yields the Kummer
sequence:
\begin{align*} 0 \to G(k) / nG(k) \to H^1(k, G[n]) \to H^1(k, G)[n] \to 0 \end{align*}
where the RHS is the Weil-Chatalet group and the LHS is the Mordell-Weil
group.
...
injective resolution, applying the functor, then taking cohomology.

A concrete description is given by \(C^n(g, A) = \map(g^n, A)\) with
\begin{align*} d: C^n(g, A) \to C^{n+1}(g, A) \\ (df)(\sigma_1, \cdots, \sigma_{n+1} \coloneqq\sigma_1 f(\sigma_2, \cdots, \sigma_{n+1}) + \sum_{i=1}^n (-1) f(\sigma _1, \cdots, \sigma_{i-1}, \sigma_i, \sigma_{i+1}, \cdots, \sigma_{n+1}) + (-1)^{n+1} f(\sigma_1, \cdots, \sigma_n) .\end{align*}
