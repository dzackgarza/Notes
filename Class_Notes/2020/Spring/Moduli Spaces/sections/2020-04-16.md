...
The criterion for $F$ being surjective is equivalent to $\Der_{k}(R, M) \surjects \Der_{k}(R, M')$, which we identify as $\hom_{R}(\Omega_{R_{/k}}, M) \surjects \hom(\Omega_{R_{/k}'}, M')$
