...
Then $\mathrm{def} = \hom_{\OO_{x}\dashmod}(I_{Z}, \OO_{Z}) = \hom_{Z}(I_{Z}/I_{Z}^2, \OO_{Z}) = H^0(N_{Z/X})$ the normal bundle $N_{Z/X} = (I/I^2)\dual$ of the regular embedding, and $\mathrm{obs} = H^1(N_{Z/X})$.

Claim: If $H^1(\OO_{Z}(1)) = 0$ (e.g. if $d > 2g-2)$ then $M$ is smooth.
...
![](figures/image_{2020}-04-09-12-51-51.png)
...
![](figures/image_{2020}-04-09-12-55-01.png)\
...
Suppose $X_{0} \injects X' \in F(A') \to F(A)$ which restricts to $X_{0} \injects X$.
...
![](figures/image_{2020}-04-09-13-06-40.png)\
...
![](figures/image_{2020}-04-09-13-08-19.png)\

And thus if we have

![](figures/image_{2020}-04-09-13-08-42.png)\
...
1. $X = X_{0}$, then the following diagrams commute by taking the identity and the embedding you have:

  ![](figures/image_{2020}-04-09-13-15-09.png)\

  In particular, this implies H2.

2. Generally, these diagrams can be completed (and thus the gluing maps are bijective) if the map $\aut(X_{0}\injects X') \to \aut(X_{0} \injects X)$, the automorphisms of $X'$ commuting with $X_{0} \injects X$ is surjective.

Thus in this situation, there is only *one* way to fill in this diagram up to isomorphism:

![](figures/image_{2020}-04-09-13-18-59.png)

If we had two ways of filling it in, we obtain bridging maps:

![](figures/image_{2020}-04-09-13-20-07.png)\
...
:   The claim is that $\aut(X_{0} \injects X) = 1$ are always trivial.
    ...
    \[
    0 \to \Der_{k}(\OO_{X_{0}},\OO_{X_{0}}) \to  \Aut(X_{0} \injects X_') \to \Aut(X_{0} \injects X)
    .\]
...
But this equals $f(x, y) = \eps a \del_{x} f + \eps b \del_{y} f + \eps g(x ,y)$, so in fact only the class of $g\in S/(f, \del_{x} f, \del_{y} f)$.
