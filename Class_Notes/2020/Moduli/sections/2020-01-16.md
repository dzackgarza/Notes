...
We can compute the cohomology using Cech cohomology, i.e. taking the Cech resolution.
We can also assume $H^i(\PP^m; F(n)) = 0$ for $n \gg 0$, and the Cech complex vanishes in high enough degree.
...
Assuming $F$ is flat, and using the fact that flatness is a 2 out of 3 property, everything above is flat.
