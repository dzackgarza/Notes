...
For any sheaf $F$ over $X$, there is a hilbert polynomial $P_F(n) = \chi(F) \in \ZZ[n]$.
The cohomology of $F$ isn't change by the pushforward into $\PP_n$ since it's a closed embedding, i.e. $\chi(X, F) = \chi(\PP^n, i_* F)$.
