...
*Proof of Claim 2:*
$F_i(S) = \theset{\mathcal O_S^{n+1} \to L \cong \mathcal O_s \to 0, s_i \neq 0}$, but there are no conditions on the sections other than $s_i$, so specifying $F_i(S)$ is equivalent to specifying $n$ functions $f_1 \cdots f_n \in \mathcal O_S(s)$ with $f_i \neq 0$.
