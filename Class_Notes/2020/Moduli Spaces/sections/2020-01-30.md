...
Recall how we constructed the hilbert scheme of hypersurfaces $\hilb_{\PP_k^n}^{P_{m, d}} = \PP H^0(\PP^n; \OO(d))\dual$.
A section $\hilb_{\PP_k^n}^{P}(s)$ corresponds to $z\in \PP^n_s$.
We can look at the exact sequence

\begin{align*}
0 \to I_Z(m) \to \OO_{\PP_S^n} \mapsvia{restrict} \OO_z(m) \to 0
.\end{align*}
...
It is determined by these polynomials because we have a sequence $0 \to I_Z(m_0) \to \OO(m_0) \to \OO_Z(m_0) \to 0$.
