...
If $X/S$ is *quasiprojective* (so locally closed, i.e. $X\injects \PP^n_S$), then $\hilb_{X/S}^P(T) \definedas \theset{z\in X_T \text{ projective, flat over $S$, fiberwise ? }}$ is still representable, but now by a quasiprojective scheme.
