...
*Proof of Claim 2:*
$F_i(S) = \theset{\mathcal O_S^{n+1} \to L \cong \mathcal O_s \to 0, s_i \neq 0}$, but there are no conditions on the sections other than $s_i$, so specifying $F_i(S)$ is equivalent to specifying $n-1$ functions $f_1 \cdots \hat f_i \cdots f_n \in \mathcal O_S(s)$ with $f_k \neq 0$.
...
For any subset $I \subset \theset{0, \cdots, n}$, we can define $F_I = \theset{\mathcal O_s^{n+1} \to L \to 0, s_i\neq 0 \text{ for } i\in I} = {\bigtimes_{i\in I}}_F F_i$, and $F_I \to F_J$ when $I \supset J$.
