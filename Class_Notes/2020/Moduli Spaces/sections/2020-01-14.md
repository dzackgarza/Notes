...
$$
F_i(s) = \theset{\OO_s^{n+1} \to L \to 0 \suchthat s_i \text{invertible}}
$$ 
...
:   Because projective space represents this functor, there is a universal object:
    \begin{center}
    \begin{tikzcd}
    \OO_{\PP_{\ZZ}^n}^{n+1} \arrow[rr] &  & L \arrow[dd, no head, Rightarrow] \arrow[rr] &  & 0 \\
    &  &                                              &  &   \\
    &  & \OO_{\PP_{\ZZ}^n}(1)                            &  &  
    \end{tikzcd}
    \end{center}
...
\node[draw,circle,minimum size=10cm,inner sep=0pt, label={[xshift=-1.5cm, yshift=-1.0cm] $X_\alpha$ }] at (-3,3) {};
\node[draw,circle,minimum size=10cm,inner sep=0pt, label={[xshift=1.5cm, yshift=-1.0cm] $X_\beta$ }] at (3,3) {};
\node[draw,circle,minimum size=10cm,inner sep=0pt, label={[yshift=-10.0cm] $X_\gamma$ }] at (0,-3) {};
\end{tikzpicture}
\end{center}

Then $\phi_{\alpha\beta}: X_{\alpha\beta}\mapsvia{\cong} X_{\beta\alpha}$ must satisfy the cocycle condition:

  1. $$\phi_{\alpha\beta}\inv \qty{ X_{\beta\alpha} \intersect X_{\beta\gamma} } = X_{\alpha\beta} \intersect X_{\alpha \gamma},$$ 
...
:   Any representable functor is a Zariski sheaf precisely because the gluing is a coequalizer.
    ...
    $$
    \disjoint_{\alpha\beta} T_{\alpha\beta} \to \disjoint_{\alpha}T_\alpha \to T$, since giving a local map to $X$ that agrees on intersections if enough to specify a map from $T\to X$.
