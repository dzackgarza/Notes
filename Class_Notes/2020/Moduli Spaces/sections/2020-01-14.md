...
\arrow[rr, shift right=.5ex,"g_{\alpha\beta}"] 
&  & \disjoint_{\alpha} X_\alpha \arrow[rr] &  & X
\end{tikzcd}
\end{center}

$$
\disjoint_{\alpha\beta} X_{\alpha\beta} \mapsvia{f_{\alpha\beta}, g_{\alpha\beta}} \coprod X_\alpha \to X
$$ 
where 
...
\node[draw,circle,minimum size=10cm,inner sep=0pt] at (-3,3) {$X_\alpha$};
\node[draw,circle,minimum size=10cm,inner sep=0pt] at (3,3) {$X_\beta$};
\node[draw,circle,minimum size=10cm,inner sep=0pt] at (0,-3) {$X_\gamma$};
\end{tikzpicture}
\end{center}
