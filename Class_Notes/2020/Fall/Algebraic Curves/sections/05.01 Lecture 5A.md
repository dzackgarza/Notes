...
Let $K$ be a field with $v$ a rank one (discrete) valuation with valuation ring $R$.
...
Also note that we view $\mathfrak{m}_{w_i}$ as an ideal lying over $\mathfrak{m}_v$, and there is an inclusion of residue fields $R_v/\mathfrak{m}_v \injects R_{w_i} / \mathfrak{m}_{w_i}$.
:::
