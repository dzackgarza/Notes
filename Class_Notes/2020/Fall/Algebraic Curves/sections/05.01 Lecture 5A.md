...
:::{.corollary title="?"}
For $v \in \places$, define the **residue field** of the local ring $R_v$ as $k(v) \da R_v / \mathfrak{m}_v$.
Then $k(v)/ k$ is a finite degree extension.
:::

:::{.proof title="of corollary"}
If $R$ is a domain with maximal ideal $\mathfrak{p}$, then the quotient map factors through the localization, giving $R/\mathfrak{p} = R_{\mathfrak{p}} / \mathfrak{p} R_{\mathfrak{p}}$:

\begin{tikzcd}
R \ar[r]\ar[dr] & R_{\mathfrak{p}} \\
 & R/\mathfrak{p}
\end{tikzcd}
