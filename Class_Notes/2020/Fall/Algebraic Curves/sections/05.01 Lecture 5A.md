...
:::{.remark}
This is a way for there to not be points on the curve, so $\Sigma(K/k)$ is empty, but it's not the deepest reason -- this is a cheap trick to produce "pointless" function fields.
It can be empty in many different ways!
:::

:::{.exercise title="?"}
Show that for $K/k$ TFAE:
