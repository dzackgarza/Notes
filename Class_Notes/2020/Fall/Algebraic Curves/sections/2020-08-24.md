...
For $A\subset K$ a subring of a field, we'll be interested in the place $\Sigma = \ts{\text{Valuation rings } R_v \text{ of } K} \st A \subset R_v \subsetneq K$.
...
We can define an equivalence of valuations and set $\Sigma(K/k) \da \ts{\text{Nontrivial valuations } v\in K \st v(x) \geq 0\, \forall x\in k\units}$.
