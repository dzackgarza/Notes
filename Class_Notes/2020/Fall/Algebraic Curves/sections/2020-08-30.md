...
Actually $\tilde K_N$ depends on the choice of $E$: difference choices differ by a quadratic twist and the $\rho_N$ differ by a quadratic character on $g(\QQ(t))$.

To fix this, we look at the *reduced galois representation*:
\[  
\bar \rho_N: g(\QQ(t)) \to \GL(2, \ZZ/N\ZZ) \surjects \GL(2, \ZZ/N\ZZ)/\ts{?}
.\]
