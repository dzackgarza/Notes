...
- Then let $M\da \inf S$, so we have $\beta < M$ by assumption.
...
\[  
x_{n_k} \leq \beta \quad \forall k \implies \lim_{k\to\infty} x_{n_k} \leq \beta
\]
  where we've used order-limit laws.

- Defining $S$ to be the set of all subsequential limits of $\ts{x_n}$, we now have $L\da \lim_k x_{n_k} \in S$, which is a subsequential limit satisfying $L\leq \beta$, and 
\[  
\inf S \leq L \leq \beta
,\]

- But $\inf S \da \liminf_n x_n$ by definition (i), so this contradicts $\beta < \liminf_n x_n$.
