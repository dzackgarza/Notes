...
- The set of subsequential limits is then just $\ts{-1, 0}$, which is a finite set and thus achieves its sup and inf.
...
- **Claim**: $\liminf \floor{\sin(n)} = 0$.
  - By the exact same argument applied to intervals of the form $[3k\pi,3k\pi + \pi]$ where $-1 < \sin(n) < 0$, w 
:::
