...
\[  
B \da \ts{n\in \NN \st x_n \leq \beta}
,\]
  then $B$ is countably infinite.

- But then $B\subseteq \NN$ implies that
\[  
\inf_{n\in N} x_n \leq \inf_{n\in B} x_n \leq \beta
,\]

- Applying order-limit laws, we then have
\[  
\lim_{n\to \infty }\inf_{n\in N} x_n \leq \inf_{n\in B} x_n \leq \beta
,\]
