...
- Defining $S$ to be the set of all subsequential limits of $\ts{x_n}$, we now have $L\in S$, where $L$ is a subsequential limit satisfying $L\leq \beta$, 

- $\inf S$ is a lower bound for $S$, so
\[  
