...
- Suppose $x_n \leq M$ for all $n$ and let $S \da \liminf_{n\to\infty} x_n$ be the infimum of subsequential limits.
- Let $\ts{x_{n_k}}$ be an arbitrary convergent subsequence, then for every $k$ we have $x_{n_k} \in \ts{x_n}$, so $\abs{x_{n_k}} \leq M$.
- By order limit laws,
\[  
...
- Since the map $x\mapsto \abs{x}$ is continuous, we can pass the limit through the absolute value to obtain
\[  
