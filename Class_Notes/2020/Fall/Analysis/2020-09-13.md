...
\hfill
- Suppose $\ts{x_n}$ is bounded and $\limsup \abs{x_n} = 0$.
- Using the subsequential definition of limsup, this says that the least upper bound of subsequential limits of $\ts{\abs{x_n}}$ is 0.
- But every subsequence is a sequence of positive numbers, so this forces every convergent subsequence to converge to zero.
