...
\hfill
- Suppose $\ts{x_n}$ is bounded and $\limsup \abs{x_n} = 0$.
- Then using the subsequential limit definition, define $S\da \ts{L \st \exists \abs{x_{n_k}} \to L}$.
  Note that the overall sequence we're considering here is $\ts{\abs{x_n}}$.
- Every convergent subsequence $\ts{\abs{x_{n_k}}}$ is a sequence of positive numbers, so $\lim_{k\to\infty}\geq 0$.
:::
