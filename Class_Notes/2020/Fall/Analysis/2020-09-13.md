...
- Then $\beta < \inf S$, and recall that $\inf S$ is the unique number such that $S\leq x$ for all $x\in S$ and for any $S'>S$, there exists an $x'\in S$ such that $S\leq x' < S'$. 
\[  
M = \inf S \implies
\begin{cases}
