...
Fixing any point $y_0 \in Y$, we have $X\cong X_{y_0} \da X\cross \ts{y_0} \subseteq X\cross Y$, so we can identify $G_U \subset X$ with $G_U\subset X_{y_0}$ inside a $Y\dash$fiber the product.
But then $G_U = X_{y_0} \intersect U$ in $X\cross Y$, where $U$ is closed in $X\cross Y$ and thus closed in $X_{y_0}$, and $X_{y_0}$ is trivially closed in itself.
This exhibits $G_U$ as the intersection of two closed s
:::
