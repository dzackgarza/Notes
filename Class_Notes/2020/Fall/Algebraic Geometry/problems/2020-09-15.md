...
\
Using the order-preserving property of $V(I(\wait))$ (noting that $V(\wait), I(\wait)$ are individually order-reversing), we have
\[  
X\subseteq \bar X \implies V(I(X)) \subseteq V(I(\bar X)) = \bar X
,\]
where we've used the above claim in the last equality.
