...
But then $\union_{i\in I}U_i \subseteq X_1$, and since $\ts{U_i}$ was a cover, this forces $X\subseteq X_1$ and thus $X_2 = \emptyset$, a contradiction.
:::


:::{.solution title="b"}

> Short proof: Every irreducible set is connected, so apply part (a)?
...
:::{.solution title="a"}
Toward a contradiction, if $f(X) = Y_1 \disjoint Y_2$, then $f\inv(f(X)) \subseteq X$ on one hand, and $f^{-1}(f(x)) = f\inv
:::
