...
In particular, we have $A(X\cross Y) = A(X) \tensor_k A(Y)$, so it suffices to show that if $R, S$ are $k\dash$algebras and integral domains, then $R\tensor_k S$ is again an integral domain.
:::
