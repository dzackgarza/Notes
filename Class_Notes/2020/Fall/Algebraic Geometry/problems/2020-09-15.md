...
Since $\ts{U_i} \covers X$, for each $i$ this would force one of $U_i \subseteq X_1$ or $U_i \subseteq X_2$, since otherwise $U_i \intersect X_1 \intersect X_2$ would be nonempty.
\
But then for every $j\neq i$, we have $U_j \intersect U_i$ nonempty by assumption, and so $U_j \subseteq X_1$
:::
