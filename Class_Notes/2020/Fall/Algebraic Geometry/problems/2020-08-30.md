...
:::{.proposition}
In $\AA^n/k$, letting $X_j$ be the $x_j\dash$coordinate axis, we have 
\[  
X_j = V\qty{\prod_{i\neq j}x_i }
.\]
...
$\subseteq$: 
Let $\vector p \in X_j$, where $\vector p = \thevector{p_1, \cdots, p_n}$ in coordinates with $p_i = 0$ for $i\neq j$.

Then if $f(x_1, \cdots, x_n) \in \gens{\prod_{i\neq j} x_i}$, then $f$ is of the form 
\[  
f(x_1, \cdots, x_n) = g(x_1, \cdots, x_n) \prod_{i\neq j} x_i
,\]
and thus
\[  
f(\vector p) = g(p_1, \cdots, p_n) \prod_{i\neq j} p_i = 0
,\]
since $\prod_{i\neq j} p_i = 0$ and $\kx{n}$ is a domain.

$\supseteq$:
Let $\vector p\in A^n/k$ and $f\in \gens{\prod_{i\neq j} x_i}$, and suppose $f(\vector p) = 0$.
:::


:::

We thus have $X = X_1 \union X_2 \union X_3$, where
