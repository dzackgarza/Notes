...
Let $J\da I(X)$, and toward a contradiction, suppose $J = \gens{r, s}$.
Define $\mfm \da \gens{x, y, z}$ and
a quotient map $$\pi: J \to J/\mfm J$$
and consider the images $\pi(r), \pi(s)$.

Note that $J/\mfm J$ is a $k[x, y, z]/\mfm\dash$module, and since $k[x, y, z]/\mfm \cong k$, it is in fact a $k\dash$vector space.
In this case, $\pi(r), \pi(s)$ generate $J/\mfm J$ as a $k\dash$module, and so $$\dim_k H/\mfm J \leq 2.$$
:::
