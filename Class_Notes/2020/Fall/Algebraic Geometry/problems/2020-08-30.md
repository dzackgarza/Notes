...
$\subseteq$: Let $\vector p \in X_j$, where $\vector p = \thevector{p_1, \cdots, p_n}$ in coordinates with $p_i = 0$ for $i\neq j$.
...
,\]
which is necessarily zero because the product term is zero and $\kx{n}$ is a domain.
