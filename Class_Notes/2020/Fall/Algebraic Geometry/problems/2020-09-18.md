...
Since $I_0\subset X$ and $\ts{U_i}$ covers $X$, we can find some $U_{0}\in \ts{U_i}$ such that $I_0\intersect U_0$ is nonempty, since otherwise there would be a point in $I_0 \intersect \qty{\}
:::
