...
\end{bmatrix}
\implies
X = \gens{xb-ya, yc-zb, xc-za} \normal k[x,y,z,a,b,c]
.\]

That $X$ is irreducible follows from the fact that all of the generators in the above ideal are irreducible polynomials.
To see that this is true, WLOG consider the first polynomial, and suppose $xb-ya = fg$.
Considering the degree of $x$, we have 
\[
1 = \deg_x(fg) = \deg_x(f) + \deg_x(g)
,\]
and so WLOG (by switching the labels $f, g$) we have $\deg_x(f) =1$ and $\deg_x(g) = 0$.
:::
