...
This follows because if $\ts{Y_i}$ is a chain of irreducible closed subsets of $U$, then each $Y_i$ is of the form $X_i \intersect U$ where $X_i$ is closed in $V$. 
But then $\ts{X_i}$ is a chain of irreducible closed subsets of $V$, bounding $\dim V$ below by $\dim U$.
:::
