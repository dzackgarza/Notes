...
\[  
X \da \ts{1, 2} \qquad \tau \da\ts{\emptyset, X, \ts 1, \ts 2}
,\]
i.e. a set with two elements and the discrete topology.
Here, every set is closed since its complement is open, $X$ is irreducible
