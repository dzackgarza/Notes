...
- Let $f(x) = 0$, then $\AA^n = V\qty{\ts{f}}$.
- Any point $(a_1, \cdots, a_n)\in \AA^n$ is uniquely determined by $V(x_1 - a_1, \cdots, x_n - a_n) = \theset{a_1, \cdots, a_n}$.
- For any finite set $r_1, \cdots, r_k \in \AA^1$, there exists a polynomial $f\in k[x]$ whose roots are $r_i$.
:::
