...
Then $T_0(X)\dual \da \hom_k(T_0X, k) \cong I_X(p) / I_X(p)^2$.
:::

:::{.remark}
Note that the hom involves an affine embedding, but the quotient of ideals does not.
We know that the category of affine varieties is equivalent to the category of reduced $k\dash$algebras, so if $A(X)$ is the coordinate ring and $I_X(p)$ is the maximal ideal in $A(X)$ of regular functions vanishing at $p$.
:::
