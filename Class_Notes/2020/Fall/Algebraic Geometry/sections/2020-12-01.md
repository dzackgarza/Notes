...
That $\phi$ is surjective follows from:

![Image](figures/image_2020-12-01-10-30-44.png)

the fact that if $\bar x_1, \cdots, \bar x_n \in I_X(p)$, then..


For injectivity, suppose $\bar \phi(\bar f) = 0$, then $\ro{f_1}{T_0(X)} = 0$, so $f_1$ is the linear part of some $f'$
