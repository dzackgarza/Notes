...
.\]
Note that $I_X(p) = \gens{\bar x_1, \cdots, \bar x_n} \in k[x_1, \cdots, x_n]/I(X)$.
We need to check that this well defined: e.g. given $\bar x_1 - \bar x_2^2 \in A(X)$, we first lift to $x_1 - x_2^2 \in A(\AA^n)$, restrict to the linear part $x_1$, then restrict to $T_0(X)$.
:::
