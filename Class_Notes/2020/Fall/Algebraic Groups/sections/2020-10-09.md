...
\[  
H^i(\lambda) 
&= R^i \ind_{P_\alpha}^G \qty{ \ind_B^{P_\alpha} \lambda } \qquad\text{by c}\\
&= R^i \ind_{P_\alpha}^G \qty{ R^1 \ind_B^{P_\alpha} s_\alpha\cdot \lambda } \qquad\text{by corollary}\\
...
&= \lambda - \qty{ \inner{\lambda}{\alpha\dual} + 1 }\alpha \hspace{4em} \text{using } \inner{\rho}{\alpha\dual} = 1 \\ \\
\implies 
