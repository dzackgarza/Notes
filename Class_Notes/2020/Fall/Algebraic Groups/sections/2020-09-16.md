...
For $A\dash$modules $M, N$, we can form the $A\dash$module $M\tensor_k N$ with
\[  
\Delta(a) = \sum a_i \tensor a_j \\ \\
a(m\tensor n) = \sum a_1 m \tensor a_2 n
.\]

If $M$ is finite-dimensional over $A$, then $M\dual = \hom_k(M, k) \ni f$ and we can define $(af)(x) \da f(s(a)x)$ for $a\in A, x\in M$.
