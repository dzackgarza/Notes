...
![Image](figures/image_2020-10-28-14-25-12.png){width=350px}

If $w\lambda$ is a vertex with $\mu \in C_\ZZ$, then $T_\lambda^\mu(H^0(w\cdot \lambda))$ can have six factors:

![Weight where the translation has six factors](figures/image_2020-10-28-14-27-05.png){width=350px}

:::{.proposition title="?"}

Suppose $\lambda \in \bar{C}_\ZZ$ and $\mu \in C_\ZZ$, and let $w\in W_p$ where $w\cdot \lambda \iin X(T)_+$.
Then $T_\lambda^\mu (H^0(w\cdot \lambda))$
:::
