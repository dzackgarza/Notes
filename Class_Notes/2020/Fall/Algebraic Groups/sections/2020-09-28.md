...
\begin{tikzcd}
& H^0(\lambda) & \\
L(\lambda) \ar[ur, hookrightarrow] & & T(\lambda)\ar[ul] \\
& V(\lambda) \ar[ul] \ar[ur, hookrightarrow]
\end{tikzcd}
\end{center}
...
.\]
where to compute $\Ext_G^n(M, N)$ we take an injective resolution $N \injects I_*$, apply $\hom_G(M, \wait)$
