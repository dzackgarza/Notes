...
2. All weights of $I_j$, say $\mu$ satisfy $\ht(\mu) \geq j$.
:::
\[  
...
\[  
E^{i, j}_2 = \Ext_G^i(N, R^j \ind_H^G M) \imlies \Ext_H^{i+j}(N, M)
\]
...
![Image](figures/image_2020-09-28-14-49-07.png)
...
b. If $H^j(P, M) \neq 0$ then there exists $\lambda$ a weight of $M$ with $-\lambda \in \NN \Phi^+$ and $\ht(-\lambda) \geq j$.
:::
