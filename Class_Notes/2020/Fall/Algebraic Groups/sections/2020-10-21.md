...
We have $b(\lambda) \in \mathcal{B}(G)$ and $"b(\lambda)" \substeq X(T)_+ \intersect W_p \cdot \lambda$, i.e. we have strong linkage.
...
Then $"b(\lambda)" = W_p^{(r)} \cdot \lambda \intersect X(T)_+$, where $W_p^{(r)} = W\semidirect p^r \ZZ\Phi$.
:::
