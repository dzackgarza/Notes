...
{\tiny \textit{Last updated:} 2020-10-21 }
\end{flushleft}
...
\([H^0 \lambda : L(\mu)] \neq 0]\), then \(\mu \uparrow \lambda\). In
other words, there's a series of reflections sending \(\mu\) to
...
\(\inner{\lambda + \rho}{\alpha\dual} \geq 0\) for all
\(\alpha\in \Delta\). Suppose \(\mu \in X(T)_+\). If
\([H^i w\cdot \lambda : L(\mu)] \neq 0]\) for some \(i\geq 0\), then
\(\mu \uparrow \lambda\).
...
Brauer-Humphreys reciprocity, so \(\sigma \in W_p \cdot \mu\). Similarly
\([I(\mu): L(\gamma)] \neq 0\) implies that
\(\gamma \in W_p \cdot \mu\), and continuing in this way we can write
\(I_1 = \bigoplus_{j=1}^t I(\gamma_j)\) with each
\(\gamma_j \in W_p \cdot \mu\). So all of these weights are strongly
linked to \(\mu\).
