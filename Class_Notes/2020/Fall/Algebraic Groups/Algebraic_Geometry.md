...
If we have a nonzero composition factor in the induced/Weyl module, so 
$[H^0 \lambda : L(\mu)] \neq 0]$, then $\mu \uparrow \lambda$.
...
Suppose $\mu \in X(T)_+$.
If $[H^i w\cdot \lambda : L(\mu)] \neq 0]$ for some $i\geq 0$, then $\mu \uparrow \lambda$.
:::
...
We can conclude that $[I(\mu) : H^0(\sigma)] = [H^0(\sigma): L(\mu)] \neq 0$ by Brauer-Humphreys reciprocity, so $\sigma \in W_p \cdot \mu$.
Similarly $[I(\mu): L(\gamma)] \neq 0$ implies that $\gamma \in W_p \cdot \mu$, and continuing in this way we can write $I_1 = \bigoplus_{j=1}^t I(\gamma_j)$ with each $\gamma_j \in W_p \cdot \mu$.
