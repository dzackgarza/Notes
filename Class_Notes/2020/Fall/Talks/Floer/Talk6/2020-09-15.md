...
- We have the pregluing function $w_\rho \in C^\infty_{\searrow}(x, z)$ from previous section (exponential decay).

- Want to construct true solution $\psi_\rho \in \mathcal{M}(x, z)$, so $\mathcal{F}(\psi_p) = 0$.

- $\mathcal{F}_\rho$ will be $F\circ \oldexp_{w_\rho}$ expanded in the bases $Z_i$ from a trivialization of $TW$.
