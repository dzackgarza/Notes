...
Moreover, the number of nondegenerate fixed points of a time one diffeomorphism is bounded below by the minimal number of critical points of a *Morse function* on $W$.
...
:::{.theorem title="Yields a Chain Complex"}
\[
\del^2 = 0
\]
:::
...
- $\mathcal{F}_\rho$ will be $F\circ \oldexp{w_\rho}$ expanded in the bases $Z_i$ coming from a trivialization of $TW$.
