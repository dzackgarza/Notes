...
\iff
\correspond{ \text{Solutions to } \\ \dd{u}{s} + J(u) \dd{u}{t} + \nabla H(u) = 0 }
.\]
\normalsize
...
- Show that all Floer solutions connecting two consecutive critical points are *also* Morse trajectories, and $d\mathcal{F}_v$ is surjective along these trajectories
- Yields equality of complexes
...
\int_\RR \norm{\dd{u}{s}}^2 \,ds \\
\mathcal{M} &\da \ts{
u \in C^\infty(\RR, V) \st \dd{u}{s} + \nabla f = 0, \quad E(u) < \infty
}
.\]
...
- WTS: trajectories of $H_k$ appearing in the Floer complex are exactly those appearing in the Morse complex.
...
- Consider case where indices differ by $1$: using broken trajectories theorem, extract a subsequence converging to some $v\in \mathcal{M}(x, y, H)$.
  ...
  - Get a contradiction from taking $k\gg 0$ and using $v_{n_k}(s, t) = v(s + \sigma_k, t) = v(s + \sigma_k)$, which does *not* depend on time
...
- Use gluing from last chapter: $\hat v_{n_k} \in \im(\hat\phi)$ for $k\gg 0$, contradicting the fact that $v_{n_k}$ doesn't depend on $t$
