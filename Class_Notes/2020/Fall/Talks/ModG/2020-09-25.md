...
- So define $\Map(X, Y) = \hom_{\Top}(X, Y)$ equipped with the compact-open topology.
  ...
  \[  
  X= I \da [0, 1] & \leadsto\quad P(Y)  \da \ts{f: I\to Y} = Y^I  \\
  X= S^1 &\leadsto\quad \Omega(Y)  = \mathcal{L}(Y; x_0) \da \ts{f: S^1\to Y } = Y^{S^1}
  .\]
  ...
  &\cong \Map(I, \Map(I, \Map(X, Y))) \\
  &\cong \Map(I, \Map(Y\cross I, X))
  .\]
  Interpretation: this is a path in the space of paths
