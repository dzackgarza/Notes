...
- For $X, Y$ topological spaces, consider 
  ...
  .\]
  - Importance in homotopy theory: the path space fibration $\Omega(Y) \to P(Y) \mapsvia{\gamma \mapsto \gamma(1)} Y$ (plays a role in "homotopy replacement", allows you to assume everything is a fibration and use homotopy long exact sequences).
