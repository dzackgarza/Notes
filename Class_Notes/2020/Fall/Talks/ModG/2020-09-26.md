...
- For $f$ any automorphism, the induced map $f_*: \ZZ^2 \to \ZZ^2$ is a group automorphism, so we can consider the map
\[  
\tilde \sigma: \Map(X,X) &\to \GL(2, \ZZ) \\
f &\mapsto f_*
.\]

- This will descend to the quotient $\MCG(X)$ iff $\tilde \sigma$ is constant 
:::
