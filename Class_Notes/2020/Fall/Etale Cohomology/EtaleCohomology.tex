...
\(\iota_{\bar x}^* \underline{\zlz} = \zlz\). If \(U\) had more than one
connected component, then the first definition would involve a limit
over \(\mathcal{F}(U)\) which are all copies of \(\zlz\). But given
this, you can always find a connected covering. So the \((U, \bar u)\)
...
the strict Hensilization (where we're picking up the version that has an
