...
- Trying to define a differential
\[  
...
&= \dim \mathcal{L}(x, y) \mod 2
,\]

  where $\mathcal{L}(x, y)$ will be the space of such trajectories

  - Want to show $\mathcal{L}(x, y)$ is a compact $0\dash$manifold.
...
:::
:::{.theorem title="9.2.1"}
Let $(H, J)$ be a regular pair with $H$ nondegenerate, and let $x, z$ be two periodic trajectories of $H$ such that
\[  
\mu(x) = \mu(z) + 2
.\]
...
\[  
\bd \bar{\mathcal{L}}(x, z) = \bigcup_{\ts{}} \mathcal{L}(x ,y) \cross \mathcal{L}(y, z) \qquad\text{where} \mathcal{I}_{x, z} = 
\ts{
y \st \mu(x) < \mu(y) < \mu(z)
}
.\]
