...
- Newton-Picard method, general idea
  ...
  - Allows finding zeros of $f$ given an approximate zero $x_0$, using the 


- Applying Newton-Picard

  - Decompose \[W^{1, p}(\RR\cross S^1; \RR^{2n})= \ker(L_{\rho}) \oplus W_{\rho}\perp\]

  - $\mathcal{F}_\rho$ will be $F\circ \oldexp{w_\rho}$ expanded in the bases $Z_i$ coming from a trivialization.

  - $L_\rho = (d\mathcal{F}_\rho)_0$ will be the linearization at zero.
