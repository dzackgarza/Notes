...
x &\mapsto \del(\del(x)) \\
&= \del \qty{\sum_{\mu(y) = k-1} n(x, y) y} \\
&= \sum_{\mu(y) = k-1} n(x, y) \del(y) \\
&= \sum_{\mu(y) = k-1} n(x, y) \qty{\sum_{\mu(z) = k-2} n(x, z) z} \\
&= \sum_{\mu(y) = k-1} \sum_{\mu(z) = k-2} n(x, y)  n(x, z) z
\\ \\
.\]

  - When $\abs{\mu(x) - \mu(y)} = 1$, get a compact 0-manifold $\mathcal{L}(x, y)$, so $n(x, y)$ is well-defined already.

  - When $\abs{\mu(x) - \mu(y)} = 2$, non-compact 1-manifold, so we compactify by adding in *broken trajectories* to get $\bar{\mathcal{L}}(x, y)$.
  
  - Want to show $\mathcal{L}(x, y)$ is a compact $0\dash$manifold, proceed by showing $\bar{\mathcal{L}}(x, y)$ is a compact $1\dash$manifold with $\mathcal{L}(x, y)$ in its boun
...
- For $0\leq k \leq \ell$, sequences $\ts{s_n^k}$.
