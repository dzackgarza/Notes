...
- $(M, \omega)$ with $\omega \in \Omega^2(M)$ is a symplectic manifold
- $H \in C^\infty(M; \RR)$ a Hamiltonian with $X_H$ the corresponding vector field.
  - Recall, could be time-dependent. If so, assumed 1-periodic.
...
- Assumption of *symplectic trivialization*: for every $u\in C^\infty(S^2; M)$ there exists a symplectic trivialization of the fiber bundle $u^* TM$ 
