...
- Define an interpolation $w_\rho$ from $u$ to $v$ in the following way: let $e^{\wait} = \oldexp_{y(t)}(\wait)$ and $\ln(\wait) = \oldexp\inv_{y(t)}(\wait)$, then

\[  
w_\rho(s,t) \da 
\begin{cases}
{\color{blue} u(s+\rho, t)} & s\leq -1 \\ \\
\exp{\beta^-(s)\ln({\color{blue} u(s+\rho, t) }) + \beta^+(s)\ln({\color{green} u(s-\rho, t)} )} & ? \\ \\
u(s-\rho, t) & s\geq 1
\end{cases}
.\]
...
- Can make this condition for large $\rho$, since 
  \[  
