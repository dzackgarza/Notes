...
\begin{equation}w_{\rho}(s, t)=\left\{\begin{array}{ll}
u(s+\rho, t) & \text { if } s \leq-1 \\
\oldexp _{y(t)}\left(\beta^{-}(s) \oldexp _{y(t)}^{-1}(u(s+\rho, t))\right. & \\
\qquad \left.+\beta^{+}(s) \oldexp _{y(t)}^{-1}(v(s-\rho, t))\right) & \text { if } s \in[-1,1] \\
