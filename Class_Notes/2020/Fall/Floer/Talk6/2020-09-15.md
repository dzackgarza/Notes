...
- Defined moduli space of solutions:
\[  
\mathcal{M}(x, y) &= \ts{\text{Contractible finite-energy solutions connecting }x, y } 
\\ \\
\mcm &= \ts{\text{All contractible finite-energy solutions}}
= \bigcup_{x, y} \mathcal{M}(x, y)
.\]

- Defined the space of trajectories connecting $x$ to $y$:
\[  
...
.\]
  ...
  \[  
  \tilde u_n \converges{n\to\infty}\too \tilde u_n \iff \exists \ts{s_n}\subset \RR \text{ such that } u_nm
  .\]
...
Note that as a corollary, $\bd^2 = 0$.
