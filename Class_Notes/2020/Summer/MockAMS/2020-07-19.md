...
- $\RP^n$ as the space of lines in $\RR^{n+1}$.
- $\mcm_g$ the moduli space of compact Riemann surfaces (curves) of genus $g$.


Here $E \definedas \theset{(\vector v, L)}$ where $L$ is a line through $\vector 0$ and $\vector v$ is a vector on $L$.
