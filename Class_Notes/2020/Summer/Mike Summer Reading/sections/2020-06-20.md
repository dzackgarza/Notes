...
Remark
:   Attempting to define a function $f: M\to \RR$ to be smooth iff $f\circ \phi\inv: \RR^n \to \RR$ is smooth for each $\phi$ may not work because many atlases give the "same" smooth structure.

    For example,
    \begin{align*}
    \begin{array}{l}
    \mathscr{A}_{1}=\left\{\left(\mathbb{R}^{n}, \operatorname{Id}_{\mathbb{R}^{n}}\right)\right\} \\
    \mathscr{A}_{2}=\left\{\left(B_{1}(x), \operatorname{Id}_{B_{1}(x)}\right): x \in \mathbb{R}^{n}\right\}
    \end{array}
    .\end{align*}
