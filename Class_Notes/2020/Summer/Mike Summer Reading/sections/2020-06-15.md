...
:   Let $M \definedas M_1 \times \cdots \times M_k$ be a product of manifolds of dimensions $n_1, \cdots, n_k$ respectively.
    ...
    - Since each $\phi_i$ is a homeomorphism onto $\phi_i(U_i)$ (by the definition of a chart on $M_i$), $\Phi$ is a homeomorphism onto its image since $\Phi(\prod U_i) = \prod \phi_i(U_i)$.
