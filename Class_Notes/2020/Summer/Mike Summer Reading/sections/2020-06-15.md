...
Some facts from the appendix:

Subspaces $A\subseteq X$:
...
The collection of sets of the form $\prod_{i=1}^N U_i$ with each $U_i$ open in $X_i$ forms a basis for the product topology on $\prod_{i=1}^N X_i$.
...
- A map $f: \prod X_i \to Y$ *out* of a product need not be continuous when its component functions 
- Projections $\pi_i$ are continuous.
