...
:   Let $M \definedas M_1 \times \cdots \times M_k$ be a product of manifolds of dimensions $n_1, \cdots, n_k$ respectively.
    ...
    \begin{align*}
    \Phi \definedas \prod \phi_i: \prod U_i \to \prod R^{n_i} \cong \RR^{\Sigma n_i}
    .\end{align*}

    - Claim: $\Phi$ is a homeomorphism onto its image.
      ...
      - It suffices to show that that the inverse image of a basic open set the target is open in the source.
      - Let $V\definedas \prod V_i \subseteq \RR^N$ be a basic open set, then
      \begin{align*}
