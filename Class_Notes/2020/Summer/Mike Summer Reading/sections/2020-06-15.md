...
: A continuous map $f:X\to Y$ is an *embedding* iff it is injective and $\tilde f:X\to f(X)$ is a homeomorphism.

Some facts from the appendix:

Subspaces $A\subseteq X$:

- Definition of the subspace topology: $U\subset A$ is open iff $U = V\intersect A$ for some open $V\subseteq X$.
