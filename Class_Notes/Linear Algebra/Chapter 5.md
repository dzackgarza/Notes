...
If, on the other hand, the $\vector b_i$ are orthonormal, then $B{-1} = B^T$, which is much easier to compute. We also obtain a rather simple formula for the coordinates of 