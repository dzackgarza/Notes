...
Geometrically, the solution is given by projecting $\vector b$ onto the column space of $A$. To see why this is the case, define the residual vector $\vector r = \vector b - A\tilde{\vector x}$. We then seek to minimize $\norm{\vector r}$, which happens exactly when $\vector r \perp \mathrm{colspace}(A)$. But this happens exactly when $\vector r \in \mathrm{colspace}(A)^\perp$, which by the fundamental subspaces theorem, is equivalent to $\vector r \in \ker A^T$.
