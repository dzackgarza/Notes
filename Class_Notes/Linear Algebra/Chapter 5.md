...
We notice that whatever $p$ is, it will in the direction of $\vector y$ and thus we will have $\vector p = \lambda \hat {\vector y}$ for some scalar $\lambda$. We can then form a "residual" vector $\vector r = \vector x - \vector p$
