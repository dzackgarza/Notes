...
\[
\mods{R} &\mapstofrom \Vect\Grp\Sch\above{R} \\
M &\mapsto V(M) \\
\Theta(R) &\mapsfrom R
.\]
...
A **vector bundle of rank $r$ over the affine scheme $\spec R$** is an $X\in \Sch\above{R}$ with a partition $1 = \sum_i f_i$ along with isomorphisms 
\[
...
\[
\Endo_{\rmod}(M) \mapsvia{\sim}  M\dual \tensor_R M \in \Alg\above{R} \intersect \rmod^{\loc\Free, r<\infty}
,\]
and so $V(\Endo_{\rmod}(M))$ makes sense.
Thus $V$ is a functor 
\[
V: \Assoc\Alg^{\unital} \to\Vect\Grp\Sch\above{R}
.\]
...
Conversely, by Zariski gluing, for any such cocycle $g_{ij}$ we can assign some $V_g\in \VectBun^r\above{R}$ with a trivializations satisfying $\phi_i \phi_j\inv = g_{ij}$.
...
:::{.remark}
This classifies $[V] \in \Bun\above{R}^r/\cong$ which are trivialized by $\mcu$. 
So there are induced maps
\[
f: \GL_r \to \GL_s \quad\leadsto\quad f_*: V_r\in \Bun^r\above{R} \to V_s \in \Bun^s\above{R}
\]
which extend to functors
\[
f_*: \Bun\above{R}^r \to \Bun\above{R}^s
.\]
...
It also induces an equivalence $\Alg\above{R} \to \Alg_{/k}$ with descent data.
...
The proof involves a construction called **twisting**: if $Y \in \Aff\Sch\above{R}$ with a left $G\dash$action, then the action map 
\[
