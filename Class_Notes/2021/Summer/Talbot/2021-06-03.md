...
:::{.proof title="in the case $\ell=1$"}
Consider $S^0[a_2] = S^0 \oplus S^2 \oplus S^4 \oplus \cdots = \Free_{E_1}(S^4)$.
This turns out to be equal to $\Sigma_+^\infty \OmegaS^3$ using ??? (see comment).
Now use $S^4 = \Omega\HP^\infty$, so this is $\Sigma_+^\infty \Omega^2 \HP^\infty$, which has an $E_2$ ring structure.
