...
In fact $e>1$ only happens for $5$ since this corresponds (for all applicable primes, which is all primes since $\ZZ_L$ is monogenic here) to the polynomial having a repeated factor mod $p$.
