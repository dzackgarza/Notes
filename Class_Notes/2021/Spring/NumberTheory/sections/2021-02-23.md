...
Recalling that \( \min_\tau(x) = f_q(x) = x^2 - x + q \), if this has one root then it has two which sum to $-b = -(-1) = 1$, where the other root satisfies 
\[
1 \leq x \leq {1\over 2} \sqrt{ \abs{d} \over 3 } + {1\over 2}
.\]
Why?
If the other root $x = r$ with $1<r<p$ doesn't satisfy this, 
