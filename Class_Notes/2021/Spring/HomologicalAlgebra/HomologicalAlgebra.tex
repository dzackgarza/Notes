...
\begin{itemize}
\item
  \(\operatorname{Tot}^{\oplus}(C) = \operatorname{Tot}^{\prod}(C)\)
  when \(C\) is bounded.
\item
...
\({\operatorname{Hom}}_{R}(M, {\,\cdot\,}): {\mathsf{Mod}{\hbox{-}}R}\to {\mathsf{Ab}}\)
...
\({\mathsf{Mod}{\hbox{-}}R}\) has enough projectives: for all
\(A \in {\mathsf{Mod}{\hbox{-}}R}\), one can take
...
Let \(M\in {\mathsf{Mod}{\hbox{-}}R}\), then a \textbf{projective
...
Every object \(M\in {\mathsf{Mod}{\hbox{-}}R}\) has a projective
...
\({\mathsf{Ab}}= {{{\mathbb{Z}}}{\hbox{-}}\mathsf{Mod}}\) has enough
injectives.
...
As a consequence, \({\mathsf{Mod}{\hbox{-}}R}\) has enough injectives
for \emph{any} ring \(R\).
...
\({\operatorname{Hom}}_R({\,\cdot\,}, A): {\mathsf{Mod}{\hbox{-}}R}\to {\mathsf{Ab}}\)
...
\({\operatorname{Hom}}_{{\mathsf{Ab}}}(N, A) \in {\mathsf{Mod}{\hbox{-}}R}\)
...
If \(M \in {\mathsf{Mod}{\hbox{-}}R}\), then the following natural map
...
\({\operatorname{Hom}}_{{\mathsf{Ab}}}(R, I) \in {\mathsf{Mod}{\hbox{-}}R}\)
is injective.
...
Show that \({\mathsf{Mod}{\hbox{-}}R}\) has enough injectives, using
...
Fix a right \(R{\hbox{-}}\)module \(M \in {\mathsf{Mod}{\hbox{-}}R}\),
then
\(F \coloneqq{\operatorname{Hom}}_{{\mathsf{Mod}{\hbox{-}}R}}(M, {\,\cdot\,}): {\mathsf{Mod}{\hbox{-}}R}\to {\mathsf{Ab}}\)
is a left-exact functor. Its right-derived functors are \textbf{ext
functors} and denoted
\(\operatorname{Ext}_{{\mathsf{Mod}{\hbox{-}}R}}^i(M, {\,\cdot\,})\).

\end{definition}

\begin{example}[?]

\begin{align*} \operatorname{Ext}_{{\mathsf{Mod}{\hbox{-}}R}}^i(M, A) = (R^i F)(A) = [ R^i {\operatorname{Hom}}_{{\mathsf{Mod}{\hbox{-}}R}}(M, {\,\cdot\,}) ] (A) .\end{align*}
...
\({\mathsf{Mod}{\hbox{-}}R}\). These upgrade the characterization
...
\(G\coloneqq{\operatorname{Hom}}_{{\mathsf{Mod}{\hbox{-}}R}}({\,\cdot\,}, B): {\mathsf{Mod}{\hbox{-}}R}\to {\mathsf{Ab}}\).
Then \(G\) is still left-exact, but is now \emph{contravariant}. We can
regard it as a covariant functor left-exact functor
\(G: {\mathsf{Mod}{\hbox{-}}R}^{\operatorname{op}}\to {\mathsf{Ab}}\).
...
Then \({\operatorname{Hom}}_{S}(B, C)\in {\mathsf{Mod}{\hbox{-}}R}\) in
...
\(A \in {\mathsf{Mod}{\hbox{-}}R}\) and
\(C\in {{{\hbox{-}}}{\hbox{-}}\mathsf{Mod}}S\) there is a natural
isomorphism
\begin{align*}
...
We looked at \(B\in ({R}, {S}){\hbox{-}}\mathsf{biMod}\) and showed
\({\,\cdot\,}\otimes_R B: {{R}{\hbox{-}}\mathsf{Mod}} \to {{S}{\hbox{-}}\mathsf{Mod}}\)
...
\({\mathsf{Mod}{\hbox{-}}R}\) satisfies (1), since direct sums of
\(R{\hbox{-}}\)modules still have an \(R{\hbox{-}}\)module structure.
Thus \({\mathsf{Mod}{\hbox{-}}R}\) is cocomplete.
...
For
\(A_i \in {\mathsf{R}{\hbox{-}}\mathsf{Mod}}, B\in {\mathsf{Mod}{\hbox{-}}R}\),
\begin{align*}
...
\((P)_R \in \mathsf{Ch}( {\mathsf{Mod}{\hbox{-}}R}), {}_R(Q) \in \mathsf{Ch}({\mathsf{R}{\hbox{-}}\mathsf{Mod}})\).
...
\(P \xrightarrow{\varepsilon} A \in {\mathsf{Mod}{\hbox{-}}R}\) and
\(Q \xrightarrow{\eta} B \in {\mathsf{R}{\hbox{-}}\mathsf{Mod}}\). We'll
form 3 tensor product double complexes.
...
Let \(A, B\in {\mathsf{Mod}{\hbox{-}}R}\), then an \textbf{extension of
\(A\) by \(B\)} is a SES
\begin{align*}
...
Given \(A, B\in {\mathsf{Mod}{\hbox{-}}R}\) (or an abelian category with
...
Note that if \({P}^{{\,\cdot\,}} \to A\to 0\) is a projective
resolution, then the comparison theorem yields maps and a commutative
diagram
...
If \({P}^{{\,\cdot\,}} \in \mathsf{Ch}({\mathsf{Mod}{\hbox{-}}R})\) is a
complex of of right \(R{\hbox{-}}\)modules and
\(M \in {\mathsf{R}{\hbox{-}}\mathsf{Mod}}\) is a left
\(R{\hbox{-}}\)module, how is the homology of \({P}^{{\,\cdot\,}}\) and
that of \({P}^{{\,\cdot\,}} \otimes_R M\) related?
...
Let \({P}^{{\,\cdot\,}}\in \mathsf{Ch}({\mathsf{Mod}{\hbox{-}}R})\) be a
...
By observation 1, the first term equals
\(H_n({P}^{{\,\cdot\,}})\otimes M\). From this, we get a flat resolution
of \(H_{n-1}(P)\):
...
Let \({P}^{{\,\cdot\,}}\) be a chain complex of free abelian groups. For
...
complex \(H_n(X; R) \coloneqq H_n({S}^{{\,\cdot\,}}(X) )\).
...
\(H_n(X; M) \coloneqq H_n( {S}^{{\,\cdot\,}}(X) \otimes_{\mathbb{Z}}M)\),
...
Let \({P}^{{\,\cdot\,}}\) be a chain complex of projective
...
\(A\in {G}{\hbox{-}}\mathsf{Mod}\) be a left \(G{\hbox{-}}\)module,
i.e.~an abelian group on which \(G\) acts by additive maps on the left,
written \(g.a\) or \(ga\) for \(g\in G, a\in A\). Here \emph{additive}
means that \(g.(a_1 + a_2) = g.a_1 + g.a_2\). Note that this implies
...
If \(B \in {G}{\hbox{-}}\mathsf{Mod}\) is another left
\(G{\hbox{-}}\)module, then
\begin{align*}
...
\({{G}{\hbox{-}}\mathsf{Mod}} = {{\mathbb{Z}}G}{\hbox{-}}\mathsf{Mod}\).
...
An object \(A\in {G}{\hbox{-}}\mathsf{Mod}\) is a \textbf{trivial}
module if and only if \(g.a = a\) for all \(g\in G\).
...
\(\operatorname{Triv}:{\mathsf{Ab}}\to {G}{\hbox{-}}\mathsf{Mod}\).
...
Let \(A\in {G}{\hbox{-}}\mathsf{Mod}\) and \({\mathbb{Z}}\) be the
trivial \(G{\hbox{-}}\)module. Then
...
Viewing
\({\mathbb{Z}}= _{{\mathbb{Z}}} {\mathbb{Z}}_{{\mathbb{Z}}G} \in ({{\mathbb{Z}}}, {{\mathbb{Z}}G}){\hbox{-}}\mathsf{biMod}\)
