...
{\tiny \textit{Last updated:} 2021-04-08 }
\end{flushleft}
...
\(\Coind\) is right adjoint to it, so these will have derived functors.
A special case is when \(H = \left\{{ 1 }\right\}\) is the trivial
group, in which case any \(H{\hbox{-}}\)module \(B\) is an abelian group
such that \(B^H = B = B_H\). So \(({\,\cdot\,})^H, ({\,\cdot\,})_H\) are
...
a set of coset representatives for \(\quotleft{G}{H}\) and hence a left
