...
{\tiny \textit{Last updated:} 2021-03-03 }
\end{flushleft}
...
\begin{align*} \operatorname{Ext}_{{\mathbf{Mod}{\hbox{-}}R}}^i(M, A) = (R^i F)(A) = [ R^i {\operatorname{Hom}}_{{\mathbf{Mod}{\hbox{-}}R}}(M, {\,\cdot\,}) ] (A) .\end{align*}
...
Then \({\operatorname{Hom}}_{S}(B, C)\in {\mathbf{Mod}{\hbox{-}}R}\) in
a natural way: given \(f:B\to C\), define \((f\cdot r)(b) = f(rb)\).
...
We looked at \(B\in ({R}, {S}){\hbox{-}}\mathbf{biMod}\) and showed
\({\,\cdot\,}\otimes_R B: {{R}{\hbox{-}}\mathbf{Mod}} \to {{S}{\hbox{-}}\mathbf{Mod}}\)
...
\hypertarget{colimits-and-adjoint}{%
\subsection{Colimits and Adjoint}\label{colimits-and-adjoint}}
...
\({\mathbf{Mod}{\hbox{-}}R}\) satisfies (1), since direct sums of
\(R{\hbox{-}}\)modules still have an \(R{\hbox{-}}\)module structure.
Thus \({\mathbf{Mod}{\hbox{-}}R}\) is cocomplete.
...
For
\(A_i \in {R{\hbox{-}}\mathbf{Mod}}, B\in {\mathbf{Mod}{\hbox{-}}R}\),
\begin{align*}
...
\((P)_R \in \operatorname{Ch}( {\mathbf{Mod}{\hbox{-}}R}), {}_R(Q) \in \operatorname{Ch}({R{\hbox{-}}\mathbf{Mod}})\).
...
\(P \xrightarrow{\varepsilon} A \in {\mathbf{Mod}{\hbox{-}}R}\) and
\(Q \xrightarrow{\eta} B \in {R{\hbox{-}}\mathbf{Mod}}\). We'll form 3
tensor product double complexes.
...
\item
  ...
  \(A \hookrightarrow\operatorname{Ch}( \mathcal{A} )\) as a complex
...
Consider \(\mathrm{\operatorname{Tot}}(C)[-1]\), which in degree \(n\)
is \((\mathrm{\operatorname{Tot}}(C))_{n-1}\) and since this was an odd
shift, negates all of the signs of differential. So in degree \(n\),
this explicitly looks like
\begin{align*}
...
is exact, i.e.~\(\mathrm{\operatorname{Tot}}(C)\) is acyclic. This
follow from the next result we'll prove, the \emph{acyclic assembly
lemma}. Note that if \(Q_j\) is projective, then it's an algebra fact
that \({\,\cdot\,}\otimes_R Q_j\) is exact (not just right exact) since
...
lemma, \(\mathrm{\operatorname{Tot}}^{\oplus}(C)\) will be acyclic. A
similar argument will go through to show that \(\one\otimes\eta\) is
also a quasi-isomorphism by adjoining \((P\otimes B)\) as the \(-1\)st
row and applying a version of the lemma for right half-plane complexes
with exact columns.
...
\(\mathrm{\operatorname{Tot}}^{\oplus }(\tau_n C)= \mathrm{\operatorname{Tot}}^{\prod}(\tau_n C)\)
...
\(\mathrm{\operatorname{Tot}}^{\oplus }C\) is acyclic since every cycle
in \(\mathrm{\operatorname{Tot}}^{\oplus }(C)\) is nonzero in only
finitely many terms. Thus each such cycle is a cycle in
\(\mathrm{\operatorname{Tot}}(\tau_n C)\) for some \(n\ll 0\), and hence
a boundary by the previous argument.
...
\(H_0 \mathrm{\operatorname{Tot}}^{\prod }C = 0\). We can write
\begin{align*}
...
\(b_{-j, j+1} \in C_{-j, j+1} \subseteq (\mathrm{\operatorname{Tot}}^{\prod} C)_1\)
such that
\begin{align*}
...
half-plane with exact columns
\(\implies \mathrm{\operatorname{Tot}}^{\prod}(C)\) is acyclic. It's
enough to check in degree 0 by shifting. Fix a 0-cycle
...
Let \(A, B\in {\mathbf{Mod}{\hbox{-}}R}\), then an \textbf{extension of
\(A\) by \(B\)} is a SES
\begin{align*}
...
Given \(A, B\in {\mathbf{Mod}{\hbox{-}}R}\) (or an abelian category with
enough projectives and injectives), there is a correspondence
\begin{align*}
\left\{{\substack{
  0 \to B \to X\to A \to 0
}}\right\}_{/\sim}
...
using that
\({{\partial}}\in \operatorname{Mor}({R{\hbox{-}}\mathbf{Mod}})\). But
this is equal to \(\Theta(\xi') + \Theta(\xi'')\), which is what we
wanted to show.
...
Note that if \(\mathbf{P} \to A\to 0\) is a projective resolution, then
the comparison theorem yields maps and a commutative diagram
...
If \(\mathbf{P} \in \operatorname{Ch}({\mathbf{Mod}{\hbox{-}}R})\) is a
complex of of right \(R{\hbox{-}}\)modules and
\(M \in {R{\hbox{-}}\mathbf{Mod}}\) is a left \(R{\hbox{-}}\)module, how
is the homology of \(\mathbf{P}\) and that of \(\mathbf{P} \otimes_R M\)
related?
...
A maps \(A \xrightarrow{f} B\) in \({R{\hbox{-}}\mathbf{Mod}}\) is
injective if and only if \(f(a) = 0_B \implies a = 0_A\). Monomorphisms
are injective maps in \({R{\hbox{-}}\mathbf{Mod}}\).
...
\begin{itemize}
\item
  In \({\mathbf{Ab}}\), direct colimits commute with finite limits.
