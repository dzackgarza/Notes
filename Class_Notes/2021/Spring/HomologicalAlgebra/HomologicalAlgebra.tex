...
{\tiny \textit{Last updated:} 2021-04-05 }
\end{flushleft}
...
\({\operatorname{Hom}}_R(N, {\,\cdot\,}): {\mathsf{R}{\hbox{-}}\mathsf{Mod}}\to {\mathsf{Ab}}\),
...
\({\operatorname{Hom}}_{\mathsf{Ch}}(C, D)\) becomes an abelian group.
...
\({\operatorname{Hom}}_{R}(M, {\,\cdot\,}): {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\to {\mathsf{Ab}}\)
...
\hypertarget{tuesday-february-02-1}{%
\section{Tuesday, February 02}\label{tuesday-february-02-1}}

\hypertarget{tuesday-february-02-2}{%
\section{Tuesday, February 02}\label{tuesday-february-02-2}}
...
\({\operatorname{Hom}}_R({\,\cdot\,}, A): {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\to {\mathsf{Ab}}\)
...
\({\operatorname{Hom}}_{{\mathsf{Ab}}}(N, A) \in {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\)
...
\({\operatorname{Hom}}_{{\mathsf{Ab}}}(R, {\,\cdot\,}): {\mathsf{Ab}}\to {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\)
...
\({\operatorname{Hom}}_{{\mathsf{Ab}}}(R, I) \in {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\)
is injective.
...
\({\operatorname{Hom}}_{{\mathsf{Ab}}}(R, {\,\cdot\,})\) is right
...
\(F \coloneqq{\operatorname{Hom}}_{{\mathsf{Mod}{\hbox{-}}\mathsf{R}}}(M, {\,\cdot\,}): {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\to {\mathsf{Ab}}\)
is a left-exact functor. Its right-derived functors are \textbf{ext
functors} and denoted
\(\operatorname{Ext}_{{\mathsf{Mod}{\hbox{-}}\mathsf{R}}}^i(M, {\,\cdot\,})\).

\end{definition}

\begin{example}[?]

\begin{align*} \operatorname{Ext}_{{\mathsf{Mod}{\hbox{-}}\mathsf{R}}}^i(M, A) = (R^i F)(A) = [ R^i {\operatorname{Hom}}_{{\mathsf{Mod}{\hbox{-}}\mathsf{R}}}(M, {\,\cdot\,}) ] (A) .\end{align*}
...
characterization involving \({\operatorname{Hom}}\) to one involving
\(\operatorname{Ext}\). \footnote{Note the typo in 2.5.1.3, it should
  say the following: ``\(B\) is
  \({\operatorname{Hom}}_{R}(A, {\,\cdot\,})\) is acyclic for all
  \(A\).''}
...
\(G\coloneqq{\operatorname{Hom}}_{{\mathsf{Mod}{\hbox{-}}\mathsf{R}}}({\,\cdot\,}, B): {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\to {\mathsf{Ab}}\).
...
Then
\({\operatorname{Hom}}_{S}(B, C)\in {\mathsf{Mod}{\hbox{-}}\mathsf{R}}\)
...
and showed
\({\,\cdot\,}\otimes_R B: {\mathsf{R}{\hbox{-}}\mathsf{Mod}} \to {\mathsf{S}{\hbox{-}}\mathsf{Mod}}\)
is left adjoint to hom, and has left-derived functors
\(\operatorname{Tor}_n^R({\,\cdot\,}, B) \coloneqq L_n({\,\cdot\,}\otimes_R B)\).
\begin{align*}
...
\item
  \({\left\lvert { {\operatorname{Hom}}_{\mathcal{I}}(i, j) } \right\rvert} \leq 1\),
  and \(i\to j \iff i\leq j\)
\end{itemize}

Note that if \(i\not\leq j\), then
\({\operatorname{Hom}}_{\mathcal{I}}(i, j) = \emptyset\).
...
i.e.~\({\operatorname{Hom}}(i, j) = \emptyset\) unless \(i=j\), in which
case \({\operatorname{Hom}}(i, i) = \left\{{ \one_i }\right\}\).
...
of \({\,\cdot\,}\otimes_R {\,\cdot\,}\) give the same results, and
similarly for the two right-derived functors
\({\operatorname{Hom}}_R({\,\cdot\,}, {\,\cdot\,})\). We'll use double
complexes!
...
\({\,\cdot\,}\otimes_R Q_j\) is exact (not just right exact) since
projective implies flat. This implies that the rows of \(C\) are exact,
since this is taking a project resolution (which is exact) and tensoring
with a flat module. Using that \(C\) is supported on the upper
half-plane and has exact rows, by this part (3) of the acyclic assembly
lemma, \(\operatorname{Tot}^{\oplus}(C)\) will be acyclic. A similar
argument will go through to show that \(\one\otimes\eta\) is also a
...
This proves that \({\,\cdot\,}\otimes_R{\,\cdot\,}\) is balanced,
i.e.~taking the derived functors in either variable with the same pair
\((A, B)\) results in the same thing. To prove a similar result for hom
and ext, we want to consider \({\operatorname{Hom}}_R(A, {\,\cdot\,})\)
which requires injective resolutions, and
\({\operatorname{Hom}}_R({\,\cdot\,}, B)\) is contravariant and
left-exact, so we take an injective resolution in
...
\(C_{i, j} \coloneqq{\operatorname{Hom}}(P_i, I^j)\) for \(i, j\geq 0\).
...
\(\beta\in {\operatorname{Hom}}(M, B)\) with \({{\partial}}\beta = x\)
...
\(\beta'\in {\operatorname{Hom}}(M, B)\) is another lift of \(x\). Note
that although \(\operatorname{Ext}^1(P, B) =0\), the fact that
\(\ker {{\partial}}= {\operatorname{Hom}}(M, B) \neq 0\), there are many
such choices of lifts. Using exactness of diagram \((*)\), there exists
an \(f\in {\operatorname{Hom}}(P, B)\) such that
...
Note that if \({P}^{\bullet} \to A\to 0\) is a projective resolution,
then the comparison theorem yields maps and a commutative diagram
...
free, hence projective, hence flat, and so \(A\otimes_F {\,\cdot\,}\) is
exact.
...
If \({P}^{\bullet} \in \mathsf{Ch}({\mathsf{Mod}{\hbox{-}}\mathsf{R}})\)
is a complex of of right \(R{\hbox{-}}\)modules and
\(M \in {\mathsf{R}{\hbox{-}}\mathsf{Mod}}\) is a left
\(R{\hbox{-}}\)module, how is the homology of \({P}^{\bullet}\) and that
of \({P}^{\bullet} \otimes_R M\) related?
...
Let \({P}^{\bullet}\in \mathsf{Ch}({\mathsf{Mod}{\hbox{-}}\mathsf{R}})\)
be a chain complex of flat right \(R{\hbox{-}}\)modules such that each
...
also flat. Taking the LES from applying \({\,\cdot\,}\otimes_R M\),
noting that \(M\) is arbitrary yields
...
that one could also apply \(\operatorname{Tor}(dP_n, {\,\cdot\,})\) to
get a similar LES. So this lifts to a SES of complexes
\begin{align*}
...
By observation 1, the first term equals \(H_n({P}^{\bullet})\otimes M\).
...
Let \({P}^{\bullet}\) be a chain complex of free abelian groups. For
every abelian groups \(M\) and every \(n\), the Kunneth sequence splits
non-canonically as
\begin{align*}
...
\({\operatorname{Hom}}_{\mathsf{Top}}( \Delta_k, X)\) where \(\Delta_k\)
is the standard simplex By ordering the vertices, this induces an
ordering on the faces by taking lexicographic ordering. Then the
restriction of a map \(\Delta_k \to X\) to the \(i\)th face of
...
complex \(H_n(X; R) \coloneqq H_n({S}^{\bullet}(X) )\).
...
\(H_n(X; M) \coloneqq H_n( {S}^{\bullet}(X) \otimes_{\mathbb{Z}}M)\),
the homology with \textbf{coefficients} in \(M\). If no coefficients are
...
\({\operatorname{Hom}}_{\mathsf{Top}}( \Delta_k, X)\), which assemble
into a chain complex \(S(X)\). For \(M\in {\mathsf{Ab}}\), we defined
\(H^n(X; M) \coloneqq H^n( {\operatorname{Hom}}(S(X), M))\) and write
...
Let \({P}^{\bullet}\) be a chain complex of projective
\(R{\hbox{-}}\)modules. Assume \(dP_n\) is also projective for all
\(n\). For \(M\in {\mathsf{R}{\hbox{-}}\mathsf{Mod}}\), there is a split
SES
\begin{align*}
...
Writing
\(\mathop{\mathrm{End}}_R(A) \coloneqq{\operatorname{Hom}}_R(A, A)\), we
...
\({\operatorname{Hom}}_{\mathcal{G}}(1, 1) = G\). Note that every
...
\item
  ...
  \(({\,\cdot\,})^G\) is right-adjoint to \(\operatorname{Triv}\). These
  should both be easy checks! So this is left-exact and has
  right-derived functors (similar to ext).
\item
  \(A_G\) is the largest \(G{\hbox{-}}\)trivial quotient of \(A\), and
  \(({\,\cdot\,})_G\) is left-adjoint to \(\operatorname{Triv}\). Thus
  it is right-exact and has left-derived functors (similar to tor).
\end{enumerate}
...
\item
  \(A^G \cong {\operatorname{Hom}}_G({\mathbb{Z}}, A)\)
  (\textbf{important!!})
\end{enumerate}
...
\end{align*}
where \({\operatorname{Hom}}_{\mathbb{Z}}({\mathbb{Z}}, A)\) has an
action \((g.f)(x) \coloneqq f(x. g)\) for \(x\in {\mathbb{Z}}g\in G\).
Since \(x.g = x\) for all \(x, g\), we have \(g.f = f\) and thus
\({\operatorname{Hom}}_{\mathbb{Z}}({\mathbb{Z}}, A)\) is a trivial
...
.\end{align*}
Thus
\({\operatorname{Hom}}_{\mathbb{Z}}({\mathbb{Z}}, {\,\cdot\,}) \cong \operatorname{Triv}({\,\cdot\,})\).
By prop 2.6.3, the functor
\({\mathbb{Z}}\otimes_{{\mathbb{Z}}G} ({\,\cdot\,})\) is left-adjoint to
\({\operatorname{Hom}}_{\mathbb{Z}}( _{{\mathbb{Z}}} {\mathbb{Z}}_{{\mathbb{Z}}G}, {\,\cdot\,})\).
Now applying exercise 6.1.1 part 2,
\(({\,\cdot\,})_G \cong \operatorname{Triv}({\,\cdot\,})\). Since
left-derived functors are universal \(\delta{\hbox{-}}\)functors, we
have a natural isomorphism
\(({\,\cdot\,})_G \cong {\mathbb{Z}}\otimes_{{\mathbb{Z}}G} ({\,\cdot\,})\)
...
Taking \(f(1)\), we have
\(A^G \cong {\operatorname{Hom}}_{\mathbb{Z}}({\mathbb{Z}}, A^G)\).
Using the adjoint property from exercise 6.1.1 part 1, this is
isomorphic to
\({\operatorname{Hom}}_G( \operatorname{Triv}({\mathbb{Z}}), A)\). Thus
\(({\,\cdot\,})^G \cong {\operatorname{Hom}}_G({\mathbb{Z}}, {\,\cdot\,})\).
...
\item
  ...
  \(G{\hbox{-}}\)coinvariants.
\item
  \(A^G \cong {\operatorname{Hom}}_{{\mathbb{Z}}G}( {\mathbb{Z}}, A)\).
...
\def\labelenumi{\arabic{enumi}.}
\item
  \(H_*(G; A) \coloneqq L_*({\,\cdot\,}))G (A)\) are the
  \textbf{homology groups of \(G\) with coefficients in \(A\)}. It is
  isomorphic to
  \(\operatorname{Tor}_*^{{\mathbb{Z}}G}({\mathbb{Z}}, A)\) by (1) in
  the lemma above. In particular, \(H_0(G; A) \cong A_G\).
\item
  \(H^*(G; A) \coloneqq R^*({\,\cdot\,})^G(A)\) is the
  \textbf{cohomology of \(G\) with coefficients in \(A\)}. It is
  isomorphic to
...
\(({\,\cdot\,})^G, ({\,\cdot\,})_G: \mathsf{G}{\hbox{-}}\mathsf{Mod} \to {\mathsf{Ab}}\)
...
\({\,\cdot\,}\otimes_{{\mathbb{Z}}T} A\) to this resolution to obtain
\begin{align*}
0\to FP_1 \to FP_0 \to 0 
&\coloneqq
...
\({\operatorname{Hom}}_{{\mathbb{Z}}T}({\,\cdot\,}, A)\) to obtain
\begin{align*}
0
...
\tightlist
\item
  \(\operatorname{coker}( {\,\cdot\,}\circ (t-1)) = A_T = H^1(T; A)\)
  (surprising!) and
\item
...
\(\mkern 1.5mu\overline{\mkern-1.5mu{\,\cdot\,}\mkern-1.5mu}\mkern 1.5mu\)
...
\(H_*( {T}^{\bullet} )\). We've used both differentials, so how do we
...
\begin{theorem}[Construction of a Spectral Sequence]
...
either by a projective resolution \({P}^{\bullet}\twoheadrightarrow A\)
or a projective resolution \({Q}^{\bullet}\twoheadrightarrow B\) was a
disguised spectral sequence argument. So we'll go recover it using the
actual spectral sequence.
...
\end{cases}
,\end{align*}
which is \(L_p(A\otimes{\,\cdot\,})(B)\) in \(q=0\). Since
\({}^{II}E_{p, q}^2 \Rightarrow H_{p+q}(\operatorname{Tot}(P\otimes Q)) = L_p({\,\cdot\,}\otimes B)(A)\),
and we thus have
\begin{align*}
...
\item
  The induces maps on boundaries and in homology are projective
  resolutions in \(\mathcal{A}\):
  \begin{align*}
...
\begin{quote}
\href{https://q.uiver.app/?q=}{Link
to Diagram}
\end{quote}
...
\item
  If \(f, g: A\to B\) are chain homotopic, then \(\tilde f, \tilde f\)
...
The invariant functor \(({\,\cdot\,})_H\) has a left adjoint and the
coinvariant functor \(({\,\cdot\,})^H\) has a right adjoint.
...
and \(({\,\cdot\,})^H\) (respectively \(({\,\cdot\,})_H\)) are right
(respectively left) adjoin to
\begin{align*}
...
By the lemma, \(({\,\cdot\,})^H\) is right adjoint to \(\rho^{\#}\),
which is exact. By prop 2.3.10, it sends injectives to injectives, and
injectives are \(F{\hbox{-}}\)acyclic for
\(F({\,\cdot\,}) = ({\,\cdot\,})^{G \over H}\). So this is a valid setup
for the Grothendieck spectral sequence.
...
compute \(H^*(H; {\,\cdot\,})\) and \(H_*(H; {\,\cdot\,})\) as well.
...
When \(A \in {\mathsf{H}{\hbox{-}}\mathsf{Mod}}^{\operatorname{Triv}}\),
...
\item
  ...
  \item
    ...
    \(f_*: {\operatorname{Hom}}(A, X) \to {\operatorname{Hom}}(A, Y)\)
    given by \(f_*(h) = f\circ h\).
  \item
    Has right-derived functors
    \(\operatorname{Ext}^i_R(A, B) \coloneqq R^i {\operatorname{Hom}}_R(A, {\,\cdot\,})(B)\)
...
\item
  ...
  \item
    Is a functor that sends \(f:X\to Y\) to
    \(f^*: {\operatorname{Hom}}(Y, B) \to {\operatorname{Hom}}(X, B)\)
    given by \(f^*(h) = h\circ f\).
  \item
    Has left-derived functors
    \(\operatorname{Ext}^i_R(A, B) \coloneqq L_i {\operatorname{Hom}}_R({\,\cdot\,}, B)(A)\)
...
\item
  ...
  \({\operatorname{Hom}}_R(M, N) \in (\mathsf{S}, \mathsf{S'}){\hbox{-}}\mathsf{biMod}\).

  \begin{itemize}
  \tightlist
  \item
    Mnemonic: the slots of \({\operatorname{Hom}}_R\) use up a left
...
Write
\(F({\,\cdot\,}) \coloneqq{\operatorname{Hom}}_R(A, {\,\cdot\,})\). This
is left-exact and thus has right-derived functors
...
\item
  ...
  .\end{align*}
\item
  Apply \(F({\,\cdot\,})\) to get a new (and usually \textbf{not exact})
  complex
  \begin{align*}
...
\item
  Use the fact that \(F({\,\cdot\,})\) is left exact and apply it to the
...
\(\phi: {\operatorname{Hom}}_{{\mathbb{Z}}}({\mathbb{Z}}, {\mathbb{Z}}/n) \xrightarrow{\sim} {\mathbb{Z}}/n\),
...
\item
  ...
  \item
    ...
    and apply
    \({\operatorname{Hom}}_{\mathbb{Z}}({\,\cdot\,}, {\mathbb{Z}})\).
  \end{itemize}
\item
