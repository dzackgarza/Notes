...
\(\operatorname{ad}_x \in \endo_k({\mathfrak{g}})\) where
\(\operatorname{ad}_x(y) \coloneqq[x, y]\). It has a well-defined (and
basis-independent) trace, so define the \textbf{Killing
...
Let \({\mathfrak{g}}\in \liegalg(\ss)\) with
