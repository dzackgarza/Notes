...
{\tiny \textit{Last updated:} 2021-03-15 }
\end{flushleft}
...
If
\({P}^{{\,\cdot\,}} \in \mathsf{Ch}({\mathsf{Mod}{\hbox{-}}\mathsf{R}})\)
...
:::\{.lemma\} Last time: started setting up group homology. For \(G\) a
group and \(A\in \mathsf{G}{\hbox{-}}\mathsf{Mod}\), we think of
\({\mathbb{Z}}\) as a trivial \(G{\hbox{-}}\)module and
...
\(H_*( {T}^{{\,\cdot\,}} )\). We've used both differentials, so how do
we continue? There are well-defined maps
