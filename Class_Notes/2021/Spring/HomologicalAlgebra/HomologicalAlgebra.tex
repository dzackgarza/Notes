...
\(\mathcal{Cx}({R{\hbox{-}}\operatorname{mod}})\). In this setting,
...
{[} u\_n: Z\_n(C\_{\,\cdot\,}) \toZ\_n(D\_{\,\cdot\,}) \textbackslash{}
u\_n: B\_n(D\_{\,\cdot\,}) \toB\_n(D\_{\,\cdot\,})
...
\(H_n(C) \coloneqq Z_n(C) / B_n(C)\). Moreover, taking
...
\tightlist
\item
  If \(\left\{{A_ \alpha}\right\}\) is a family of complexes, we can
  form two new complexes:
