...
The claim is that $\tilde s: P \to Q[1, 0]$ forms the horizontal component of a nullhomotopy $\mathbf{s}: P\to Q[1, 1]$, which follows as long as 
\[
\tilde f^p = \tilde s_{p-1} \bd_P^h + \bd_Q^h \tilde s_{p}
\]
as a map of complexes $P_{p, *} \to Q_{p, *}$, where the $\bd^h$ are the horizontal maps in the respective CE resolutions.

\todo[inline]{Not sure how to show this.}


It now remains to construct the vertical component $s^v: P \to Q[0, 1]$.

\todo[inline]{Note sure where these maps should come from.}
