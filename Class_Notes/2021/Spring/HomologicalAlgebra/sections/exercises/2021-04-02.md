...
The claim is that $\tilde s$ forms the horizontal component of a nullhomotopy $\mathbf{s}: P\to Q$, which follows as long as $\tilde f^p = \tilde s_{p-1} \bd_P^h + \bd_Q^h \tilde s_{p}$ as a map of complexes $P_{p, *} \to Q_{p, *}$.
