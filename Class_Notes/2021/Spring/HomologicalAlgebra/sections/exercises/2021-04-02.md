...
:::{.claim}
The claim is that $\tilde s: P \to Q[1, 0]$ forms the horizontal component of a nullhomotopy $\mathbf{s}: P\to Q[1, 1]$, which will follow if we can construct a vertical homotopy $s^v$ such that 
\[
