...
By exactness, $\ker d_n^D = \im d_{n+1}^D$ and we can thus pull $\iota$ back to some $f \in D_{n+1} \da \Hom_R(Z_n, C_{n+1})$, and since our original $y \in \ker d_n^C \da Z_n$, it makes sense to consider $x \da f(y) \in C_{n+1}$.
