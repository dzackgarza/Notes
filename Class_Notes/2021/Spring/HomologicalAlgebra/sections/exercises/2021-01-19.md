...
By assumption, the following complex is exact at $n$ since its homology vanishes:
\[
...
This can be seen by writing $d_n^D(\ker d_n^C \mapsvia{\iota} C_n) = (\ker d_n^C \mapsvia{\iota} C_n \mapsvia{d_n^C} C_{n-1})$.
...
By exactness, $\ker d_n^D = \im d_{n+1}^D$ and we can thus pull $\iota$ back to some $f \in D_{n+1} \da \Hom_R(Z_n, C_{n+1})$, and since our original $y \in \ker d_n^C \da Z_n$, it makes sense to consider $f(y) \in C_{n+1}$.
Importantly, this $f$ satisfies $f = d_{n+1}^D(f) \da d_{n+1}^C \circ f$
