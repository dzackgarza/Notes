...
\right\},
  ...
  &= (g+_D f)(x)
  ,\]
  and so $+_C$ is a commutative addition.
  The additive inverse of $f$ is $-f$, there is an identity function $\id_{C_n}(x) \da x$, and the sum of two functions $A\to C_n$ is again a function $A\to C_n$.
