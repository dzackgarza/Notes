...
It only remains to check that the $u_n$ assemble to a chain map $K\to \ker f$, which follows from the commutativity of squares 1 and 2.
That these are commutative follows from the universal property of kernels in $\rmod$, since they are precisely the following squares:
