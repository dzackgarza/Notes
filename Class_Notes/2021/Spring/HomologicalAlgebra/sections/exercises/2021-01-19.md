...
:::{.solution}
We can first identify the following and consider their free ranks:

- $H_0(C) = \coker(A) \cong R^V/\im A \implies \rk H_0(C) = V - \rk \im A$, and

- $H_1(C) = \ker(A) \implies \rk H_1 (C) = \rk \ker A$
...
Given this claim, we can consider the SES of free \(R\dash\)modules
\[
0 \injects \ker(A) \to R^E \to \im(A) \to 0
,\]
and since $\im(A)$ is free and thus projective, this sequence splits.
So $R^E \cong \ker(A) \oplus \im(A)$
