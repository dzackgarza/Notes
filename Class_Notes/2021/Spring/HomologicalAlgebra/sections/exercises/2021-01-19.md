...
:::{.solution}
For a fixed map $f:A\to B$, the *kernel* of $f$ is an object $\ker f$ satisfying the following universal property: for any object $K$ with a morphism $K \mapsvia{g} A$ with
