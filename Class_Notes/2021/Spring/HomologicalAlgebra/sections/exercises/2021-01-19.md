...
It only remains to check that the $u_n$ assemble to a chain map $K\to \ker f$, which follows from the commutativity of e.g. square (1).
That (1) is commutative follows from the fact that the larger rectangle formed by (1) and (3) together commutes (since $g^K$ is a chain map) and (3) commutes since $\iota^f: \ker f \to A$ was shown to be a chain map in the previous step.
