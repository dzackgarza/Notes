...
Writing $Z_n \da Z_n(C)$, we now show the following:
\[
...
Note that $D$ is a complex obtained by applying the functor $F(\wait) \da \Hom(Z_n, \wait)$ to $C$, and so the induced maps on $D$ are given by $d^D \da F(d^C)$ where
\[
F(d_n^C): \Hom(Z_n, C_n) &\to \Hom(Z_n, C_{n-1}) \\
f & \mapsto d_n^C \circ f
.\]
