...
That is, the functor $\diagonal \hat F$ admits structure maps $\Phi: F\to \diagonal \hat{F}$ which assemble into a natural transformation (i.e. a morphism in $\cat{C}^{\cat{I}}$) such that any other natural transformation from $F$ to a diagonal object $\diagonal X$ produces a unique natural transformation $\eta: \diagonal X\to \diagonal F$.
:::
