...
:::{.solution}
Note: we'll assume $R$ is commutative.

$\implies$:
Suppose $A$ is divisible, we then want to show every \(R\dash\)module morphism of the following form lifts:
...
Using $R\dash$linearity and the fact that $j$ can not be a zero divisor and is thus left-cancelable, we have
\[
...
,\]
where in the last step we have canceled an $r$ on the left, using the fact a PID is a domain and thus has no nonzero zero divisors, and in particular $r$ is not a zero divisor and is thus left-cancelable.
