...
1. Taking an *injective* resolution of $B\to I \da (I_0 \mapsvia{\bd_0}  I_1 \mapsvia{\bd_1}  \cdots)$
2. Applying $\Hom_R(A, \wait)$ to get the complex $FI \da (0\to \Hom_R(A, I_0) \mapsvia{F(\bd_0)}  \Hom_R(A, I_1) \mapsvia{F(\bd_1)} \cdots)$
3. Defining $R^iF(B) \da \ker F(\bd_i) / \im F(\bd_{i-1})$.

Note that in step (2), if $\Hom_R(A, \wait)$ is exact, then noting that $I$ is exact, $FI$ is exact/acyclic, and so $\ker F(\bd_i) = \im F(\bd_{i-1}) = 0$ for $i\geq 1$.
:::
...
If $F$ is exact, then $F\xi$ is the SES $0\to FC\to FB \to FA\to 0$.
...
Thus all of the higher terms are iterated cokernels, and inductively since all of the numerators are zero, $L_iF(M_j) \da \Ext^i(A, M_j) = 0$.
