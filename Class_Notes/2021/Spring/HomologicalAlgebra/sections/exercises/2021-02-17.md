...
we then want to show that $jR = R$.
$J = \gens{ j }$ for some generator.
...
&= rj \tilde f_a(1_r)\\
\implies \vector{a} = j\tilde f_a(1_r)
,\]
where in the last step we have canceled an $r$ on the left, using the fact 
