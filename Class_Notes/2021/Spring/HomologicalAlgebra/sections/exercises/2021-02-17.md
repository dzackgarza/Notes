...
\[
\tau\qty{ \hat{F} \mapsvia{f} X  } \da \qty{ F \mapsvia{\Psi} \diagonal \hat{F} \mapsvia{\diagonal(f)} \diagonal X }
,\]
i.e. we take the image $\Delta(f)$ and pre-compose with the structure morphism $\Psi$.

This is surjective by the universal property: any morphism $F \mapsvia{g} \diagonal X$ in $\cat{C}^{\cat{I}}$ factors through $\diagonal \hat{F}$.
