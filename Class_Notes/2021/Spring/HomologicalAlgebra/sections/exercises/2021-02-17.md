...
Since $R$ is a PID, we can write $J = \gens{ j }$ for some generator.
Fixing a $\vector{a}\in A$, define a map $f_a: J\to A$ on generators as follows: for $x\in J$, use the fact that \( \gens{ j }\da jR  \) to first write $x = jr$ for some $r\in R$, and then set $f_a(x) = f_a(jr) \da r \vector{a}$.
To summarize, we have
\[
f_a: J = jR &\to R \\
jr &\mapsto r\vector{a}
.\]
...
&= \tilde f_a(\iota(jr)) \\
&= \tilde f_a(jr) \\
&= jr \tilde f_a(1_r) && \text{using $R\dash$linearity} 
&= rj \tilde f_a(1_r)
.\]
