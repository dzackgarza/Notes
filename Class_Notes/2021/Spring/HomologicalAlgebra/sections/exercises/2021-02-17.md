...
- Applying the natural isomorphism coming from the adjunction, this is isomorphism to a sequence involving terms $\Hom(\wait, RB)$.
- This sequence is exact, so applying Yoneda yields an exact sequence $0\to RB' \to RB \to RB''$, making $R$ left-exact.

Finally, pushing a left adjoint $L$ into the opposite category makes $L\op$ a right adjoint.
Thus $L\op$ is left-exact, making $L$ right-exact.
:::
