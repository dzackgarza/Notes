...
:::{.proof title="?"}
Using the same argument above, consider the lower order terms of the associated LES:
...
That the map indicated by the dotted line exists and is an isomorphism holds in any abelian category, using that fact that whenever $0\to A \mapsvia{f} B \to C\to 0$ is a SES we have $A \cong \ker f$.
