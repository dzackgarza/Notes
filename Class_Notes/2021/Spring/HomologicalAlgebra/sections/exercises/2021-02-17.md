...
2. $f$ descends to a map $\tilde f: \ZZ/m \to \QQ/\ZZ$ if and only if $f(n) \in \ZZ$, i.e. $f(n) = [0]$ is in the equivalence class of zero in the quotient, and so
\[
[1] = [0] = f(n) = nf(1)
.\]

We can thus identify the submodule $\Hom(\ZZ/m, \QQ/\ZZ)$ as all of those morphism $\ZZ\to \QQ/\ZZ$ which descend to make the following diagram commute.
...
Moreover, we can restrict our attention to coset representatives in the interval $[0, 1) \intersect \QQ \subseteq \RR$, and we then look for rationals $q \da f(1) \in [0, 1)$ such that $nq = 1$.
