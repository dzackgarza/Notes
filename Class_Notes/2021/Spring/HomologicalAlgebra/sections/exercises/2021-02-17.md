...
By definition, this is equivalent to showing the existence of a natural bijections of sets 
\[
\tau: \Hom_{\cat{A}}(\hat{F}, X) \mapsvia{\sim} \Hom_{\cat{A}^{\cat{I}} }(F, \diagonal X)
.\]
...
Applying the $\diagonal$ functor, we can view this as a simpler universal property in $\cat{C}^{\cat{I}}$, since the above data is precisely the data of a natural transformation:
...
That is, the functor $\diagonal \hat F$ is equipped with structure maps $\Phi: F\to \diagonal \hat{F}$ which assemble into a natural transformation (i.e. a morphism in $\cat{C}^{\cat{I}}$) such that any other natural transformation from $F$ to a diagonal object $\diagonal X$ produces a unique natural transformation $\eta: \diagonal X\to \diagonal F$.
This provides exactly the data needed to specify $\tau$:
\[
\tau: \Hom_{\cat{A}}(\hat{F}, X) &\mapsvia{\sim} 
...
:::{.proof title="?"}
This is surjective by the universal property: any morphism $F \mapsvia{g} \diagonal X$ in $\cat{C}^{\cat{I}}$ factors through $\diagonal \hat{F}$, and so all such morphisms are of this form.
...
\begin{tikzcd}
	...
	\arrow["{(Lf)_*}", from=1-1, to=1-3]
	\arrow["{g_*}", from=1-3, to=1-5]
	\arrow["{f_*}", from=3-1, to=3-3]
	\arrow["{(Rf)_*}", from=3-3, to=3-5]
\end{tikzcd}


> [Link to Diagram](https://q.uiver.app/?q=)
