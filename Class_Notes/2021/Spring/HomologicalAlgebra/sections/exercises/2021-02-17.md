...
For general setup, let $\xi$ be a SES, then apply the covariant hom $F(\wait) \da \Hom_R(A, \wait)$ to $\xi$ to obtain an associated LES:
...
- $M_1 = A$, 
- $M_2$ be the first stage in an injective resolution $A \mapsvia{\bd_0} I$, and
- $M_3 \da \coker \bd_0$.

This yields a SES:
\[
\xi: 0 \to A \mapsvia{\bd_0}  I \to \coker \bd_0 \to  0
,\]
and thus a LES of the above form.
Since $I$ is injective, it is $F\dash$acyclic for $F(\wait) \da \Hom_R(A, \wait)$
