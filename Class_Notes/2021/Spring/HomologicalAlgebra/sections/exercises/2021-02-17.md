...
Starting with the left-hand side, to compute left-derived functors, we'll need projective resolutions, so let $P$ be a projective resolution of $X$.
...
\implies
U\qty{ L_iF(X)}  \da U\qty{ \ker F(\bd_i) \over \im F(\bd_{i+1})}
.\]
