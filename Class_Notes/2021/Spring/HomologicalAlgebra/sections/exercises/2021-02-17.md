...
\[
f_{i'}(1) = f_{i + kn}(1) = \left[ {i + kn \over n}\right] = \left[{i\over n} + k\right] = \left[ {i\over n} \right] 
,\]
since $k$ was an integer, so by the first isomorphism theorem $\Psi$ descends to an isomorphism
\[
\tilde \Psi: \ZZ/n \mapsvia{\sim} \Hom(\ZZ/n, \QQ/\QQ)
.\]
