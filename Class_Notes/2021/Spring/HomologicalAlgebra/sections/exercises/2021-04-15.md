...
:::{.proof title="of 2"}
Let $M \mapsvia{ f} N$ be a monic morphism of $\lieg\dash$modules, so that for any pair $g_1, g_2: Z\to M$ if $\liegmod$ morphisms with $f \circ g_1 = f \circ g_2$, we have $g_1 = g_2$.
:::
