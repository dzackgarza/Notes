...
&\da (g\tensor m \mapsto f(g, m))
,\]
and so this recovers $f$.

\

Similarly, starting with $h: \lieg \to \liegl(M)$, applying $\Psi$ yields $\Psi_h \da (g\tensor m \mapsto h(g)(m))$, and applying $\Theta$ yields 
\[
\Theta_{\Psi_h} 
&\da (g \mapsto \Psi_h(g, \wait)) \\
&\da (g\mapsto ?)
.\]
