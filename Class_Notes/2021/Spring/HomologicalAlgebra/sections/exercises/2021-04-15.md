...
\[
\Theta: \Hom_{\liegmod}(M, N) &\to \Hom_{\kmod}(M, N)^{\lieg} \\
f & \mapsto F(f)
\]
where $F: \liegmod\to \kmod$ is the forgetful functor, the above argument shows that this is a bijection.
It only remains to check that $\Theta$ is a morphism of $\lieg\dash$modules, but this follows from the fact that $F$ and $F(f)$ are pointwise defined in precisely the same way on the underlying sets, and so the condition $\Theta(x\cdot f) = x\cdot \Theta(f) \da x\cdot F(f)$
\[
\Theta(x\cdot f)(m)
&\da \theta( x\cdot f(m) - f(x\cdot m)) \\
&= (x\cdot \Theta(f))(m)
,\]
and so $\Theta(xf) = x\Theta(f)$.
