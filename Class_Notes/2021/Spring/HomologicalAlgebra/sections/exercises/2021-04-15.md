...
This argument in fact shows that the following forgetful functor is faithful:
\[
F(\wait) \da \Forget(\wait): \liegmod \to \kmod
,\]
which gives a set-level injection of hom sets
\[
\Hom_{\liegmod}(M, N) \injects \Hom_{\kmod}(F(M), F(N) ) && \forall M, N \in \liegmod
.\]
So suppose $f\in \Hom_{\liegmod}(M, N)$ is monic, so $fg_1 = fg_2 \implies g_1 = g_2$ for all $g_i \in \Hom_{\liegmod}(M, N)$.
Considering the subcategory $F(\liegmod) \leq \kmod$
