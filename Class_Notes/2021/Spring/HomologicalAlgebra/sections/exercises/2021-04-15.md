...
Consider the forgetful functor $F: \liegmod\to\kmod$.
Since it is a functor, there will be an induced map on hom sets, and the content of this question is showing that the image consists only of $\lieg\dash$invariant functions under the described $\lieg\dash$action.
We first note that we can write
\[
\Hom_{\kmod}(M, N)^{\lieg} 
&\da \ts{ f\in \Hom_{\kmod}(M, N) \st x\cdot f = 0 \,\,\forall x\in \lieg } \\
&\da \ts{ f\in \Hom_{\kmod}(M, N) \st xf(m) - f(xm) = 0_M  \,\,\forall m\in M,\,\, \forall x\in \lieg } \\
&\da \ts{ f\in \Hom_{\kmod}(M, N) \st xf(m) = f(xm)  \,\,\forall m\in M,\,\, \forall x\in \lieg } \\
&\supseteq \Hom_{\liegmod}(M, N)
,\]
since we've constructed the $\lieg\dash$action precisely to satisfy this condition.
For the reverse containment, given $g\in \Hom_{\liegmod}(M, N)$, we have $F(g)\in \Hom_{\kmod}(M, N)$ and the claim is that it is $\lieg\dash$invariant.
But this is precisely the same condition:
