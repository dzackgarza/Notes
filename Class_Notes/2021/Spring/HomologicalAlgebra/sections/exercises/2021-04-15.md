...
\Psi_h \da (g\tensor m \mapsto h(g)(m) ) &\mapsfrom h
,\]
so $\Theta_f(g)(m) \da f(g, m)$ and $\Psi_h(g, m) \da h(g)(m)$.
...
:::{.proof title="?"}
Starting with $f: \lieg\tensor M\to M$, we obtain $\Theta_f\da (g\mapsto f(g, \wait))$.
Applying $\Psi$ yields 
\[
\Psi_{\Theta_f} \da (g \tensor m \mapsto \Theta_f(g)(m)) \da (g\tensor m \mapsto f(g, \wait)(m) ) \da (g\tensor m \mapsto f(g, m))
,\]
and so this recovers $f$.

\

Similarly, starting with $h: \lieg \to \liegl(M)$, applying $\Psi$ yields $\Psi_h \da (g\mapsto h(g)(\wait))$, and applying $\Theta$ yields 
\[
\Theta_{\Psi_g} \da (g \mapsto \Psi_g(g, \wait))
.\]
