...
It only remains to check that $\Theta$ is a morphism of $\lieg\dash$modules, but this follows from the fact that $F$ and $F(f)$ are pointwise defined in precisely the same way on the underlying sets.
Thus we can write
\[
\Theta(x\cdot f)(m)
...
\da (x\cdot \Theta(f))(m)
,\]
and so $\Theta(xf) = x\Theta(f)$, making $\Theta$ product-preserving.
