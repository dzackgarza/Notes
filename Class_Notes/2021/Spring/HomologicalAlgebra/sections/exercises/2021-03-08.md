...
\begin{tikzcd}
	...
	\arrow["{\phi_i}", curve={height=-30pt}, from=3-3, to=4-8]
	\arrow["{\phi_j}"', curve={height=30pt}, from=5-3, to=4-8]
...
\begin{tikzcd}
	...
	\\
	i && {F(i)} \\
	&&&& \textcolor{rgb,255:red,51;green,109;blue,255}{\exists \colim_{i'\in \cat{I}} F(i') } &&& RX \\
	j && {F(j)}
	\arrow["{F\eps_{ij}}"', from=3-3, to=5-3]
	\arrow["{R\phi_i}", curve={height=-30pt}, from=3-3, to=4-8]
	\arrow["{R\phi_j}"', curve={height=30pt}, from=5-3, to=4-8]
...
Here, the existence of the blue data is supplied by the fact that $F$ has a colimit $\hat F$ in $\cat{A}$.
We've also used the fact that $RLF(i) \cong F(i)$,
