...
\]
is exact.
Note that $2\implies 3$ is immediate, and that $2\implies 1$ follows by applying $\wait \tensor_R B$ to a SES and taking the associated LES (noting that tensoring is covariant, right-exact, and so the tor terms occur as left-derived functors):
...
\begin{tikzcd}
	...
	\\
	& \textcolor{rgb,255:red,214;green,92;blue,92}{\Tor_1^R(A'', B)} & \textcolor{rgb,255:red,214;green,92;blue,92}{\Tor_1^R(A, B)} & \textcolor{rgb,255:red,214;green,92;blue,92}{\Tor_1^R(A', B)} \\
	\\
	& {} && \cdots
...
> [Link to Diagram](https://q.uiver.app/?q=)

If the terms $\Tor^R_n$ all vanish, then in particular the red terms vanish, yielding an exact sequence
\[
0 \to A'' \tensor_R B \to A\tensor_R B \to A' \tensor_R B \to 0
,\]
making $\wait\tensor_R B$ an exact functor.
By the same argument, $3\implies 1$, since this only requires $\Tor_1$ to vanish, so it only remains to show 
