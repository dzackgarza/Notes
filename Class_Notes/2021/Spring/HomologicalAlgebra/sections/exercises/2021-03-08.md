...
That the inverse map is well-defined follows from the fact that the canonical quotient map $\pi_A$ exists, and compositions in a category are unique when they exist since (axiomatically) the composition pairing must form a well-defined set map on morphisms:
\[
