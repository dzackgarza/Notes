...
&=
\prod_{i+j=q} 
\Hom_\Ab \qty{F(P_p), { \Hom_\Ab(Q_i, I_j) } } \\
&= 
\prod_{i+j=q} 
...
- We've used that $\Hom_\Ab(\cdots)$ will only have a \(\ZZ\dash\)module structure in general, and an \(R\dash\)module morphism into a \(\ZZ\dash\)module is determined by what it does to the underlying \(\ZZ\dash\)module.
  - We've also applied the forgetful functor $F: \mods{R} \to \mods{\ZZ}$ explicitly in one step, and implicitly used the underlying \(\ZZ\dash\)module after the hom was converted to a hom in $\Ab$.
- We've applied the tensor-hom adjunction in \( \rmod \), 
- Used that finite direct products are isomorphic to direct sums.


:::{.remark}
I think this was the intended proof/computation, but there are some parts that do still seem sketchy.
- The adjunction step in the second computation doesn't work directly as I've written it, since $I_j$ has a \(\ZZ\dash\)module structure but no assumed \(R\dash\)module structure.

  - However, it's not clear if a forgetful functor \( \rmod \to \mods{\ZZ} \) can be applied at that step and still induce an isomorphism on the hom sets.

- There are two different hom sets appearing, the first in \(\ZZ\dash\)modules and the second in \(R\dash\)modules.
