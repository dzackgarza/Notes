...
Now to see that $1\implies 2$, we use dimension shifting.
Letting $A \in \modsright{R}$ be an arbitrary right \(R\dash\)module, by exercise 2.4.3 if we define the functor $F(A) \da A\tensor_R B$ and take any truncated projective resolution 
\[
M_m \to P_m \to \cdots \to P_0 \to A\to 0
,\]
which is an exact complex of projective \(R\dash\)modules.
