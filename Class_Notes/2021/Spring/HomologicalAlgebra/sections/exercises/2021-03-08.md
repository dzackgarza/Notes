...
Letting $A \in \modsright{R}$ be an arbitrary right \(R\dash\)module, if we define the functor $F(A) \da A\tensor_R B$, we can take a projective cover $P \mapsvia{f}  A$ to produce an exact sequence
\[
0 \to \ker f \to P \to A\to 0
,\]
and as a result of the exercise, we can compute $\Tor_i^R(A, B) \da L_iF(A) \da H_i (FP)$.
