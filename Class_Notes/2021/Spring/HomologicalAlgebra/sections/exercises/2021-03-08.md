...
Again using the adjunction isomorphism, this yields a system of corresponding maps 
\[ 
\eta_{ij} \in \Hom_{\cat B}(L \colim\limits_{i' \in \cat I} F(i), X) 
.\]


However, this means that taking the object $Y\da L \colim\limits_{i' \in \cat I} F(i')$ satisfies the universal property of the colimit $\colim_{i' \in \cat{I}} LF(i')$, which makes them equal by uniqueness.
