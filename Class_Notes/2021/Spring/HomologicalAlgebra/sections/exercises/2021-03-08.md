...
\begin{tikzcd}
	...
	\\
	i && {LF(i)} \\
	&&&& \textcolor{rgb,255:red,51;green,109;blue,255}{\exists Y} &&& X \\
	j && {LF(j)}
...
By the definition of the adjunction, we have an isomorphism
\[
...
\begin{tikzcd}
	...
	\\
	i && {F(i)} \\
	&&&& \textcolor{rgb,255:red,51;green,109;blue,255}{\exists \colim\limits_{i'\in \cat{I}} F(i') } &&& RX \\
	j && {F(j)}
	...
	\arrow["{\exists \tilde \psi_i}", color={rgb,255:red,51;green,109;blue,255}, dashed, from=3-3, to=4-5]
	\arrow["{\exists \tilde\psi_j}", color={rgb,255:red,51;green,109;blue,255}, dashed, from=5-3, to=4-5]
	\arrow["{\exists ! \tilde\eta_{ij}}", color={rgb,255:red,51;green,109;blue,255}, dashed, from=4-5, to=4-8]
	\arrow["{\eps_{ij}}", from=3-1, to=5-1]
\end{tikzcd}
...
Here, the existence of the blue data is supplied by the fact that $F$ has a colimit $\colim_{i'\in \cat I} F(i')$ in $\cat{B}$ by assumption, and since the system of maps $\varphi_i \in \Hom_{\cat A}(LF(i), X)$ induces a system of maps \( \tilde \varphi_i \in \Hom_{\cat B}( F(i), RX) \), by the universal property of colimits there is a system of maps \( \tilde\eta_{ij} \in \Hom_{\cat{A}}(\colim F(i), RX) \).
Again using the adjunction isomorphism, this yields a system of corresponding maps \( \eta_{ij} \in \Hom_{\cat B}(L \colim\limits_{i' \in \cat I} F(i), X) \).


However, this means that the object $L \colim\limits_{i' \in \cat I} F(i')$ satisfies the universal property of the colimit 

We note that we have the following isomorphism by adjunction:

- $\Hom_{\cat{A}}(F(i), \hat F) \cong \Hom_{\cat{B}}(LF(i)$
