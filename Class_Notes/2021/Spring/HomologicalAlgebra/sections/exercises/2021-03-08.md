...
\begin{tikzcd}
	...
	\\
	i && {F(i)} \\
	&&&& \textcolor{rgb,255:red,51;green,109;blue,255}{\exists \hat{F} } &&& RX \\
	j && {F(j)}
...
Here, the existence of the blue data is supplied by the fact that $F$ has a colimit $\hat{F}$ in $\cat{B}$ by assumption.
Since the system $\varphi_i \in \Hom_{\cat A}(LF(i), X)$ induces a system \( \tilde \varphi_i \in \Hom_{\cat B}( F(i), RX) \), the universal property of colimits supplies a system \( \tilde\eta_{ij} \in \Hom_{\cat{A}}(\hat{F}, RX) \).
