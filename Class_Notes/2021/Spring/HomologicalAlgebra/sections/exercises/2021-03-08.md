...
&=
\prod_{i+j=q} 
\Hom_R \qty{P_p \tensor_R Q_i , { I_j } } \\
&\cong
\bigoplus_{i+j=q} 
\Hom_R \qty{P_p \tensor_R Q_i , { I_j } } 
,\]
where we've applied the tensor-hom adjunction in \( \rmod \) and used that finite direct products are isomorphic to direct sums.


:::{.remark}
I think this was the intended proof sketch, but there are some parts that do not seem to go through! 
For example, the adjunction step in the second computation doesn't work directly as I've written it, since $I_j$ has a \(\ZZ\dash\)module structure but no assumed \(R\dash\)module structure.
...
There is also the issue of the two different hom sets appearing, the first in \(\ZZ\dash\)modules and the second in \(R\dash\)modules.
:::
