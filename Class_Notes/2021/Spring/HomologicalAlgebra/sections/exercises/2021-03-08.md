...
Here, the existence of the blue data is supplied by the fact that $F$ has a colimit $\colim_{i'\in \cat I} F(i')$ in $\cat{B}$ by assumption, and since the system of maps $\varphi_i \in \Hom_{\cat A}(LF(i), X)$ induce a system of maps \( \tilde \varphi_i \in \Hom_{\cat B}( F(i), RX) \), by the universal property of colimits there is a system of maps \( \tilde\eta_{ij} \in \Hom_{\cat{A}}(\colim F(i), RX) \).
