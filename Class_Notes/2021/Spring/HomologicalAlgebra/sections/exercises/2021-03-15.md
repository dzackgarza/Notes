...
if $f(g_j) \neq 0$ for any $j$ then noting that $G$ acts on itself transitively by left-multiplication, we can write any $g_i$ as $g_i = h(i) g_j$ for some $h=h(i)$ depending on $i$.
We then have \(  x = \sum f(g_i) g_i = \sum f(h(i) g_j)h(i)g_j \), and noting that 
\[
f(g_j) = f(h(i) g_j) \neq 0
,\]
if $G$ is infinite then this produces a sum with infinitely many nonzero terms.
$\contradiction$
