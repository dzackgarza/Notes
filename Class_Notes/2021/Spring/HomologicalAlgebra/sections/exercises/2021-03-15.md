...
Since this converges to $E^2_{p, q} \abuts H_{p+q}T \da H_n(T)$, there is increasing filtration $F_\bullet$ on $H_n(T)$ of the following form:
\[
0 \subseteq F_1 H_n(T) \subseteq F_2 H_n(T) = F_3 H_n(T) = \cdots = H_n(T)
,\]
where we've used the fact that $E^{\infty }
