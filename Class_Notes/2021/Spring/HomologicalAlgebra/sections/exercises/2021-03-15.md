...
.\]
Noting that the first term is isomorphic to $E_{2, 0}^{\infty }$ by the above remark and the last is isomorphic to $E_{1, 0}^{\infty }$ by the analysis in total degree 1, this yields a sequence
\[
E_{2, 0}^{\infty } \injects E_{2, 0}^2 \to E_{0, 1}^2 \to E_{0, 1}^{\infty }
.\]
