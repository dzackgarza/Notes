...
The inverse map sends $g$ to the same underlying set map, now regarded as a map of trivial \(G\dash\)module.
This is a well-defined \(G\dash\)module morphism, using the fact that $gm = m$ implies that $g\phi(m) = \phi(gm) = \phi(m)$.
