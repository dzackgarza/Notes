...
\[
\ZZ G^G &\to \ZZ \tensor_{\ZZG} \ZZG &&\to \ZZ \\
nN\da n \qty{\sum_{g\in G} g} &\mapsto n \tensor \qty{\sum_{g\in G} g} &&\mapsto \qty{ \sum_{g\in G} g}\actson n \da \sum_{g\in G} n 
,\]
where we've used the trivial action to write $(g+h)\actson n = gn + hn \da n + n = 2n$ and extend this over the sum.
So we have
\[
n N \mapsto \sum_{g\in G} n = n \sum_{g\in G} 1 = n (\# G)
,\]
and taking $n=1$ sends $N\mapsto \# G$.
That the first map is well defined follows from the fact that the there is a map to the product
\[
\ZZG^G \to \ZZ \times \ZZ_G
n N &\mapsto (n, N)
,\]
and this is $\ZZG\dash$bilinear since the $\ZZG\dash$action on $\ZZ$ is trivial.
