...
\[
h \mapsto \Psi_{BM}(h) \mapsto f\circ \Psi_{BM}(h)
,\]
and now going right and down yields
\[
h \mapsto Lf \circ h \mapsto \Psi_{AM}(Lf \circ h)
.\]
We can note that for $f\in \Hom_\Ab(A, B)$, the map $Lf\in \Hom_G(\Triv(A), \Triv(B))$ is precisely the same map on the underlying sets, so $Lf(a) = f(a)$ at the level of elements.
We can then use the fact that $\Triv(B)^G = \Triv(B)$, since $G$ acts trivially by definition, and so $\Psi_{AM}(Lf) \in \Hom_\Ab(A, \Triv(B)^G) = \Hom_\Ab(A, B)$.
Moreover, $\Psi$ preserves the underlying set function but just restricts codomains, so $\Psi_{AM}(Lf)(a) = Lf(a)$ on elements.
But then
\[
\Psi_{AM}(Lf)(a) = Lf(a) = f(a) && \forall a\in A
,\]
forcing $\Psi_{AM}(Lf) = f$ as functions.

We also have that $\Psi_{AM}(h) = \Psi_{BM}(h)$
