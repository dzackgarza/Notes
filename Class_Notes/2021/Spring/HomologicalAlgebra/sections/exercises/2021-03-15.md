...
This follows because the degree zero part of the associated graded will be given by $\bigoplus_{p+q=0}E_{p, q}^\infty$, so $p=q=0$ will contribute a nonzero term, and every other term must involve either $p<0$ or $q<0$ and thus be zero, and $F_j / F_{j-1} = 0$ for some $j$ forces $F_j \cong F_{j-1}$.
We thus have
\[
E_{0, 0}^2 
&= E_{0, 0}^{\infty}\\
