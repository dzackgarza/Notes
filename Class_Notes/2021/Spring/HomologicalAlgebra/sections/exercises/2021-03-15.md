...
For ease of notation, we assume $G$ is countably infinite.
...
g \sum_{j\in J} f(g_j) g_{j} = 
\sum_{j \in J } f(g_j) g_{j}\,\,\forall g\in G
}
...
The coefficient of $gg_j$ in the first expression is $f(gg_j)$, and is $f(g_j)$ in the second, so we must have $f(g_j) = f(gg_j)$ for all $g\in G$.
But since $G$ is infinite, fixing an $x\in (\ZZ G)^G$, if $f(g_j) \neq 0$ for any $j$ then noting that $G$ acts on itself transitively by left-multiplication, we can write any $g_i$ as $g_i = g(i) g_j$ for some $g=g(i)$ depending on $i$.
We then have \(  \) <++>
