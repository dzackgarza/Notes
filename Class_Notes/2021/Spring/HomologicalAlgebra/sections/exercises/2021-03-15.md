...
\[
\Hom_{\ZZ G}( \ZZ, A) &\mapsvia{\sim} A^G \\
f &\mapsto f(1)
.\]

:::
Note that taking $A \da \ZZ G$ yields the first in (1), since by definition 6.1.2,
\[
H^0(G; A) \cong \Ext_{\ZZ G}^0(\ZZ, A) \da \Hom_{\ZZ G}(\ZZ, A)
.\]
