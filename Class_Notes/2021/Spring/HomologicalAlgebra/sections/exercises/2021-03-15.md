...
\begin{sseqdata}[name = ex1, homological Serre grading, classes = {draw = none }, xscale=3, yscale=3]
\class["E_{0, 2}^2"](0,2)
...
Passing to the 3rd page yields the following, where several new terms have stabilized:
...
This follows because the associated graded will be given by $\bigoplus_{p+q=0}E_{p, q}^\infty$, so $p=q=0$ will contribute a nonzero term, and every other term must involve either $p<0$ or $q<0$ and thus be zero, and $F_j / F_{j-1} = 0$ for some $j$ forces $F_j \cong F_{j-1}$.
We thus have
\[
E_{0, 0}^2 
&= E_{0, 0}^{\infty}\\
