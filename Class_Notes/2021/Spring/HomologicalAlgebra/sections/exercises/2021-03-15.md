...
For some indexing set $J$, we can write
\[
(\ZZ G)^G \da \ts{ 
\sum_{\alpha \in J} n_{ \alpha} g_{\alpha} \st 
g_\beta \sum_{\alpha\in J} n_{ \alpha} g_{\alpha} = 
\sum_{\alpha\in J } n_{ \alpha} g_{\alpha}\,\,\forall \beta\in J
}
.\]
Since the \( g_{ \alpha} \) form a basis of the free $\ZZ\dash$module $\ZZ G$, we can equate coefficients: writing \( g_{ \alpha \beta} \da g_{ \alpha} g_{\beta} \).
