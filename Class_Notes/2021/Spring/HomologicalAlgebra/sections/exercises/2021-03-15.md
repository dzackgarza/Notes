...
Since this converges to $E^2_{p, q} \abuts H_{p+q}T \da H_n(T)$, there is increasing filtration $F_\bullet$ on $H_n(T)$ such that $E^{\infty }_{p, q} = F_p H_n(T) / F_{p-1}H_n(T)$.
Thus the filtration has the form
\[
0 \subseteq F_0 H_n(T) \subseteq F_1 H_n(T) = F_2 H_n(T) = \cdots = H_n(T)
,\]
where we've used that if $i\neq 0, 1$ we have $E^{\infty }_{i, q} = 0 = F_i H_n(T) / F_{i-1}H_n(T)$ which implies $F_i H_n(T) = F_{i-1}H_n(T)$.
Setting $i=0, 1$ we have
\[
E^2_{0, q} \cong F_0 H_n(T) \\
E^2{1, q} \cong F_1 H_n(T) / F_0 H_n(T)
,\]
