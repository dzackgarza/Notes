...
where we realize $M_G \da M/ \gens{ gm-m }$ and take $\bar f$ to be the natural quotient map, and define $\tilde h(m) \da h(m + R)$ as a map of sets.
...
The inverse map is a well-defined \(G\dash\)module morphism, using the fact that $gm = m$ implies that $g\phi(m) = \phi(gm) = \phi(m)$.
