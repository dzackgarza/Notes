...
Thus $B\cong \ZZ/m \oplus \ZZ/2 \cong \ZZ/m \times \ZZ/2 \cong \ZZ/2m$, where we've now used the $\gcd(2, m) = 1$.
So
\[
H_n(C_{2m}; \ZZ) 
=
...
% https://q.uiver.app/?q=
\begin{tikzcd}
...
Let $\rho: H\to G$ be a group morphism, then any \(G\dash\)module becomes an \(H\dash\)module by composition with $\rho$, which yields a functor
\[
\rho^{\#}: \mods{G} \to \mods{H}
.\]
