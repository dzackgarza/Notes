...
We say two extensions $\xi, \xi'$ iff
...
Then $\xi$ is split if and only if $\one-B$ lifts to some $\sigma\in \Hom(X, B)$ if and only if $\one_B \in \im f_* = \ker \bd$ where $\Theta(\xi) = 0$ (even if $\Ext^1(A, B) \neq 0$), 
:::
