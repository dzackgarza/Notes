...
> [Link to Diagram](https://q.uiver.app/?q=)
...
:::{.observation}
If $R$ is a field $F$ then $\Tor_n^F(A, B) = 0$ for all $n>0$, i.e. every module over a field is a vector space, hence free, hence projective, hence flat, and so $A\tensor_F \wait$ is exact.
:::

:::{.question}
If $\vector P \in \Ch(\modr)$ is a complex of of right \(R\dash\)modules and $M \in \rmod$ is a left \(R\dash\)module, how is the homology of $\vector P$ and that of $\vector P \tensor_R M$ related?
:::
...
\begin{tikzcd}
	0 & A & B & C & 0 \\
	& {A_2/\ker f = A_2/\im\alpha = \coker \alpha} && {\im g = \ker f}
...
Let $\vector P\in \Ch(\modr)$ be a chain complex of flat right \(R\dash\)modules such that each boundary module $dP_n$ is again flat.
Then for every $M\in \rmod$ and all $N$, there is an exact sequence 

\begin{tikzcd}
	0 && {H_n(\vector P)\tensor_R M} && {H_n(\vector P \tensor_R M)} && {\Tor^1_R(H_{n-1}(\vector P), M)} && 0
	\arrow[from=1-7, to=1-9]
	\arrow[from=1-5, to=1-7]
	\arrow[from=1-3, to=1-5]
	\arrow[from=1-1, to=1-3]
...
\begin{tikzcd}
	{Z_n\tensor_R M} && {P_n\tensor_R M} && {dP_n\tensor_R M} && 0 \\
	\\
	&&&& {\Tor_1(dP_n, M)}
...
> [Link to Diagram](https://q.uiver.app/?q=WzAsNSxbNiwwLCIwIl0sWzQsMCwiZFBfblxcdGVuc29yX1IgTSJdLFsyLDAsIlBfblxcdGVuc29yX1IgTSJdLFswLDAsIlpfblxcdGVuc29yX1IgTSJdLFs0LDIsIlxcVG9yXzEoZFBfbiwgTSkiXSxbNCwzXSxbMywyXSxbMiwxXSxbMSwwXV0=)
