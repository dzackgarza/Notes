...
:::{.remark}
Our proof in 2.7 that $\Tor_*^R(A, B)$ could be computed either by a projective resolution $\complex{P}\surjects A$ or an injective resolution $B \injects \complex{Q}$ was a disguised spectral sequence argument.
So we'll go recover it using the actual spectral sequence.
:::


:::{.remark}
We have a $QI$ double complex $(P\tensor Q)_{p, q} = P_p\tensor Q_q$.
:::
