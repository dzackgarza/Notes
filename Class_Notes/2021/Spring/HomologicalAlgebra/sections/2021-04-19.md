...
\[
H^1(\lieg; M) \cong { \Hom_{\liegmod}( \II, M) \over \im \qty{ \Hom_{\Ug}(\Ug, M)  \to \Hom_{\lieg}( \II, M)  } }
,\]
where the source in the denominator is isomorphic to $M$, given by the map $\ev_1$.
We found a map
\[
\Hom_\lieg(\II, M) \mapsvia{\sim} \Der(\lieg, M) \\
...
The map appearing there is restriction to $\II$, i.e. $\phi \mapsto \ro{\phi}{\II}$.
...
\[
\Extalgebra^* \lieg \da {T(\lieg) \over \gens{x^{\tensor 2} \st x\in \lieg  } = \bigoplus _{p\geq 0} \Extalgebra^p \lieg
.\]
We write $x_1\wedge x_2\wedge\cdots \wedge x_p$ for the image of $\lieg \injects \Extalgebra \lieg$
Note that this is a 2-sided homogeneous ideal, and since $x\wedge x = 0$ we have $x\wedge y = -y\wedge x$.
:::

:::{.remark}
If \( \ts{ x_\alpha } \) is an ordered basis for $\lieg$, then there is an ordered basis for $\Extalgebra^p \lieg$:
\[
...
We also have \( \Extalgebra^0 \lieg \cong k \) with a basis of $1_k$, and \( \Extalgebra^1 \lieg \cong \lieg \).
In particular, if $\dim \lieg = n < \infty$, then \( \Extalgebra^p \lieg = 0 \) for all $p>n$, and in this case \( \Extalgebra^n \lieg \cong k \).
:::

:::{.definition title="The Chevalley-Eilenberg (or Koszul) Complex"}
Define
\[
V_p(\lieg) \da \Ug \tensor_k \Extalgebra^p\lieg
,\]
...
\begin{tikzcd}
	...
	\\
	{\Ug \tensor_k \Extalgebra^p \lieg} && {\Ug \tensor_k \Extalgebra^{p-1} \lieg} &&&& {\Ug \tensor_k \lieg} && \Ug &&&& 0 \\
	{u \tensor \Wedge_j x_{i_j}} && {\theta_1 + \theta_2} &&&& {u\tensor x} && ux
	\arrow["\cong", no head, from=3-9, to=5-9]
	\arrow["{\exist \eps}", dashed, from=5-9, to=5-11]
...
> [Link to Diagram](https://q.uiver.app/?q=)


Here we define
\[
\theta_1 &\da \sum_{i=1}^p ux_i \tensor x_1 \tensor x_1 \wedge \cdots \wedge \hat{x_i} \wedge \cdots \wedge x_p
\theta_2 &\da \sum_{i<j}^p (-1)^{i+j} u\tensor [x_i x_j] \tensor x_1 \tensor x_1 \wedge \cdots \wedge \hat{x_i} \wedge \cdots \wedge \hat{x_j} \wedge \cdots \wedge x_p
,\]
where the hat denotes omitting a term.
Note that $\im d_1 = \Ug\,\lieg = \II = \ker \eps$, so we get exactness at the first position, and exercise 7.7.1 shows that $d^2 = 0$.
...
The Koszul complex $V_*(\lieg) \surjectssvia{\eps} k$ is a projective resolution in $\liegmod$.
:::
