...
1. For all $r\geq a$ and all $p, q\in \ZZ$, a family \( \ts{ E_{p, q}^r } \) of objects in \( \cat{A} \) (some of which my be zero), where typically $a=1, 2$.

2. A family of maps \( \ts{ d_{p, q}^r: E_{p, q}^r \to E_{p-r, q+r-1}^r } \)  with $d^r \circ d^r =0$ of slope $-\frac{r-1}{r}$ in that lattice $E_{*, *}^r$ the form chain complexes.
...
3. Isomorphisms $E_{p, q}^{r+1} \cong \ker d_{p, q}^r / \im d_{p+r, q-r+1}^r$.
