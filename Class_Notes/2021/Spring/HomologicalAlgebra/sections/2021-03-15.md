...
> [Link to Diagram](https://q.uiver.app/?q=WzAsMTAsWzIsNSwicC0xIl0sWzMsNSwicCJdLFswLDMsIm4tcD1xIl0sWzAsMiwicSsxIl0sWzIsMiwiXFxidWxsZXQiXSxbMywzLCJcXGJ1bGxldCJdLFswLDAsIm4iXSxbNSw1LCJuIl0sWzEsMSwiXFx2ZG90cyJdLFs0LDQsIlxcdmRvdHMiXSxbOCw0XSxbNCw1XSxbNSw5XV0=)
...
a. For all $r\geq a$ and all $p, q\in \ZZ$, a family \( \ts{ E_{p, q}^r } \) of objects in \( \cat{A} \) (some of which my be zero), where typically $a=1, 2$.

b. A family of maps \( \ts{ d_{p, q}^r: E_{p, q}^r \to E_{p-r, q+r-1}^r } \)  with $d^r \circ d^r =0$ of slope $-\frac{r-1}{r}$ in that lattice $E_{*, *}^r$ the form chain complexes.
...
c. Isomorphisms $E_{p, q}^{r+1} \cong \ker d_{p, q}^r / \im d_{p+r, q-r+1}^r$.
...
:::{.remark}
The term $E_{p, q}^{r+1}$ is a *subquotient* of $E_{p, q}^r$, and hence inductively a subquotient of $E_{p, q}^a$ by transitivity of "being a subquotient".
:::
