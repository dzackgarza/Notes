...
::: {.remark}
Last time: Riemann-Roch for surfaces, today we'll discuss some examples. Recall that if \( S \in {\mathbf{Mfd}}_{\mathbb{C}}^2 \) is closed and compact (noting that \( S\in {\mathbf{Mfd}}_{\mathbb{R}}^4 \)) and \( L\to S \) is a holomorphic line bundle then
\[
\chi(S, L) = \chi({\mathcal{O}}_S) + {1\over 2}(L^2 - L \cdot K)
\]
where \( K = c_1(K_S) \) for \( K_S \coloneqq\Omega_S^2 \) the canonical bundle and \( L = c_1(L) \). We also saw
\[
\chi({\mathcal{O}}_S) = {1\over 12}(K^2 + \chi_{{\mathbf{Top}}}(S))
,\]
where \( \chi_{\mathbf{Top}} \) is the Euler characteristic and is given by
\[
\chi_{\mathbf{Top}}(S) = 2 h^0(S; {\mathbb{C}}) - 2 h^1(S, {\mathbb{C}}) + h^2(S; {\mathbb{C}})
.\]
:::
...
We can use this to conclude that \( \chi_{\mathbf{Top}}({\mathbb{CP}}^n) = n+1 \) and \( \chi_{\mathbf{Top}}({\mathbb{CP}}^2) = 3 \). Over \( {\mathbb{CP}}^n \) we have a **tautological line bundle** \( {\mathcal{O}}(-1) \) given by sending each point to the corresponding line in \( {\mathbb{C}}^{n+1} \), i.e. \( {\mathcal{O}}(-1) \to {\mathbb{CP}}^n \) given by
\[
...
Given any \( X\in {\mathbf{Mfd}}_{\mathbb{C}} \), define \( {\operatorname{Pic}}(X) \) as the set of isomorphism classes of holomorphic line bundles on \( X \). This is an abelian group given by \( L \otimes L' \) and inversion \( L\to L^{-1} \).
:::
...
::: {.proof title="?"}
We know that \( \chi_{\mathbf{Top}}(S^4) = 2 \). If \( S^4 \) had a complex structure, then \( c_1(K_{S^4}) \in H^2(S^4; {\mathbb{Z}}) = 0 \). Thus would make \( K_{S^4}^2 = 0 \), and so
\[
...
::: {.proof title="?"}
We have \( \chi_{\mathbf{Top}}= 3 \), and \( K_{\mkern 1.5mu\overline{\mkern-1.5mu{\mathbb{CP}}\mkern-1.5mu}\mkern 1.5mu^2} = -c_1(T \mkern 1.5mu\overline{\mkern-1.5mu{\mathbb{CP}}\mkern-1.5mu}\mkern 1.5mu^2) = \pm 3H \). Then
\[
\chi({\mathcal{O}}) = {1\over 12}\qty{ K_{\mkern 1.5mu\overline{\mkern-1.5mu{\mathbb{CP}}\mkern-1.5mu}\mkern 1.5mu^2}^2 + \chi_{\mathbf{Top}}} = {1\over 12}(-9+3) \not\in {\mathbb{Z}}
.\]
:::
...
::: {.remark}
We had \( \chi({\mathcal{O}}) = {1\over 12} \qty{ K^2 + \chi_{\mathbf{Top}}} = {1\over 12}(3-n^2) \), and since \( 3-n^2 \in 12{\mathbb{Z}} \), we have \( n^2 \in 3 + 12{\mathbb{Z}}\subset 3 + 4{\mathbb{Z}} \) and this forces \( n^2 \equiv 3 \pmod 4 \).
:::
...
\]
be a complex (so \( d^2 = 0 \)) of smooth vector bundles on a smooth manifold \( X\operatorname{im}{\mathbf{Mfd}}_{\mathbb{R}}^{C^\infty} \). Suppose that the \( d_i \) are **differential operators**, i.e. in local trivializing charts over \( U \) we have
...
::: {.example title="?"}
For \( X\in {\mathbf{Mfd}}_{\mathbb{R}}^{C^ \infty } \), we can take
\[
...
::: {.example title="?"}
For \( X\in {\mathbf{Mfd}}_{\mathbb{C}} \) and \( \mathcal{E} \to X \) a holomorphic vector bundle, take
\[
...
A differential complex \( (\mathbf{\mathcal{E}}, d) \) is **elliptic** if the symbol complex \( (\pi^* \mathbf{\mathcal{E}}, \sigma(d)) \) is an exact sequence of sheaves (importantly) on \( T^\vee X \setminus\left\{{s_z}\right\} \) for \( s_z \) the zero section.
:::

::: {.claim}
\( (\mathbf{\Omega}, d) \) is elliptic. To check exactness of a sequence of vector bundles, it suffices to check exactness on every fiber. Fix \( (p, \alpha) \in T^\vee X \setminus\left\{{ s_z }\right\} \), then
\[
