...
{\tiny \textit{Last updated:} 2021-04-23 }
\end{flushleft}
...
describe \(\operatorname{Cl}({\mathbb{R}}^n, \cdot)\) in general? Choose
an orthonormal basis \(\left\{{ e_i }\right\}\), then
\begin{align*}
...
Define the \textbf{degree filtration} on \(\operatorname{Cl}(V, \cdot)\)
as the filtration induced by the degree filtration on
\(T(V) \coloneqq\bigoplus _{n\geq 0} V^{\otimes n}\).
...
\item
  ...
  simply define
  \(\operatorname{Cl}(V, \cdot)^i = \operatorname{im}T(V)^i\) since the
  relations have mixed degree: for example \(e_1^2 = -1\) So
  \(\operatorname{Cl}(V)\) isn't graded, but is still filtered: take the
  filtration \(F\) on \(T(V)\) defined by
...
\(\operatorname{Cl}(V)\) has a \({\mathbb{Z}}/2\) (``super'') grading,
so
\begin{align*}
