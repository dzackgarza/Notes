...
\item
  Grouert : Every \(C^ \infty\) structure gives a unique real-analytic
  structure.
\item
...
\item
  Kirby-Sibenmann: Let \(X\) be a topological manifold of
  \(\dim X\geq 5\), then there exists a cohomology class
  \(k_s(X) \in H^4(X; {\mathbb{Z}}/2{\mathbb{Z}})\) which is 0 if and
  only if \(X\) admits a PL structure. Moreover, if \(ks(X) = 0\), then
  (up to concordance) the set of PL structures is given by
  \(H^3(X; {\mathbb{Z}}/2{\mathbb{Z}})\).
\item
  Moise: Every topological manifold in \(\dim X\leq 3\) admits a unique
  smooth structure.
\item
  Smale (et al): in \(\dim X\geq 5\), the number of smooth structures on
  a topological manifold \(X\) is finite. In particular,
  \({\mathbb{R}}^n\) for \(n \neq 4\) has a unique smooth structure. So
  dimension 4 is interesting!
\item
  Toubes: \({\mathbb{R}}^4\) admits uncountably many non-diffeomorphic
  smooth structures.
\item
...
Let
\(V = \left\{{a^2 + b^2 + c^2 + d^3 + e^{6k-1} = 0}\right\} \subseteq {\mathbb{C}}^5\)
and
...
\(k=1,2,\cdots 28\) yields the 28 smooth structures on \(S^7\). Notice
...
invariant \(ks(x) \in H^4(X; {\mathbb{Z}}_2)\) for \(X\) a topological
manifold where \(ks(X) = 0 \iff X\) admits a PL structure, with the
caveat that \(\dim X \geq 5\). We can use this to cook up an invariant
of 4-manifolds.

\end{remark}

\begin{definition}[Kirby-Sibemann Invariant of a 4-manifold]
...
\(ks(X) \neq 0\) implies that \(X\) has no smooth structure, since
\(X \times{\mathbb{R}}\) doesn't. Note that it was not known if this
invariant was nonzero for a while!
...
\(X\cong Y\) are homeomorphic if and only if \(ks(X) = ks(Y)\) and
\(Q_X \cong Q_Y\) are isometric, i.e.~there exists an isometry
\begin{align*}
...
\(H^2(X; {\mathbb{Z}}) \xrightarrow{PD} H_2(X; {\mathbb{Z}})\), where we
can think of elements in the latter as closed surfaces \([\Sigma]\), and
\begin{align*}
...
.{]} Then the signature is \(a-b\), the dimension of the
positive-definite space minus the dimension of the negative-definite
space.
...
\textbf{smooth} 4-manifold. Then 16 divides \(\sig(X)\).
...
\(b=0\), then \(Q_X\) is diagonalizable and there exists an orthonomal
...
\(\left\{{e, Je}\right\}\) is a bsis of \(T_p X\).
...
\({\mathbb{R}}^2\setminus{x{\hbox{-}}\text{axis}}\) Let
\(\tilde {\mathbb{R}}^2\) be an exotic \({\mathbb{R}}^2\).

\begin{itemize}
\item
  Step 1: Choose a metric on \(\tilde {\mathbb{R}}^2\)
  \(g \coloneqq\sum f_I g_i\) with \(g_i\) metrics on coordinate charts
  \(U_i\) and \(f_i\) a partition of unity.
\item
  Step 2: Find an almost complex structure on \(\tilde {\mathbb{R}}^2\).
  Choosing an orientation of \(\tilde {\mathbb{R}}^2\), \(g\) defines a
  unique almost complex structure
  \(J_p e \coloneqq f\in T_p \tilde {\mathbb{R}}^2\) such that
  \(g(e, e) = g(f, f)\) and \((e, f)\) is an oriented basis of
  \(T_p \tilde {\mathbb{R}}^2\) and \(g(e, f) = 0\). This is because
  after choosing \(e\), there are two orthogonal vectors, but only one
  choice yields an \emph{oriented} basis.
\end{itemize}
