...
A differential complex \(({\mathcal{E}}^{\bullet}, d)\) is
\textbf{elliptic} if the symbol complex
\((\pi^* {\mathcal{E}}^{\bullet}, \sigma(d))\) is an exact sequence of
...
\(({\Omega}^{\bullet}, d)\) is elliptic. To check exactness of a
...
If \(( {\mathcal{E}}^{\bullet}, d)\) is an elliptic complex of smooth
vector bundles on a compact oriented
...
Here we define
\(\operatorname{ch}( {\mathcal{E}}^{\bullet} \coloneqq\sum_i (-1)^i \operatorname{ch}( \mathcal{E}^i )\).
...
\(\operatorname{ch}( {\Omega}^{\bullet}X \otimes{\mathbb{C}}) = \prod_i (1-e^{x_i}) (1 - e^{-x_i})\)
...
,\end{align*}
and
\(\operatorname{ch}({ \Omega }^{\bullet}) = \operatorname{ch}( {\mathcal{O}}) - \operatorname{ch}( \Omega^1 \otimes{\mathbb{C}}) + \operatorname{ch}(\Omega^2\otimes{\mathbb{C}})\).
...
\(({ \mathcal{E} }^{\bullet}, d)\) we had a symbol complex
\(( \pi^* {\mathcal{E}}^{\bullet}, \sigma(d) )\) where
\(\pi: T^\vee X\to X\) and
...
.\end{align*}
We say that \(( {\mathcal{E} }^{\bullet}, d )\) is \textbf{elliptic} if
the symbol complex is exact on \(T^\vee X \setminus\left\{{0}\right\}\)
...
\(( {\mathcal{E}}^{\bullet}, d)\) is elliptic if and only if
...
column vector, i.e.~given a section
\((\varphi_1, \varphi_2) \in H^0(U, {\mathcal{O}}_S(-C) \oplus {\mathcal{O}}_S(-D) )\)
we send it to \(\phi_1 \cdot s_D - \phi_2 \cdot s_C\). Why is this
exact? Considering the composition, we have
\begin{align*}
\phi \xrightarrow{p_1}
(\phi s_D, \phi s_C)
\xrightarrow{p_2}
