...
{\tiny \textit{Last updated:} 2021-03-04 }
\end{flushleft}
...
Let \(X \in {\mathbf{Mfd}}_{\mathbb{C}}\), we'll use the fact that
...
given \(X \in {\mathbf{Mfd}}_{\mathbb{C}}^n\) of complex, dimension
...
There is a unique complex \(X\in {\mathbf{Mfd}}_{\mathbb{C}}\)
...
biholomorphic. Let \(X \in {\mathbf{Mfd}}_{\mathbb{C}}^1\) with
...
i.e.~\(S\in {\mathbf{Mfd}}_{\mathbb{C}}^2\). An example might be
...
\(\mathcal{E}\). Note that \(S \in {\mathbf{Mfd}}_{\mathbb{R}}^4\) is
...
here yields \(1-6+1 = -4 = \chi_{{\mathbf{Top}}}(C)\). More generally,
we have
\begin{align*}
...
Recall that if \(S \in {\mathbf{Mfd}}_{\mathbb{C}}^2\) is closed and
compact (noting that \(S\in {\mathbf{Mfd}}_{\mathbb{R}}^4\)) and
...
,\end{align*}
where \(\chi_{\mathbf{Top}}\) is the Euler characteristic and is given
by
\begin{align*}
...
\(\chi_{\mathbf{Top}}({\mathbb{CP}}^n) = n+1\) and
\(\chi_{\mathbf{Top}}({\mathbb{CP}}^2) = 3\). Over \({\mathbb{CP}}^n\)
...
Given any \(X\in {\mathbf{Mfd}}_{\mathbb{C}}\), define
...
We know that \(\chi_{\mathbf{Top}}(S^4) = 2\). If \(S^4\) had a complex
...
We had
\(\chi({\mathcal{O}}) = {1\over 12} \qty{ K^2 + \chi_{\mathbf{Top}}} = {1\over 12}(3-n^2)\),
...
manifold \(X\operatorname{im}{\mathbf{Mfd}}_{\mathbb{R}}^{C^\infty}\).
...
For \(X\in {\mathbf{Mfd}}_{\mathbb{C}}\) and \(\mathcal{E} \to X\) a
...
A differential complex \((\mathbf{\mathcal{E}}, d)\) is
\textbf{elliptic} if the symbol complex
\((\pi^* \mathbf{\mathcal{E}}, \sigma(d))\) is an exact sequence of
sheaves (importantly) on \(T^\vee X \setminus\left\{{s_z}\right\}\) for
\(s_z\) the zero section.

\end{definition}

\begin{claim}

\((\mathbf{\Omega}, d)\) is elliptic. To check exactness of a sequence
of vector bundles, it suffices to check exactness on every fiber. Fix
