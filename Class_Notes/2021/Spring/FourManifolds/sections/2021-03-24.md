...
Then the universal cover admits a biholomorphism $\tilde C \cong \H \da \ts{ z\in \CC \st \Im(z) > 0 }$.
:::
...
:::{.remark}
We can write \( \Omega^1(C_\RR) \tensor_\RR \CC = A^{1, 0}(C) \oplus A^{0, 1}(C) \), and by lifting we can write this as \( \ts{ f(z, \bar z)\dz + g(z, \bar z) \dzbar \st z\in \H }  \) where $f,g$ are smooth functions.
But $\dz$ is *not* invariant under the map $z\mapsto {az+b \over cz+d}$, since \( \dz \mapsto {\dz \over (cz+d)^2 \).
In order to descend $f(z)$ to $C$, we need $f\qty{az +b \over cz + d} = (cz+d)^2 f(z)$ for all $[a,b; c,d] \in \Gamma$.
