...
h^0(\CP^n, \OO(d)) =
\)
will be the number of linearly independent degree $d$ polynomials in the variables $x_0, \cdots, x_n$, which is $\multichoose{n+1}{d} = \choose{n+d}{n}$, using the fact that monomials span this space.
:::
