...
:::{.proposition title="?"}
If $X$ is **Calabi-Yau**, so $K_X = \OO_X$ (i.e the canonical bundle is trivial), then the Hodge diamond has a $D^4$ symmetry, i.e. there is a rotation by $\pi/2$.
:::
...
By Serre duality, $H^p(X; \Omega_X^q)\dual \cong H^{n-p}(X; (\Omega_X^q)\dual \tensor K_X )$.
:::
