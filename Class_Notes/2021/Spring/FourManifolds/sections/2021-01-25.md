...
For $G=\ZZ/2\ZZ$, for any (possibly nonoriented) manifold $X$ there is an *orientation principal bundle* $P$ which is locally a set of orientations on $U$, i.e. $P\da \ts{(x, O) \st x\in X,\, O \text{ is an orientation of }T_p X}$.
Note that $P$ is an oriented manifold, $P\to X$ is a local isomorphism, and has a canonical orientation.?
This can also be written as $P = \Frame X / \GL_n^+(\RR)$.
