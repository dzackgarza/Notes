...
.\]
where $\pi:\Tdual X\to X$, and the symbol was defined by replacing $\dd{}{x_i}$ with a function$y_i: \Tdual X\to \RR$.
We can write
\[
...
We have a tautological form \( \alpha\in H^0(\Tdual X, \pi^* \Omega^1) \) where $(p, \alpha) \mapsto \alpha$, and 
...
.\]
where \( \ts{ \pm x_i } \) are the Chern roots of $\Tdual X$.
:::


:::{.proof title="?"}
Use the splitting principle to write $\Tdual X \tensor_\RR \CC = \bigoplus _{i=1}^n L_i \tensor L_i\inv$
