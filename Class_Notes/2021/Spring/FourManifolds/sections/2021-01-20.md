...
Let  = \RR$ and define the **skyscraper sheaf** at \in \RR$ as
\[
\RR_p(U) \da 
\begin{cases}
\RR & p\in U 
...
Taking the cokernel $\coker(x\cdot)$ as a presheaf, this assigns to $U$ the presheaf $\OO(U) / x\OO(U)$, yielding the skyscraper sheaf $\RR_0$.
