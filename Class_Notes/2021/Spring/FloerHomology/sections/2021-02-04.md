...
So we won't have a well-defined index, but we'll have something that can replace the *difference* of indices: the **Maslov index** $\mu(x)$, the expected dimension of \( \mathcal{M}(x, y)  \).
