...
The Thurston norm $\norm{a}$ can be computed from this data by considering a perturbed version of $\hat{HF}$, denoted $\underline{\hat{HF}}$, in the following way: taking the first Chern class $c_1(\mathfrak{s}) \in H^2(M)$, we have 

\[
\norm{ \alpha} = \bigoplus _{\underline{\hat{HF}}(M, \mathfrak{s}  ) \neq 0 }
\inner{ c_1(\mathfrak{s}  }{ \alpha }  
.\]
<++>
