...
Given a knot $K \subseteq M$ a 3-manifold (e.g. $M = S^3$), there is extra algebraic structure on $\hat{CF}(M)$: a filtration which yields a bigraded abelian group $\hat{HFK}(M, K)$ which is also a $\ZZ_2\dash$vector space.
