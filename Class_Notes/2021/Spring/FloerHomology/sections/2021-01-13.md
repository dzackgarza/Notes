...
These invariants can be used to compute the Thurston seminorm of a 3-manifold: for \( \alpha\in H_2(M)\), it can be represented as \( \alpha\in [S] \) for $S$ a closed surface whose fundamental class represents \( \alpha \) where $S = \Union_{i=1}^n S_i$ can be a union of closed embedded surfaces $S_i$.
Then
\[
