...
Given a knot $K \subseteq M$ a 3-manifold (e.g. $M = S^3$), there is a filtration on $\hat{CF}(M)$, which yields a bigraded group $\hat{HFK}(M, K)$ which is also a $\ZZ_2\dash$vector space.
