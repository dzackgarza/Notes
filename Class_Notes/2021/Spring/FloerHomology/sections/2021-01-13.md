...
\[
HF^\wait(M) = \bigoplus _{S\in \Spin^c(M)} HF^\wait(M, S) 
\] 
...
The Thurston norm $\norm{a}$ can be computed from this data by considering a perturbed version of $\hat{HF}$, denoted $\underline{\hat{HF}}$.
