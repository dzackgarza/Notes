...
:::{.fact title="Properties of Knot Floer Homology"}
\envlist
...
- $\hat{HFK}(K)$ detects the **Seifert genus** of a knot $g(K)$, defined as the smallest $g$ such that there exists an embedded surface $F$ of genus $g$ in $S^3$ that bounds $K$, so $\bd F = K$.


:::{.example title="The Unknot"}
The unknot bounds a disc, so its genus is zero:

![image_2021-01-18-23-29-23](figures/image_2021-01-18-23-29-23.png)
