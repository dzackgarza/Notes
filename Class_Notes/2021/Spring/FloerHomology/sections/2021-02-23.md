...
1. Suppose \( x = \ts{ x_1, \cdots, x_g } \)  and \( y = \ts{ y_1, \cdots, y_g } \) such that \( x_i \in \alpha_i \intersect \beta_i \) and \( y_i \in \alpha_i \intersectg \beta_{ \sigma\inv(i)} \) for some permutation \( \sigma\in S_g \).
  Then for any \( \varphi\in \pi_2(x, y) \),  i
:::
