...
\[
\CF( \Sigma, \alpha, \beta) = \bigoplus_{x \in \TT_{ \alpha} \intersect\TT_{ \beta} } \ZZ/2 \gens{ x } 

\\
\bd x = \sum_{y\in \TT_{ \alpha} \intersect \TT_{ \beta} } \sum _{ \varphi\in \pi_2(x, y) ?} \# \hat{ \mathcal{M}}(\varphi)  
.\]
...
\[
\tilde \bd x \da \sum_{y \in \TT_{ \alpha} \intersect\TT_{ \beta} } \sum_{ \varphi \in \pi_2(x, y), \mu( \varphi) = 0, n_z( \varphi) = 0} \# \hat{\mathcal{M}}(\varphi) y 
.\]
where $n_z$ denotes the coefficient of $\phi$ at $z$, i.e. the number of intersection points $\# (\im \varphi \intersect L_z)$.
