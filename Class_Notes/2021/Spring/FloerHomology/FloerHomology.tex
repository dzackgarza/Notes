...
Check that this equation can be written as \(J\, du = \du \circ i\)
where \(i\) is the standard complex structure on
...
\item
  ...
  \(p\in \Crit(f)\) (i.e.~\(f\) is \textbf{self-indexing}), and
\item
...
\item
  ...
  \((\Sigma, \alpha, \beta) \mapsto (\Sigma \mathop{\Large \#}T^2, \alpha \cup\ts{ \alpha_{g+1 } , \beta \cup\left\{{ \beta_{g+1} }\right\}\),
...
\(S_g \curvearrowright\Sigma^{\prod^g}\) where if \(\varphi\in S_g\) we
set
\(\varphi(x_1, \cdots, x_g) = x_{ \varphi(1)}, \cdots, x_{ \varphi(g) }\).
Then set \(\Sigma^{\prod^g} \coloneqq\Sigma^{\prod g} / S_g\). Why does
this yield a smooth manifold? Is this action free? The diagonal
\(D \subseteq \Sigma^{\prod g}\) consists of the points with at least 2
...
the quotient map
\(\Sigma^{\prod g} \xrightarrow{\pi} \operatorname{Sym}^g(\Sigma)\) is
holomorphic.
...
\({\mathbb{T}}_ \alpha \coloneqq\prod+{i=1}^g \alpha_i \subset \Sigma^{\prod g}\),
...
\({\mathbb{T}}_ \alpha \embeds \operatorname{Sym}^g( \Sigma)\).
Similarly,
\({\mathbb{T}}_ \beta\coloneqq\prod_{i=1}^g \beta_i \embeds \operatorname{Sym}^g( \Sigma)\).
...
submanifolds of \(\Sigma^{\prod g}\). If you restrict \(\pi\) to
\(\Sigma^{\prod g}\setminus D \xrightarrow{\pi} \operatorname{Sym}^g(\Sigma) \setminus\pi(D)\),
...
\(\pi ^{-1} \diagonal\) that yields a fiber bundle:
