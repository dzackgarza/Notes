...
Using fact (2), we have $\lieg^\perp \normal \lieg$ and thus $\lieg^\perp \subseteq \mathrm{rad}(\lieg)$, which is 0 since because $\lieg$ is semisimple. So either $\lieg^\perp = 0$ or $\kappa$ is nondegenerate.
...
So for all $x,y \in lieg$ and all $v\in V$, we have $[x,y]\actson v = x\acts(y\actson v) - y\actson(x\actson v)$. But both of the terms in parenthesis are in $U$, and all elements in $\lieg$ kill elements in $U$, so this is zero. So $[\lieg, \lieg] \actson V$ trivially.
