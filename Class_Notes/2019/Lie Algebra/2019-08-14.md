...
\wrapenv{\Begin{exercise}}
Look at notes for basis elements of $\sl(2, F)$,
$$
...
- The commutator (or derived) algebra $[\mathfrak g, \mathfrak g] = \theset{\sum_i [x_i, y_i] \suchthat x_i, y_i \in \mathfrak g}$.
  - Moreover, $[\mathfrak{gl}(n, F),\mathfrak{gl}(n, F) ] = \sl(n, F)$.
...
Suppose that $\ch F \neq 2$, then $\sl(2, F)$ is not simple.
\wrapenv{\End{theorem}}

\wrapenv{\Begin{proof}}

Recall that we have a basis of $\sl(2, F)$ given by $B = \theset{e, h, f}$ where

- $[e, f] = h$,
- $[h, e] = 2e$,
- $[h, f] = -2f$.

So think of $[h, e] = \ad_h$, so $h$ is an eigenvector of this map with eigenvalues $\theset{0, \pm 2}$. Since $\ch F \neq 2$, these are all distinct. Suppose $\sl(2, F)$ has a nontrivial ideal $I$; then pick $x = ae + bh + cf \in I$. Then $[e, x] = 0 - 2be + ch$, and $[e, [e,x]] = 0 - 0 + 2ce$. Again since $\ch F \neq 2$, then if $c\neq 0$ then $e\in I$. Now you can show that $h\in I$ and $f\in I$, but then $I = \sl(2, F)$, a contradiction. So $c=0$.
...
which implies that $I = \sl(2, F)$ and thus it is simple.
