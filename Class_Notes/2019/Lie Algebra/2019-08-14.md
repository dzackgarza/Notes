...
Last time:
We have $\lieg = \lieg \oplus \left( \bigoplus_{\alpha \in \Phi} \lieg_\alpha \)$ where $\kappa\mid_{\lieh}$ is nondegenerate.
...
Set $f_\alpha \in \lieg_{-\alpha}$ such that $\kappa(e_\alpha, f_\alpha) = \frac{2}{\kappa(t_\alpha, t_alpha)}$.
...
= \frac{2}{\kappa(t_\alpha, t_alpha)}t_\alpha \\
= h_\alpha
...
\[
\begin{align*}
[h_\alpha, e_\alpha] = \frac{2}{\kappa(t_\alpha, t_alpha)}[t_\alpha, e_\alpha] \\
= \frac{2}{\kappa(t_\alpha, t_alpha)} \alpha(t_\alpha e_\alpha \\
= 2 e_\alpha)
.\end{align*}
\]
