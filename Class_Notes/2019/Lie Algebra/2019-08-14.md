...
c. If $\Gamma_A$ contains a path $p_1 \to \cdots \to p_t$, then $A' \coloneqq \theset{p} \union A\setminus \theset{p_1, \cdots, p_t}$ where $p \coloneqq \sum p_i$. Moreover, $\Gamma_{A'}$ is obtained from $\Gamma_A$ by contracting this path.
