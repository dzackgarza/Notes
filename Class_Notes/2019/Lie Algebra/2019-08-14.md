...
A *reflection* in $E$ is an element $s\in \gl(E)$ such that
...
Suppose that $S \in \gl(E)$ satisfies
...
This $\tau \actson \RR\alpha$ by 1, and similarly $\tau \actons E/\RR\alpha$ by 1 by picking a representative in $H$.
...
$$
W = \generators{S_\alpha \mid \alpha \in \Phi} \subseteq \gl(E)
$$

Note that $W \injects \Sigma_{\abs \Phi}$, a permutation group of size $\abs \Phi$.

Lemma:
If $g \in \gl(E)$ and $g(\Phi) = \Phi$, then for all $\alpha, \beta \in \Phi$, we have
\[
\begin{align*}
...
- Show that $\ord(S_\alpha, S_\beta) = 2,3,4,6$ for types $A_1 \cross A_1, B_2, G_2$.
...
$W = S_{\alpha_1} \cdots S_{alpha_{t-1}} S_{\alpha_t+1}} \cdots S_{\alpha_{j-1}} S_{\alpha_j}$,
...
Recall from last time the Dynkin diagrams:Recall from last time:
...
\[
\begin{align*}
\Gamma = v_1 \to \circ \to \cdots \to v_p \rightrightarrows w_q \to \circ \to \cdots \to w_1
.\end{align*}
\]
...
Supposing wlog that $p \geq q$, we have either $p=q=2$, in which case we get $\circ \to \circ \rightrightarrows \circ \to \circ$.
Otherwise $q=1$, and we get $\circ \to \cdots \to \circ \rightrightarrows \circ$.
...
Thus 
$$
\inner v_i p = \begin{cases} \inner v_i p_j & \text{if $v_i$ is connected to $p_j$} \\ 0 & \text{else}
$$

We thus have $4{\inner v_i p}^2 = 4{\inner v_i p_j} \in \theset{0,1,2,3} \indicator{v_i \sim p_j}$.
...
Definition:
For $\lieg$ a lie algebra, define $U(\lieg) = T(\lieg)/J$ where $J = \generatosr{x\tensor y - y\tensor x - [x,y]} \normal T(\lieg)$.
...
$$
\{
\text{Lie algebra representations } \lieg \to \liegl(V)
\} \iff
\{
\text{Algebras from} U(\lieg) \to \mathrm{End}(V)
\}, \theta \mapsto \psi, \theta = \psi\circ\iota \mapsfrom \psi
$$
...
> Note: PBW is Poincaré-Birkhoff-Witt 
