...
f. For any nonzero $e_\alpha \in \lieg_\alpha$, there exists a unique $f_\alpha \in \lieg_{-\alpha}$ such that $[e_\alpha, f_\alpha] = h_\alpha \coloneqq \frac{\lambda}{\kappa(t_\alpha, t_\alpha) t_\alpha$.} Moreover, $\generators{e_\alpha, f_\alpha, h_\alpha} = \sl(2, \CC)$.
