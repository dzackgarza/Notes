...
$\lieso(n, \FF) = \theset{x\in \liegl(n, F) \suchthat \beta(x(u), v) = -\beta(u, x(v))}$, where $B$ has the matrix of a symplectic form if $n$ is odd, or this matrix with a 1 in the top-left corner if $n$ is odd.
