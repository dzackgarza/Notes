...
\begin{align*}
\phi: \lieg \to \liegl(V) \\ 
x \mapsto \phi(x) = \phi(x)_s + \phi(x)_n = \phi(x_n) + \)hi(x_s) \\
...
= (\lambda - 2i + 2) v_{i-2} + (\lambda + i - 2)(i-1_ v_{i-1}) \\
= i (\text{RHS})
.\end{align*}
\]
...
Definition:
A module $V$ is a *highest weight module* of weight $\lambda$ if $V = \lieg\actson v_0$ for some maximal vector $v_0$.
