...
Let $\lieg \subseteq \liegl(V)$ be a Lie subalgebra for some finite dimensional vector space $V$. If $x$ is nilpotent as an endomorphism on $V$, then there exists a nonzero vector $v\in V$ such that $\lieg v =0$, so $x\in \lieg \imples x(v) = 0$.
