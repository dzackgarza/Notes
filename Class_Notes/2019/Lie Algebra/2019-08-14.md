...
Overview:
The killing form is defined as $\kappa: \lieg^2 \to \FF$ where $\kappa(x, y) = \trace(\ad_x \circ \ad_y)$.

Then we have **Cartan's Criteria**:

- $\lie g$ solvable $\iff$ $\kapp(x,y) = 0 \forall x\in [\lieg, \lieg], y\in \lieg$.
- $\lie g$ semisimple $\iff \kappa$ is non-degenerate.
