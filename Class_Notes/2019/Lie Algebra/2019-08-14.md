...
Definition:
A subalgebra $\mathfrak{h} \leq \lieg$ is *toral* if $\mathfrak{h} \neq 0$ and it consists of only semisimple elements (i.e. $x_n = 0 \forall x\in \lieg$)

Lemma:

- There exists a toral subalgebra of $\lieg$, which is a nontrivial maximal toral subalgebra.
