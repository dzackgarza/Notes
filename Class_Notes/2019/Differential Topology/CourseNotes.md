...
Let $M = \RR$, and $X: M \to TM \cong \RR$ be defined by $X(p) = p^2$. Then if a curve $\gamma: \RR \to M$ is defined by $\dd{}{t} \gamma(t) = X(\gamma(t))$, we have $\dd{p}{t}(t) = p^2(t)$ which has solution $p(t) = 
