...
Let $M$ be a smooth manifold with tangent bundle $TM$ and a maximal smooth atlas $\mathcal{A}$. Choose a covering of $M$ by charts $\mathcal C = \theset{(U_i, \phi_i) \mid i\in I} \subseteq \mathcal A$ such that $M \subseteq \union_{i\in I} U_i$.Then choose a partition of unity $\theset{f_i}_{i\in I}$ subordinate to $\mathcal C$. 
...
$$
which is defined for pairs of vectors in $T \phi_i(U_i) \cong T \RR^n \cong \RR^n = \mathrm{span}_\RR \theset{\partial x_i}_{i=1}^n$, and is defined on basis vectors as the Kronecker delta and extended linearly.
