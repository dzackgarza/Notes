...
> Exercise: Show that $f$ is continuous at $x \iff \omega_f(x) = 0$.
...
Motivation: 
This comes from working backwards from 4-5 triangle inequalities that will appear later.
...
**Corollary:** 
$\RR\setminus\QQ$ is not an $F_\sigma$ set. Suppose it was, so $\RR\setminus \QQ = \union_{n\in\NN} F_n$ with $A_n$ closed. 
...
$\impliedby$ is the easy direction.
...
**Corollary:** 
This holds for $f: E \to \overline \RR$, not just positive functions.
...
> Remark: This $x$ is only unique if $\theset u_n$ is *complete*, i.e. $\inner{y}{u_n} = 0 \quad \forall n \implies y = 0$.
...
If $N$ is large enough, in particular $N \geq n$, then we have the identity 
$$
