...
Let $e_n(x) \coloneqq e^{2\pi i n x}$ for all $n\in \ZZ$ and $x\in [0,1]$.
...
$$
\hat{f}(n) = \int_0^1 f(x) e^{-2\pi i n x} ~dx \quad \forall n \in \ZZ
$$

> Note that this resembles $\inner{f}{e_n}$, although this is not an inner product space.

The *Fourier series* of $f$ is defined as $\sum_{n\in \ZZ} \hat{f}(n) e^{-2\pi i n x}$.

> Note that this isn't necessarily *equal* to $f$, this only makes sense if the partial sums are converging to $f$ in some sense.

Define the $N$th partial sum $S_nf(x) \coloneqq \sum_{\abs{n} \leq N} \hat{f}(n) e^{2\pi i n x}$.

> Remark: We have $L_2(\Pi) \subseteq L^1(\Pi)$, so the Fourier coefficients do make sense as an inner product for all $f\in L_2(\Pi)$.

Some consequences:

- By Riesz-Fischer, given any $\theset{a_n} \in \ell^2(\ZZ)$, there is a function $f\in L^2(\Pi)$ such that $\hat{f}(n) = a_n$ for all $n$.
- By Parseval, $\sum_{n\in \ZZ} \abs{\hat{f}(n)}^2 = \int_0^1 \abs{f(x)}^2~dx$
...
> Remark: note that for the Fourier transform, we have $f(x) = \int \hat{f} e^{-2\pi i x}~dx$ when $f\in L^1$ and $\hat{f} \in L^1$, and we can get analogous statements here.

Theorem:

If $f\in L^1(\Pi)$ and $\theset{\hat{f}(n)} \in \ell^1$, then $S_Nf \to f$ *uniformly* on $\Pi$.

Corollary (exercise):

1. If $f\in C^1(\Pi)$, then $S_N f \to f$ uniformly on $\Pi$.
2. If $f\in C(\Pi)$ (note: not enough, since there is a continuous function whose fourier series diverges at a point) *and* $f' \in L^2(\Pi)$, then $S_Nf \to f \to f$ uniformly on $\Pi$.

> So if $f\in C^1$, it is *equal* to its Fourier series. Everyone should know this fact!

A beautiful and fundamentally amazing fact, the Riemann-Localization Principle:
...
So $S_Nf \to g$ uniformly for some continuous $g$.

How can we argue $g = f$? We can consider
\[
\begin{align*}
\hat{g}(n) 
...
so the question is now whether $\hat{g}(n) = \hat{f}(n) ~\forall n \implies g = f$ almost everywhere.

> Note: if we accept this fact at face value, this proof only requires undergraduate analysis: facts about uniform convergence allowing sums to commute with integrals.

This will be true if $f \in L^2$. 
Why is this the case? 
It's not strict inclusion, since $L^2([0, 1]) \subseteq L^1([0, 1])$, *but* $\ell^1(\ZZ) \subseteq \ell^2(\ZZ)$.
...
- $y \mapsto L_y \coloneqq \inner{\wait}{y}$ is a conjugate linear isometry $H \to H\dual$ which is surjective.
