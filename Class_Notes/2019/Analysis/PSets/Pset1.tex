...
\begin{solution}\hfill
\begin{enumerate}
  \item To show the forward direction, suppose $X$ is a $F_\sigma$, so $X = \intersect_{i\in\NN} A_i$ with each $A_i$ an open set. By definition, each $A_i^c$ is closed, and we have
    $$
    X^c = \left( \intersect_{i\in \NN}A_i \right)^c = \union_{i\in\NN} A_i^c,
    $$
    which exhibits $X^c$ as a countable intersection of closed sets, making it an $F_\sigma$.

    The reverse direction proceeds analogously: supposing $X^c$ is $F_\sigma$, we can write $X^c = \union B_i$ with each $B_i$ closed, where $B_i^c$ is open by definition, and
    $$
    X = (X^c)^c = (\union B_i)^c
    $$
\end{enumerate}
