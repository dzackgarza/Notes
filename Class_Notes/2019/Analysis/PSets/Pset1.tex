...
\begin{solution}\hfill\\
  ...
  \item We first show that $C$ is nowhere dense by showing that the interior of its closure is empty, i.e. $(\overline C)^\circ = \emptyset$.
    ...
    As shown above, we know that $\mu(C_n) = \left( \frac 2 3 \right)^n$, and so $\mu([0,1] \setminus C^n) = 1-\left( \frac 2 3 \right)^n$. Using the additivity of measure over disjoint sets, we have
    \begin{align*}
      \mu(C) &= 1 - \mu(C^c) \\
