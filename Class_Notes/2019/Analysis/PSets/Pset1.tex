...
\[
  ...
  ,\end{align*}
\]
and note that $x_i < x_j$ forces $S(x_i) \subseteq S(x_j)$. In particular, it is worth noting that if $y\in S(x_i)$, for example, then $y = x_n$ for some $n$ where $f_n(x_i) = 1$ precisely because $x_i > x_n = y$.
...
\[
  \begin{align*}
    ...
    f(x_j) = \sum_{i=1}^\infty \frac 1 {n^2} \indic{x_j > x_n} = \sum_{x \in S(x_j)}\frac 1 {n^2}
  .\end{align*}
\]
  from which we claim
\[
  \begin{align*}
    S(x_i) \subseteq S(x_j) \implies \sum_{x \in S(x_i)}\frac 1 {n^2} ~\leq \sum_{x\in S(x_j)} \frac 1 {n^2}
  .\end{align*}
\]
