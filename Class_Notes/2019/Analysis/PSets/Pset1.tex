...
\begin{solution}\hfill\\
  \begin{enumerate}
    ...
    Claim: If $x\in C$, then one can take its ternary expansion and find that all of its digits are either $0$ or $2$, i.e.
    $$
    x = \sum_{k=1}^\infty a_k 3^{-k} \text{ where } a_k \in \theset{0, 2}.
    $$
    Proof: Towards a contradiction suppose that $x\in C$ and contains a $1$ in its ternary expansion, so $a_k =1$ for some $k$.
