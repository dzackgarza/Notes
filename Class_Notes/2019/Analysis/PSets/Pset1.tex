...
\]
  ...
  To see that $\omega_f(x) = 0 \implies f$ is continuous at $x$, let $\varepsilon > 0$ be arbitrary; we want to produce a $\delta$ to use in the definition of continuity. Since $\omega_f(x) = 0$, we can find a $\delta$ such that
  $$
  \sup_{y, z \in B_\delta(x)} \abs{f(y) - f(z)} < 2\varepsilon.
  $$
\end{enumerate}1
