...
\]
  ...
  The claim is that this is the $\delta$ we want. If we let $y\in B_{\delta}(x)$ be arbitrary, we can note that there always exists some $\delta_0 < \delta$ such that $B_{\delta_0}(y) \subset B_{\delta}(x)$. Since suprema can only get larger when taken over larger sets, we can write
  \[
  \begin{align*}
