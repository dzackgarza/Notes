...
\[
  \begin{align*}
    \sum_{x\in S(x_j)} \frac 1 {n^2} = \sum_{x\in S(x_i)} \frac 1 {n^2} + \sum_{x\in S(x_j) - S(x_i)} \frac 1 {n^2}
  ,\end{align*}
\]

  where the second term is nonzero precisely because $S(x_j)\setminus S(x_i)$ is nonempty -- indeed, it contains $x_j$ itself since $x_j > x_i$.
