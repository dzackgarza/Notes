...
\begin{solution}\hfill
  To see that $f$ is increasing, we need to show that $x_i \leq x_j \implies f(x_i) \leq f(x_j)$. We can note that both $x_i, x_j \in X$ implies that they occur in the enumeration, so let $i,j$ correspond to their indices in the enumeration.
\end{solution}
