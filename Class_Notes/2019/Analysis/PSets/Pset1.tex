...
\item Note that if $A_\varepsilon$ is closed, we can write $D_f = \union_{n=1}^\infty A_{\frac 1 n}$, which is a countable union of closed sets, making $D_f$ an $F_\sigma$ set.
  ...
  $$
  \abs{y - x} < \delta \implies \omega_f(y) < \varepsilon
  $$
  ...
  and in particular, if $y \in B_\delta(x)$, then $\abs{f(y) - f(x)} < \varepsilon$. But if $y\in B_\delta(x)$, then $x \in B_\delta(y)$, and so
