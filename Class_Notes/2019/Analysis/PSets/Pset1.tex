...
\begin{solution}\hfill
\begin{enumerate}
  \item To show the forward direction, suppose $X$ is a $G_\delta$, so $X = \intersect{i\in\NN} A_i$ with each $A_i$ an open set. By definition, each $A_i^c$ is closed, and we have
    $$
    X^c = \left( \intersect{i\in \NN}A_i \right)^c = \union{i\in\NN} A_i^c,
    $$
