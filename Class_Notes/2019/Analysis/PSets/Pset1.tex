...
\]
  ...
  Noting letting $y\in B_{\delta_0}$ be arbitrary, we can choose a new $\delta$ such that $B_\delta(y) \subset B_{\delta_0}(x)$. This means that if $a,b \in B_\delta(y)$, then certainly $a, b \in B_{\delta_0}(x)$, and we can write
  $$
  \sup_{a,b \in B_\delta(y)} \abs{f(a) - f(b)} \leq \sup_{s, t \in B_\delta(y)} \abs{f(s) - f(t)} \leq \sup_{s, t \in B_{\delta_0} (x)} < \varepsilon.
  $$
