...
\begin{enumerate}
  \item We need to show that
    $$
    \forall c, ~\forall\varepsilon > 0, ~\exists \delta \suchthat \quad \abs{x-c} \leq \delta \implies \abs{f(x)} \leq \varepsilon.
    $$

    To that end, let $\varepsilon$ be fixed and $c \in I$ be arbitrary. If $c\in I\setminus\QQ$, then $f(c) = 0 < \varepsilon$ and there's nothing to prove. Otherwise, $c\in\QQ$ and so $c = r_n$ for some $n$. Towards a contradiction, suppose there
\end{enumerate}
\end{solution}
