...
\begin{solution}\hfill
  To see that $f$ is increasing, we need to show that $x_i \leq x_j \implies f(x_i) \leq f(x_j)$. We can note that both $x_i, x_j \in X$, so let $i,j$ correspond to their indices in the enumeration. Note that we may or may not have $i < j$. We have
\[
  \begin{align*}
    f(x_i) = \sum_{k=j}^\infty \frac 1 {n^2}\\
    f(x_i) = \sum_{k=i}^\infty \frac 1 {n^2} \\
  .\end{align*}
\]
\end{solution}
