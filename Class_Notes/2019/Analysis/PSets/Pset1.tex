...
\]
  ...
  We now want to take the limit as $\delta \to 0^+$; if we have $\delta_i \leq \delta$ then $B_{\delta_i} \subseteq B_{\delta}$, so this can only make the left-hand-side of the above inequality smaller. We thus have so we have
  $$
  \sup_{y, z \in B_\delta_1(x)} \abs{f(y) - f(z)} .
  \sup_{y, z \in B_\delta_1(x)} \abs{f(y) - f(z)} < \varepsilon.
  $$
