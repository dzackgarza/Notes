...
\item Note that if $A_\varepsilon$ is closed, we can write $D_f = \union_{n=1}^\infty A_{\frac 1 n}$, which is a countable union of closed sets, making $D_f$ an $F_\sigma$ set.
  ...
  $$
  y\in B_\delta(x) \implies \abs{y - x} < \delta \implies \omega_f(y) < \varepsilon \implies \lim_{\delta\to 0^+} \sup_{a,b \in B_\delta(y)}\abs{f(a) - f(b)} < \varepsilon
  $$

  Since $\omega_f(x) < \varepsilon$, we can find a $\delta_0$ such that
  $$
  \sup_{y,z \in B_\delta(x)} \abs{f(y) - f(z)} < \varepsilon,

  $$

  and so we can further choose a $\delta'$ such that $B_{\delta'}(y) \subset B_{\delta}(x)$.

  and in particular, if $y \in B_\delta(x)$, then $\abs{f(y) - f(x)} < \varepsilon$. But if $y\in B_\delta(x)$, then $x \in B_\delta(y)$, and s
