...
\begin{enumerate}
  \item We need to show that
    ...
    $$
    \delta < \min\theset{\abs{c - s} \suchthat s\in S}$, so that $S \intersect B_\delta(c) = \emptyset$.

    This means that if $x\in \B_\delta(c) \intersect \QQ$, then $x = r_m$ where $\frac 1 m < \varepsilon$ by construction.
\end{enumerate}
\end{solution}
