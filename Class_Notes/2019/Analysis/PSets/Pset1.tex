...
\begin{enumerate}
  ...
  $$
  which equivalently stated says $\abs{f_n(x) - f(x)} < \varepsilon$ for all $x\in I$. So let $x$ be arbitrary but fixed, then the sequence $\theset{f_n(x)}_{n=1}^\infty$ is a sequence of real numbers, and since $\theset{f_n}$ is Cauchy, we have
  $$
  \abs{}
  $$
