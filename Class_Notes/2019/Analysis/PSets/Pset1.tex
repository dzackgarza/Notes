...
\]
  ...
  $$
  by definition and this limit exists, we can find some \textbf{particular} $\delta_0$ such that
  $$
  \sup_{s, t \in B_{\delta_0}(x)} \abs{f(s) - f(t)} < \varepsilon
  $$
  and so
  $$
  y, z \in B_{\delta_0}(x) \implies  \abs{f(y) - f(z)} \leq \sup_{s, t \in B_{\delta_0}(x)} \abs{f(s) - f(t)} < \varepsilon.
  $$

  Noting letting $y\in B_{\delta_0}$ be arbitrary, we can choose a new $\delta < \delta_0$ such that $B_\delta(y) \subset B_{\delta_0}(x)$. Since suprema can only get larger when taken over larger sets, we can write
  $$
  \sup_{a,b \in B_\delta(y)} \abs{f(a) - f(b)} \leq \sup_{s, t \in B_\delta(y)} \abs{f(s) - f(t)} \leq \sup_{s, t \in B_{\delta_0} (x)}\abs{f(s) - f(t)} < \varepsilon.
  $$
