...
\begin{enumerate}
  ...
  We can then say that if $y,z \in B_\delta(x)$, then $\abs{f(y) - f(z)} \leq \abs{f(y)} + \abs{f(z)} \leq 2M$, and thus the set $\theset{\abs{f(y) - f(z)} \suchthat y,z \in B_\delta(x)}$ is bounded above and thus has a least upper bound.
\end{enumerate}
\end{solution}
