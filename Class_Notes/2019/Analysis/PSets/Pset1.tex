...
\]
  ...
  Since
  $$
  \omega_f(x) < 0 \implies \lim_{\delta \to 0^+} \sup_{s,t \in B_\delta(x)} \abs{f_(s) - f(t)} < \varepsilon
  $$
  Since $\omega_f(x) < \varepsilon$, the limit in its definition exists, and we can find a $\delta$ such that
  $$
  \sup_{y,z \in B_{\delta}(x)} \abs{f(y) - f(z)} < \varepsilon,
  $$
