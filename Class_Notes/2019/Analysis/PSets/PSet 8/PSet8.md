...
This follows from the fact that if $-M \leq g(x)\leq M$ then we can define $s_n$ by partitioning $[-M, M]$ into $n$ subintervals with endpoints $\theset{x_k \mid 1\leq k \leq n}$ and then letting $I_k = f^{-1}([x_{k-1}, x_k])$ and $s_n = \sum_k x_k \chi_{I_k}$
