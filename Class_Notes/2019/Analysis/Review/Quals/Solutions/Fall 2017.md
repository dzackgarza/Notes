...
Let $s = \sum_{i=1}^M c_i \chi_{A_i}$ where each $A_i$ is measurable.
...
Chose let $I_N \definedas \disjoint_{j=1}^N I_j$, and choose $N_i$ large enough such that $m(O_i \setminus I_{N_i}) < \varepsilon$.
