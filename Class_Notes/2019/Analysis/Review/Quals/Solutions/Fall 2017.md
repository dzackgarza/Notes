...
**Lemma:** If $g$ is differentiable, then
$(f \ast g)' = f \ast g'$ almost everywhere.
...
&= \lim_{h\to 0} \int \frac{f(x + h - y) - f(x - y)}{h} g(y) \\
&\overset{DCT}=  \int \lim_{h\to 0} \frac{f(x + h - y) - f(x - y)}{h} g(y) \\
&= \int f'(x-y) g(y) \\
&= (f' \ast g)(x)
.\end{align*}
