...
and since $\phi'$ is continuous and compactly supported, $\phi'$ is uniformly bounded by some $M < \infty$ and thus
\begin{align*}
...
To see that $f\ast \phi$ is compactly supported, choose $x$ large enough so that $x\not\in \mathrm{supp}(\phi)$; then
\begin{align*}
(f\ast \phi)(x) = \int \phi(x-y) f(y)~dy
.\end{align*}
