...
To see that the convergence is not uniform, let $x_n = \frac 1 n$ and $\varepsilon = 1$

\begin{align*}
\sup_{x\in [0, 1]}\abs{nx(1-x)^n} \leq \abs{nx_n (1-x_n)^n} = \abs{\left( 1 - \frac 1 n\right)^n} \converges{n\to\infty}\to e > \varepsilon
.\end{align*}
