...
**Lemma:** 
$(f \ast g)' = f \ast g' = f' \ast g$ almost everywhere.
...
&= 2\pi i \xi ~\mathcal{F}(f\ast g) \\
&= 2\pi i \xi ~ \mathcal{F}(f) ~ \mathcal{F}(g) \\
&= \mathcal{F}(f) \cdot \left( 2\pi i \xi ~\mathcal{F}(g)\right) \\
...
\begin{align*}
(f\ast g)'(x)
&= \lim_{h\to 0} \frac{(f\ast g)'(x+h) - (f\ast g)'(x)}{h} \\
&= \lim_{h\to 0} \frac 1 h \int \frac{f(x + h - y) - f(x - y)}{h} g(y) \\
&\overset{DCT}=  \int \lim_{h\to 0} \frac 1 h \frac{f(x + h - y) - f(x - y)}{h} g(y) \\
.\end{align*}
 

 To see that the DCT is justified,
