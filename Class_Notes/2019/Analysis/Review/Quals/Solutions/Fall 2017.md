...
It suffices to show that $S$ is dense in the class of simple functions, and in fact by linearity/finiteness, it suffices to show this for $\chi_A$ for $A$ a measurable set.
...
So define $I_N \definedas \disjoint_{j=1}^N I_j$, and choose $N$ large enough such that $m(O \Delta I_{N, k}) < \varepsilon = \frac 1 k$.

Now define $h_k = \chi_{I_{N, k}}$, then
\begin{align*}
\norm{s - h_k}_1 = \int \abs{\chi_A - \chi_{I_{N, k}}} \leq m(A \Delta I_N) \converges{k\to\infty}\to 0
.\end{align*}

Now letting $s = \sum_{j=1}^N c_j \chi_{A_j}$, we can define an $h_{k, j}$ for each $A_j$, and 
