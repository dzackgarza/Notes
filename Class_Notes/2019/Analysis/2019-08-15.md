...
For any $M > 0$, we can find an $E_j$ such that $m(E_j) > M$. Since $E_j$ is bounded, by case 1 we get a compact $K \subseteq E_j \subeteq E$ with $m(K) > M$.
...
A subset $E\susbeteq \RR^n$ is measurable iff for all $A\subseteq \RR^n$,
$$
