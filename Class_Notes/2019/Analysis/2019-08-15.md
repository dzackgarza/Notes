...
Corollary: $\RR\setminus\QQ$ is not an $F_\sigma$ set. Suppose it was, so $\RR\setminus \QQ = \union_{n\in\NN} F_n$ with $A_n$ closed. Then $\RR = \union_n F_n \union \theset{q_i}$ where $\QQ = \union \theset{q_i}$. This exhibits $\RR$ as a countable union of closed sets.
