...
- Uniform: $f_n \uniformlyconverges f: \forall \varepsilon ~\exists N \suchthat ~n\geq N \implies \abs{f_N(x) - f(x)} < \varepsilon \quad \forall x.$
...
- $f_n:$ see weird example (insert image). Then $f_n \to 0$ in $L^1$ but is not 1,2, or 3.
...
Let $g_1 = f_{n_2}, g_k = f_{n__k+1} - f_{n_k}$ be the consecutive differences.
Then 
...
Theorem:
$L^1$ is a complete normed space, i.e. a Banach space, so every Cauchy sequence in $L^$ converges to a function in $L^1$.
...
From the previous proof, we know a subsequence in $L^1$ converges to some limit in $L^1$.
