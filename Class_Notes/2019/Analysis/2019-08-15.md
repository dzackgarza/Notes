...
\hfill
\wrapenv{Begin{$1}}
\item A (Cantor): $\RR$ is not countable.
\item B (Baire): $\RR$ is not meager. (Baire Category Theorem)
\item C (Borel): $\RR$ is not null.
\End{itemize}
...
\wrapenv{Begin{$1}}[A second proof of (A)]
