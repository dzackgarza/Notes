...
- Monotonicity: If $E_1 \subseteq E_2$ then $m_*(E_1) \leq m_*(E_2)$.
- Countable Subadditivity: If $E = \union_{i\in\NN} E_i$ for any countable union, then
$$
m_*(\union E_i) \leq \sum m_*(E_i)
$$

- If $E \subseteq \RR^n$ then $\forall \varepsilon > 0$ there exists an *open* set $G \supseteq E$ such that
$$
...
- If $E = \union_{j\in \NN}Q_j$ with the $Q_j$ *almost disjoint*, then $m_*(E) = \sum \abs{Q_j}$.
