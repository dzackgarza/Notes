...
The pointwise limit of continuous functions has a meager set of discontinuities. If $f$ is integrable, the set of discontinuities is null. If $f$ is monotone, they are countable. There is a continuous nowhere differentiable function: let $f(x) = \sum \frac{\norm{10^n x}}{10^n}$
