...
- If $\phi, \psi \in L^+$ are simple, then
  - If $c \geq 0$ then $\int c\phi$ is defined and equal to $c\int \phi$. (Easy to show)
- $\int (\psi + \phi) = \int \psi + \int \phi$.
- If $\psi \leq \phi$, then $\int \psi \leq \int \phi$.
- The map $A \mapsvia{\mu_A} \int_A \phi$ is a *measure*.
