...
Let $A_1 = E_1, A_2 = E_2 \setminus E_1, \cdots A_j = E_j \setminus E_{j-1}$. Then $\theset{A_j}$ are disjoint, and $E = \disjoint A_j$, so
\[
\begin{align*}
m(E) &= \sum_j m(A_j) \\
...
Let $A_i = E_j\setminus E_{j+1}$, so $\theset{A_i}$ are disjoint. Then (important!!) $E_1 = \union A_i \union E$, which are disjoint and measurable. Then,
\[
\begin{align*}
m(E_1) &= \sum \mu(A_i) + \mu(E) \\
&= \lim_{k\to\infty} \sum_{i=1}^{k-1} \mu(A_j) \\
&= \lim_{k\to\infty} \sum_{i=1}^{k-1} \mu(E_i) - \mu(E_{i+1}) \\
.\end{align*}
\]
