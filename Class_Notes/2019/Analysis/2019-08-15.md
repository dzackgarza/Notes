...
For any given $a$, we want to show that $S = \theset{x \suchthat \sup_n f_n(x) > a}$ is measurable. Then there is some $n$ for which $f_n(x) >  a$, and we claim that $S = \union_n \theset{x \suchthat f_n(x) > a}$. But this follows formally by just checking set inclusions.
