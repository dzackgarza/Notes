...
Definition: $E$ is a *null set* (or has *measure zero*) if $\forall \varepsilon >0$, there exists a sequence of intervals $\theset{I_j}_{j=1}^\infty$ such that $E \subseteq \union_{j=1}^\infty$ and $\sum \abs{I_j} < \varepsilon$.
