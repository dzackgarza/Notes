...
Note that $\abs f = f_+ + f_-$, so if $f:\overline \RR \to \RR$ is integrable, then $\int \abs f = \int (f_+ + f_-) = \int f_+ + \int f_-$, which both must be finite.
We now have two finite *numbers*, so we can subtract.

Definition:
For $f:\RR^n \to \overline \RR$, we define $\int f = \int f_+ - \int f_-$.
