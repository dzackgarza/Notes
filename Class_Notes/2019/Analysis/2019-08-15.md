...
Proof:
Let $G \subseteq \RR$ be open. Then $(\varphi \circ f)\inv (G) = f\inv(\varphi\inv(G)) \in \mathcal M$. But $\varphi\inv(G)$ is open since $\varphi$ is continuous, and thus measurable.
