...
But then $m(N) = 0 \implies 1 > m(N)$, and if $m(N) = \varepsilon$ then $\sum m(N) = \sum \varepsilon
 ...
 Any open set in $\RR$ can be written as a *countable* union of intervals (Exercise). But how do you go to closed sets, or to $\RR^n$? Any open set in $\RR^n$ can be written as an *almost disjoint* union of closed cubes.
