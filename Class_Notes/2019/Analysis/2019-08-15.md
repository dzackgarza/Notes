...
Now let $\QQ \intersect [-1, 1] = \theset{q_j}$, and define $N_j = N + q_j$. Note $j\neq k \implies N_j \intersect N_k = \emptyset$. By translation invariance, $m(N_j) = m(N)$, and $[0, 1) \subseteq \union_j N_j \subseteq [-1, 2]$. But then by taking measures $1 \leq sum_j m(N_j) \leq 3$.
