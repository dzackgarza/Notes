...
Define a translation $\tau_h(x) = x+h$, then $\tauf(x) = f(x-h)$ for all $h\in\RR\units$.
...
&= \int_{\RR} \int_{\RR^n} \chi_{S \coloneqq {x\in \RR^n \suchthat f(x) \geq y \geq 0} \\
...
> Best way to show measurability: use Borel characterization, or show that it's an $F_\sigma \disjoint N$ where $N$ is null.
