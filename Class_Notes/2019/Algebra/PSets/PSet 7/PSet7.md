...
where the $1$ occurs in the $i$th position. The claim is that $\mathcal{B}^{m} \coloneqq \theset{\vector e_i \vector \vector \beta_k \mid 1 \leq i \leq n,~~1\leq k \leq m}$ forms a basis for $V^{\oplus m}$.
