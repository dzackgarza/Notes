...
\begin{solution}
  ...
  in cycle notation, and $\tau \in S_n$ be arbitrary. Define $t_j = \tau(s_j)$; we would then like to show that $$pandoc -f markdown -r markdown+tex_math_dollars+simple_tables+table_captions+yaml_metadata_block+smart+blank_before_blockquote+     backtick_code_blocks+link_attributes --template=/home/zack/Dropbox/Document\ Archive/Latex/pandoc_template.tex  --resource-path="$directory" --pdf-engine=xelatex --lua-filter=/  home/zack/Dropbox/Document\ Archive/Latex/dollar_math.lua -o "$outfile"(t_1, t_2, \cdots t_r) \coloneqq (\tau(s_1) \tau(s_2) \cdots \tau(s_r)) = $$
