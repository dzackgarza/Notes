...
$M$ is cyclic as an $R\dash$module and generated by the basis vector $[1, 0]^2 \in \CC^2$, since 
\[
\begin{align*}
(tA + s)
\left[\begin{array}{cc}
...
It is then easy to see that $\CC^2$ is torsion as a module over $\CC[x]$, since by Cayley-Hamilton we have $\mathrm{Ann}(A) = (\mathrm{minpoly}(A)) = (x^2-2x)$ 
