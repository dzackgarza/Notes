...
*Corollary:* 
Let $\gamma \in \mathcal{C}$ be constructible. Then there exist $\theset{\alpha_i}_{i=1}^n$ such that $\gamma = \prod \alpha_i$, $[\QQ(\alpha_1, \cdots, \alpha_j): \QQ(\alpha_1, \cdots, \alpha_{j-1})] = 2$, and $[\QQ(\alpha): \QQ] = 2^d$ for some $d$.
