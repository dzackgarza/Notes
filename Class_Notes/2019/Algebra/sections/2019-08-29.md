# Thursday August 29th (Lecture 5)
...
Recall that for any group $A$, we can consider the free group on the elements of $A$, $F[A]$. 
...
Assume all elements have order 1,2. Let $a,b\in G$, consider $(ab)^2 = abab \implies ab=b\inv a\inv = ba$, so $G$ is abelian. 
...
So $b^2 = 1, a^2$ (these are both valid options!)
