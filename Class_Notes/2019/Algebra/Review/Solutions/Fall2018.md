...
Let $\phi$ denote the map in question, it suffices to show that $\phi$ is $R\dash$linear, i.e. $\phi(r\vector x + \vector y) = r\phi(\vector x) + \phi(\vector y)$.

We have
\begin{align*}
\phi(r\vector x + \vector y) = 
.\end{align*}
