...
Since the characteristic of a ring must divide its size, we have $\char R = 2$ or $4$.

Using the hint, we see that only $\ZZ/(4)$ has characteristic 4, but this ring only has one zero divisor.

So $R$ must have characteristic 2.

However, we can write $\ZZ/(2)[t]/(t^2) = \theset{a + bt \suchthat a,b\in \ZZ/(2)}$, and checking the multiplication table we have
...
and so we find that the only nonzero zero divisor is $t$.