...
$$
\abs{\ker \phi_i} \leq n < \infty \quad \text{for each } i
.$$
...
Every element in this ideal is a zero divisor by (b), so 
$$
\abs{R/K_i} \leq n < \infty
.$$
...
Moreover, 
$$
\abs{R} = [R:K_i]\abs{K_i} \leq (n)(n) = n^2
.$$
