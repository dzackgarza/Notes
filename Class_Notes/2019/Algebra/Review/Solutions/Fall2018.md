...
By (b), $K_i$ consists of zero divisors, so $\abs{\ker \phi} \leq n < \infty$ for each $i$.

Now consider $R/K_i \definedas \theset{r + K_i}$.
By the first isomorphism theorem, $R/K_i \cong \im \phi = \generators{z_i}$, the ideal generated by $z_i$.
Every element in this ideal is a zero divisor by (b), so there are finitely many cosets.

But then $\abs{K_i}$ is finite and $\abs{R/K_i} = [R: K_i]$ is finite as well, so $R$ itself must be finite.
