...
For any vector $\vector w$, since $\theset{A^j \vector v}$ spans $V$ we have 
\begin{align*}
B\vector w = \sum_{j=0}^{n-1}c_j A^j \vector v
.\end{align*}

So let $p(x) = \sum_{j=0}^{n-1}c_jx^j$.
Then consider how $B$ acts on any basis vector $A^j \vector v$.

We have 
\begin{align*}
BA^j \vector v = A^j B\vector v
.\end{align*}
