...
We also have a natural quotient map $\rho: \FF_p[x] \to K$. 
...
\rho(q(x)) = q(\rho(x)) = \rho(x)^{p^d} - \rho(x) = 0 \in K
\implies q(x) \in \ker \rho
$$

Thus $x^{p^d}-x \in \generators{\pi(x)}$, i.e. this polynomial is in the ideal generated by $\pi(x)$, and is thus some multiple of $\pi(x)$ and so by definition we have $\pi(x) \divides x^{p^d}-x$. 
