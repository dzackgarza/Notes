...
We also have a natural quotient map $f: \FF_p \to K$. Since $K$ is also a group of size $p^d$, we know that for any $y \in K_d$, we have by Lagrange's theorem that the order of $y$ divides $\abs{K_d} = p^d$, and so $y^{p^d} = y$.
...
and thus the polynomial $x^{p^d}-x \in \ker f$, since it is identically zero in the quotient. Thus $x^{p^d}-x \in \generators{\pi(x)}$, i.e. this polynomial is in the ideal generated by $\pi(x)$, and is thus some multiple of $\pi(x)$ and so by definition we have $\pi(x) \divides x^{p^d}-x$. 
