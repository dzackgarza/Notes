...
Proof: 
We'll use the first theorem. First make a map
$$
G/K \to G/H \\
\phi(gk) = gH
$$
...
**Theorem:** 
$S_{A}$ is in fact a group. Check associativity, inverses, identity, etc.
...
Then $V$ becomes a $k[x]\dash$module by defining 
$$
...
Let $v\in V$, and $\generators{v} = \theset{\phi^i(v) \suchthat i = 0,1,2,\cdots}$, the *cyclic submodule generated by $v$*, and we write $\generators{v} = k[x].v$.
