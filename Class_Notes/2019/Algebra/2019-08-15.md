...
Suppose that $a,b \neq 0$ with $ab = 0$. Then $a,b$ are said to be *zero divisors*. A ring without zero divisors is an *integral domain*.

In $\ZZ_n$, an element $a$ )s a zero divisor iff $\gcd(a, n)$.
