...
Definition:
Let $\Phi_n(x) = \prod_{i=1}^{\varphi(n)} (x-\alpha_i)$, where this product runs over all of the primitive $n$th roots of unity.o
...
$G_m = \theset{[m] \suchthat \gcd(m, n) = 1}$ and $\abs G = \phi(n)$.

> Note that as a *set*, these are the units in $\ZZ_n$, but we are only keeping the additive structure.
