...
$$
E_I = \theset{\alpha \in E \suchthat \sigma(a) = a \forall a \in I}
$$
...
If $E$ is separable, we have $\abs{\mathrm{Gal}(E/F)} = \left{ E: F \right} [E: F]$.

Fact:
$\left{F(\alpha): F \right}$ is equal to number of *distinct* zeros of $\mathrm{min}(\alpha, F)$. 
...
Then $[F: F(\alpha_1, \cdots, \alpha_{n-1})] = n_j v_j = v_j }theset{F: F(\alpha_1, \cdots, \alpha_{n-1})}$.
...
Lemma: 
$f(x) \in F[x]$ has a multiple root iff $f(x), f'(x)$ have a nontrivial (multiple) factor.
...
> Big theorem: Any field of characteristic zero is perfect.
