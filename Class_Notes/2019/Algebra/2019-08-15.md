...
Exercise: for a field $F$, $\char F = 0, p$ where $p$ is a prime.

Note that if $\char F = 0$, then $\ZZ \in F$ since $1, 1+1, 1+1+1, \cdots$ are all in $F$.
Since inverses must also exist in $F$, we must have $\QQ \in F$ as well.
So $\char F = 0 \iff F$ is infinite.

If $\char F = p$, $\ZZ_p \subset F$.
...
Corollary:
Let $E$ be a finite field where $\char E = p$. Then $\abs E = p^n$ for some $n$.
...
Let $f(x) \in \FF[x]$ by a polynomial with a multiple zero $\alpha \in E$ for some $E \geq \FF$.
