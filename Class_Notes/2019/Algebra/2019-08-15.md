...
1. Since $\alpha$ is algebraic, $f(\alpha) = 0$. So write $f$ in terms of its irreducible factors, so $f(x) = \prod p_j(x)$ with each $p_j$ irreducible; then $p_i(\alpha) = 0$ for some $\i$ because we are in a field and thus don't have zero divisors.
