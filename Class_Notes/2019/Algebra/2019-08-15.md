...
An element $\alpha \in E$ with $F \to E$ is *algebraic* over $F$ iff there is a nonzero polynomial in $f \in F[x]$ such that $f(\alpha) = 0$. Otherwise, $\alpha$ is said to be *transcendendtal*.

Examples:

- $\sqrt 2 \in \RR \from \QQ$
