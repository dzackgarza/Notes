...
> Note that adjoining any root of a minimal polynomial will yield isomorphic (not usually not identical) fields. These are distinguished as subfields of (say) the algebraic closure of the base field.
...
We have $[F(\alpha):F] = [F(\alpha): F(\beta)][F(\beta): F]$, so $[F(\beta) : F]$ is less than $n$ since this is a finite extension, and the division of degrees falls out immediately.
