...
- For a finite index set $I$, it is the case that $\prod_I G = \bigoplus_S G$ in $\mathbf{Grp}$, i.e. that direct product and direct sum coincide. Otherwise, if $S$ is infinite the direct sum requires cofinitely many zero entries -- so here we always use $\prod$.
