...
- For a finite group $S$, it is the case that $\prod_S G = \bigoplus_S G$ in $\mathbf{Grp}$, i.e. that direct product and direct sum coincide. and otherwise the direct sum requires cofinitely many zero entries -- so here we always use $\prod$.
