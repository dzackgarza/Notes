...
- To build a Moore space $M(n, \ZZ_p)$, take $X = S^n$ and attach $e^{n+1}$ via a map $\Phi: S^n = \del B^{n+1}\to X^{(n)} = S^n$ of degree $p$.
	...
	- Can also use M.V. $H_{n+1}(\Sigma X) = H_n(X)$ and just suspend spaces with known homology.

## Theorem: Van Kampen's Theorem

Claim: If $X = U \union V$, then $\pi_1(X) = \pi_1(U) \bigast_{\pi_1(U \intersect V)} \pi_1(V)$.
