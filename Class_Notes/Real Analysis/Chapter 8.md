...
In general, one may wonder just how large the error $R_n(x)$ is in some interval, how fast it goes to zero as $n$ is increased, or how many terms one has to take so the the error is sufficiently small.
