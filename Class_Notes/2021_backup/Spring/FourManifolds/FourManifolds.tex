...
{\tiny \textit{Last updated:} 2021-04-26 }
\end{flushleft}
...
For \(M\) a Kahler manifold, the odd Betti numbers
...
divisor
\(D = \sum n_i D_i \in {\mathbb{Z}}[\operatorname{SubMfds}(\operatorname{codim}_1)]\).
...
\(\operatorname{Res}_C\)? We claim it is \(\Omega2_S\), which follows
...
\(\operatorname{Bl}_p(U) { \operatorname{cl}} (\phi(U^*))\) to be the
...
There is a map \(\operatorname{Bl}_p(U) \to U\) given by projection onto
the first coordinate which is the identity on \(U^*\).
...
We claim that
\(\pi_U^{-1}(0, 0) \subset \operatorname{Bl}_p(U) = \left\{{ p }\right\} \times{\mathbb{CP}}^1\),
...
\(\operatorname{Bl}_{p}{\mathbb{CP}}^2\) for \(p=[0:0:1]\), and consider
the strict transforms of the lines \(L\) to obtain
\(\widehat{L} \subset \operatorname{Bl}_p {\mathbb{CP}}^2\). Any two are
disjoint since they pass through different slopes of the exceptional
...
on \(\operatorname{Bl}{\mathbb{CP}}^2\).
...
\(\operatorname{Tot}{\mathcal{O}}(1)\) is diffeomorphic to the
complement of a ball in \({\mathbb{CP}}^2\), which ends up precisely
being taking a connect-sum. So we obtain
\(\operatorname{Bl}_{0} {\mathbb{C}}^2 \cong {\mathbb{C}}^2 \mathop{ \Large\scalebox{0.8}{\raisebox{0.4ex}{\#}}}\mkern 1.5mu\overline{\mkern-1.5mu{\mathbb{CP}}^2\mkern-1.5mu}\mkern 1.5mu\).
...
Then \(\operatorname{Bl}_0 {\mathbb{C}}^2\) is the result of collapsing
\(S^3\) along an \(S^1{\hbox{-}}\)foliation
...
Last time: we defined the blowup \(\operatorname{Bl}_0{\mathbb{C}}^2\)
as the closure of
\begin{align*} 
...
we have the blowup morphism \(\pi: \operatorname{Bl}_pS\to S\) and we
...
consider \(\operatorname{Bl}_0 {\mathbb{C}}^2\). Then
\begin{align*}
...
relate \(K_{\operatorname{Bl}_p S}\) to \(K_S\)?
...
We'll abbreviate \(\widehat{S} \coloneqq\operatorname{Bl}_p(S)\). Let
...
,\end{align*}
where \(\operatorname{Bl}_0 {\mathbb{C}}^2 \to C\). Then
...
conclude \(\widehat{C} = \operatorname{Bl}_{[0:0:1]} {\mathbb{CP}}^2\)
has self-intersection number \(\widehat{C}^2 = 9-2^2 = 5\).
...
such that
\(S = \operatorname{Bl}_p \mkern 1.5mu\overline{\mkern-1.5muS\mkern-1.5mu}\mkern 1.5mu\)
...
points, and let
\(\operatorname{Bl}_{p, q} {\mathbb{CP}}^2 \coloneqq\operatorname{Bl}_p \operatorname{Bl}_q {\mathbb{CP}}^2\).
...
.\end{align*}
Under
\(\pi: \operatorname{Bl}_{p, q} {\mathbb{CP}}^2 \to {\mathbb{CP}}^2\),
...
follows because
\(\mkern 1.5mu\overline{\mkern-1.5muS\mkern-1.5mu}\mkern 1.5mu \in \operatorname{ZHS}^4\),
and Noether's formula implies that every \(\operatorname{ZHS}^4\) has no
complex structure.
...
Let
\(\mkern 1.5mu\overline{\mkern-1.5mu\ell\mkern-1.5mu}\mkern 1.5mu \subset \operatorname{Bl}_{p, q}({\mathbb{CP}}^2)\)
...
\(r\in \operatorname{Bl}_{p, q}{\mathbb{CP}}^2\), then there are three
possibilities:
...
.\end{align*}
Question from class: what's the motivation for studying the Clifford
algebra in the first place? Rough answer, they give a nice way of
studying orthogonal transformations in \({\mathbb{R}}^n\).

\end{remark}

\begin{definition}[Pin]

The group \({\operatorname{Pin}}(V)\) is the subgroup of units of
\({ \operatorname{Cl}} (V)\), which is generated by
\(\left\{{ v\in V {~\mathrel{\Big|}~}{\left\lVert {v} \right\rVert} = 1 }\right\}\),
so
\begin{align*}
...
,\end{align*}
so we can identify \(V\) with \(V^{\otimes 1}\) in the degree 1 piece of
\({ \operatorname{Cl}} (V)\). In fact,
\(V^{\otimes 1} \subseteq { \operatorname{Cl}} _1(V)\), the odd part,
since writing
\({ \operatorname{Cl}} (V) = \bigoplus_{i\geq 0} V^{\otimes i}/\cdots\),
anything falling in an odd degree is in the odd part. If
\({\left\lVert {v} \right\rVert} = 1\), we have \(v^2 =-1\) and thus
\(v(-v) = 1\), so \(v\in { \operatorname{Cl}} (V)^{\times}\).

\end{remark}

\begin{question}

If \(v\in V\) with \({\left\lVert {v} \right\rVert} = 1\), is it true
that \(v\in {\operatorname{Spin}}(V)\)?

\end{question}

\begin{answer}

No, since \(V \subseteq { \operatorname{Cl}} (V)\) is in fact in
\({ \operatorname{Cl}} _1(V)\).

\end{answer}

\begin{example}[?]

What do elements of \({\operatorname{Spin}}({\mathbb{R}}^2)\) look like?
We can write
\begin{align*}
{ \operatorname{Cl}} ({\mathbb{R}}^2) = {\mathbb{R}}\oplus {\mathbb{R}}e_1 \oplus {\mathbb{R}}e_2 \oplus {\mathbb{R}}e_1 e_2
,\end{align*}
and note that \(e_1, e_2 \in {\operatorname{Pin}}(V)\) if we take the
\(e_i\) to be orthonormal. But similarly,
\(\cos(\theta) e_1 + \sin(\theta) e_2 \in {\operatorname{Pin}}(V)\) for
any \(\theta \in {\mathbb{R}}/2\pi {\mathbb{Z}}\). Also note that
\(e_1e_1 \in {\operatorname{Pin}}(V) \cap{ \operatorname{Cl}} _0(V)\),
and \(e_1 e_1 = -1 \in {\operatorname{Spin}}(V)\). What are some other
elements? We can compute
\begin{align*}
...
\({\operatorname{Spin}}(n) \coloneqq{\operatorname{Spin}}({\mathbb{R}}^n)\).
...
We saw that
\({ \operatorname{Cl}} _0({\mathbb{R}}^3) \cong { \operatorname{Cl}} ({\mathbb{R}}^2) \cong {\mathbb{H}}\),
and so
\({\operatorname{Spin}}(3) \subseteq {\mathbb{H}}\cong { \operatorname{Cl}} _0({\mathbb{R}}^3)\)
is the subset of the group of units. We have
\begin{align*}
...
,\end{align*}
and \({\operatorname{Spin}}(3)\) is generated by all products \(vw\)
where \(v, w\in {\mathbb{R}}^3\) with
\({\left\lVert {v} \right\rVert} = {\left\lVert {w} \right\rVert} = 1\).
So vary over all \(w\) in the unit circle in
\({\mathbb{R}}e_1 \oplus {\mathbb{R}}x\) for \(x\in {\mathbb{R}}^3\),
then the product \(vw\) yields a 1-parameter subgroup of
\({\mathbb{H}}^{\times}\). First consider
\begin{align*}
...
.\end{align*}
Then \({\operatorname{Spin}}(3)\) contains any 1-parameter subgroup of
\({\mathbb{H}}^{\times}\), which means
\(S^1({\mathbb{H}}) \subseteq {\operatorname{Spin}}(3)\) (where
\(S^1({\mathbb{H}})\) are the unit norm elements). By changing basis, we
may assume \(v= e_1\), so this gives an equality.
...
.\end{align*}
Fix a copy \({\mathbb{R}}^3 \subseteq {\mathbb{R}}^4\) (similar to how
we fixed \({\mathbb{R}}^2 \subseteq {\mathbb{R}}^3\) above) to get an
identification
\({ \operatorname{Cl}} _0({\mathbb{R}}^4) \cong { \operatorname{Cl}} ({\mathbb{R}}^3)\).
...
.\end{align*}
The copy of \({\mathbb{R}}^3\) induces inclusions
\({\operatorname{Spin}}(3) \hookrightarrow{\operatorname{Spin}}(4)\),
coming from
\(S^1({\mathbb{H}}) \hookrightarrow{\mathbb{H}}\hookrightarrow{\mathbb{H}}^{\oplus 2}\).
One needs to check that
\begin{align*}
S^3 = {\operatorname{SU}}(2) = S^1({\mathbb{H}}^{\oplus 2})
,\end{align*}
and that these elements generate
\((S^3)^{\times 2} = ({\operatorname{SU}}(2))^{\times 2}\).
...
\({\operatorname{Spin}}({\mathbb{R}}^n) \to {\operatorname{SO}}({\mathbb{R}}^n)\)
with kernel \({\mathbb{Z}}/2\). If \(n\geq 3\), then
\({\operatorname{Spin}}({\mathbb{R}}^n) \cong \overline{{\operatorname{SO}}({\mathbb{R}}^n)}\)
...
Note that
\({\operatorname{Spin}}(V) \subseteq { \operatorname{Cl}} (V)^{\times}\)
acts on \({ \operatorname{Cl}} (V)\) by conjugation:
\begin{align*}
{\operatorname{Spin}}(V) &\mapsto \mathop{\mathrm{End}}_{{\mathsf{Vect}}_{/{\mathbb{R}}}} { \operatorname{Cl}} (V) \\
g&\mapsto ( x \mapsto gxg^{-1})
.\end{align*}
Looking at \({\operatorname{Pin}}(V)\) is perhaps easier, since we have
generators
\(\left\{{ u {~\mathrel{\Big|}~}{\left\lVert {u} \right\rVert} = 1 }\right\}\).
Noting that \(u^{-1}= -u\) by a previous computation, we can write
\(uxu^{-1}= -uxu\), and we also have \(xu+ux = -2u\cdot x\), and so
\begin{align*}
-uxu 
&= -u( -ux -2u\cdot x ) \\
&=-x + 2u(x\cdot u)
,\end{align*}
so note that if \(x\in V \subset { \operatorname{Cl}} _1(V)\), then the
result \(-uxu \in V \subset { \operatorname{Cl}} _1(V)\) as well.

\begin{corollary}[?]

The conjugation action by \({\operatorname{Pin}}(V)\) preserves
\(V = V^{\otimes 1} \subset {\operatorname{Spin}}(V)\). Question: what
does the following map do geometrically?
\begin{align*}
V &\to V \\
x &\mapsto -x + 2u(x\cdot u)
.\end{align*}
Writing this \(x\mapsto -R_u x\), here \(R_u\) is reflection in the
plane orthogonal to \(u\). So there is a map
\begin{align*}
{\operatorname{Pin}}(V) &\to O(V) \\
u &\mapsto -R_u 
,\end{align*}
and on \({\operatorname{Spin}}(V)\), any two reflections preserve
orientation, so
\begin{align*}
u_1 u_2 \mapsto (-R_{u_1})(-R_{u_2}) = R_{u_1} R_{u_2} \in {\operatorname{SO}}(V)
,\end{align*}
and this yields a group homomorphism
\({\operatorname{Spin}}({\mathbb{R}}^n) \to {\operatorname{SO}}({\mathbb{R}}^n)\).
...
Every element of \({\operatorname{SO}}(V)\) is an even product of
reflections, so
\({\operatorname{Spin}}(V) \twoheadrightarrow{\operatorname{SO}}(V)\).

\end{remark}

\begin{exercise}[?]

As an exercise, check that \({\mathcal{O}}({\mathbb{R}}^n)\) can be
...
What is the kernel of this map? If for \(g\in {\operatorname{Spin}}(V)\)
we have \(gxg^{-1}= x\) for all
\(x\in V \subset { \operatorname{Cl}} (V)\), then
\(g\in Z { \operatorname{Cl}} (V)\) is in the center. Moreover, one can
check that for \(n\) odd,
\(Z { \operatorname{Cl}} (V) = \pm\left\{{ 1, e_1, \cdots, e_n }\right\}\),
and for \(n\) even
\(Z{ \operatorname{Cl}} (V) = \pm\left\{{ 1 }\right\}\). In either case,
\begin{align*}
