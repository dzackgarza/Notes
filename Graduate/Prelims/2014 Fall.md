...
$$
    ...
    1. We want to compute $\min\theset{\inner{A\vector x}{\vector x}\suchthat \vector x \in S^2}$. I don't know how to do this from the given information, but this is equivalent to the operator norm $\norm{A}_{\mathrm{op}} = \max\theset{\sqrt \lambda \suchthat \lambda \in \spec A}$. So in this case, we get $\sqrt 6$. $\qed$
...
$$
        ...
        which can be approached with Lagrange multipliers, i.e. looking at where $\nabla f = \lambda \nabla g$. This forces the conclusion that either $y_1 =0$ or $y_2=0$, yielding 4 combinations for $\vector y$ that can exhaustively tested, where it turns out that the minimum inner product is equal to 1.
