...
1. Let $\vector w_i$ be the proposed new basis elements -- then $\theset{\vector w_i}$ will be a basis if it is linearly independent and spans $\RR^3$.
  we can write down a linear map that sends $\vector w_i$ to $\vector v_i$ which is given by the matrix 
  $$
