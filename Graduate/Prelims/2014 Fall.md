...
1. Let $\vector w_i$ be the proposed new basis elements -- then $\theset{\vector w_i}$ will be a basis if it is linearly independent and spans $\RR^3$. Since $\theset{\vector v_i}$ is known to be a basis and thus spans $\RR^3$, we know we can write any $\vector x$ as a linear combination of $\vector v_i$, and so if we can also express $\vector x$ as a linear combination of $\vector w_i$, then the $W\dash$basis will span $\RR^3$ as well.
   
  With the information given, we can only go in the other direction: writing a vector in the $W\dash$basis in terms of the $V\dash$ basis:
  $$
  \vector x = c_1 \vector w_1 + c_2 \vector w_2 + c_3 \vector w_3 \\ 
  \implies \vector x = c_1 (\vector v_1 + \vector v_2)  + c_2(\vector v_1 + \vector v_2 + \vector v_3) + c_3(-\vector v_2 + 2\vector v_3)\\
  \implies \vector x = c_1 \vector v_1 + (c_1+c_2+c_3) \vector v_1 + (-c_2 + 2c_3) \vector v_3
  $$
  We can thus write down a linear map that a vector in $W\dash$ coordinates to one in $V\dash$coordinates given by the matrix 
  $$
  A = \left[ \begin{array} { l l l } { 1 } & { 0 } & { 0 } \\ { 1 } & { 1 } & { 1 } \\ { 0 } & { - 1 } & { 2 } \end{array} \right].
  $$

  Then $A\inv$ is a map that takes the $V\dash$ coordinates of a vector and gives us the correspond $W\dash$coordinates. If $A^{-1}$ is full-rank, the resulting linear system always has a solution, meaning this change of coordinates can always be done and thus the $\vector w_i$ span $\RR^3$.
...
2. $f: \RR^2 \to \RR$ is totally differentiable at $\vector x$ iff there is an open neighborhood of $\vector 0$ containing $\vector h$ and there exists a linear map $D: \RR^2 \to \RR$ such that $f(\vector x + \vector h)- f(\vector x) = \vector D(\vector x)\vector h + \vector o(\vector h)$, where $\frac{o(\vector h)}{\norm{\vector h}} \to 0$.
    ...
    &= {\color{purple}h_1f_x(A, 0)} + {\color{green}h_2f_y(0, 0) + o(h_2)} \\
    &= h_1 f_x(0, 0) - h_1 f_x(0, 0) + {\color{purple}h_1f_x(A, 0)} + {\color{green}h_2f_y(h_1, 0) + o(h_2)} \\
    &= h_1 f_x(0, 0) + h_2 f_y(0, 0) + h_1(f_x(A,0) - f_x(0,0)) + o(h_2)
    \end{align*}$$

    Now, since $f_x$ is continuous, $f_x(A,0) - f_x(0,0) \to 0$ and so $h_1(f_x(A,0) - f_x(0,0)) = o(h_1)$. We thus have
    $$\begin{align*}
    f(h_1, h_2) - f(0, 0)
    ...
    and so if we define $D(\vector h) = h_1 f_x(0, 0) + h_2 f_y(0, 0)$, we can take absolute values to find that 
    $$
