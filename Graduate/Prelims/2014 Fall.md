...
1. Let $\vector w_i$ be the proposed new basis elements -- we can write down a linear map that sends $\vector w_i$ to $\vector v_i$ which is given by the matrix 
  $$
  ...
  That is,  
  $$
  \vector x = c_1 \vector w_1 + c_2 \vector w_2 + c_3 \vector w_3 \implies \vector x = c_1 \vector v_1 + (c_1+c_2+c_3) \vector v_1 + (-c_2 + 2c_3) \vector v_3
  $$
