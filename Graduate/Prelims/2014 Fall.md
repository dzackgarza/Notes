...
1. $f: \RR^2 \to \RR$ is totally differentiable at $\vector 0 = (0, 0)$ iff there is an open neighborhood of $\vector 0$ and there exists a linear map $D: \RR^2 \to \RR$ such that $f(\vector x + \vector h)- f(\vector x) = D(\vector x) + \vector \varepsilon(x,y)$
