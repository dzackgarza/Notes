...
3. $f: \RR^2 \to \RR$ is totally differentiable if there exists a linear map $D: \RR^2 \to \RR$ such that 
$$f(\vector x + \vector h)- f(\vector x) = D(\vector x)\vector h + \vector o(\vector h) \text{ where } \frac{o(\vector h)}{\norm{\vector h}} \to 0.
$$
