...
1. Suppose $\exists M \suchthat \forall x,~ g(x) < M$. Then let $\varepsilon > 0$ be arbitrarily chosen; we want to show that there exists a $\delta$ such that $\abs{x} \leq \delta \implies \abs{f(x)g(x)} \leq \varepsilon$. Since $\lim_{x\to 0} f(x) = 0$, there exists a $\delta_f$ such that $\abs{x} \leq \delta_f \implies \abs{f(x)} \leq \varepsilon$. 
