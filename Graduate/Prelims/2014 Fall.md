...
1. Parts
  ...
  $$
   ...
   1. We will use the fact that $A = PDP\inv$ where since $A$ is symmetric, $P$ is orthogonal, and thus $\norm{P\vector x} = \norm{\vector x}$ for all $\vector x$. This follows because $P^TP = I$, and so
   $$
   \norm{P\vector x}^2 = \inner{P \vector x}{P \vector x} = (\vector x^T P^T) P \vector x = \vector x^T I \vector x = \inner{\vector x}{\vector x} = \norm{\vector x}^2.
   $$
   ...
   $$
   where $\vector x\in S^2 \implies P^T\vector x \definedas \vector y \in S^2$ since $P^T$ is orthogonal (so $P^T \vector x \in S^2)$ and full-rank (and thus a bijection $S^2 \selfmap$).
    ...
    $$
      ...
      We now note that we can take $\vector y = \thevector{0, 1}$, in which case $D\vector y = \thevector{0, \lambda_2} = \thevector{0, 1}$ and thus $\inner{\vector y}{D \vector y} = 1$ is a candidate minimum. Using Cauchy-Schwarz for sums, we have
    $$
    \abs{\sum_{i=1}^2 \lambda_i y_i^2}^2 \leq \sum_{i=1}^2 \abs{\lambda_i}^2 \sum_{i=1}^2 \abs{y_i^2}^2 = \sum_{i=1}^2 \abs{\lambda_i}^2
    $$

    since $\norm{\vector y} = 1$.


    We can alternatively minimize the inner product using Lagrange multipliers. Letting $\vector y = \thevector{y_1, y_2}$, we can multiply this inner product out to formulate this as a constrained optimization problem
    $$
...
3. $f: \RR^2 \to \RR$ is totally differentiable at $\vector x$ iff there is an open neighborhood of $\vector 0$ containing $\vector h$ and there exists a linear map $D: \RR^2 \to \RR$ such that $f(\vector x + \vector h)- f(\vector x) = \vector D(\vector x)\vector h + \vector o(\vector h)$, where $\frac{o(\vector h)}{\norm{\vector h}} \to 0$.
