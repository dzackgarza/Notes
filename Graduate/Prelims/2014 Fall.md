...
$$
  ...
  $$
    ...
    which is can be writte as the matrix equation
  $$
...
1. We first note that we can rewrite the equation of the region a bit to obtain something more familiar: $x^2 + y^2 = 2x \implies (x-1)^2 + y^2 = 1$, which is a translated circle. Integrating over this region will be easy compared to the line integral, so we apply Green's theorem:
$$
