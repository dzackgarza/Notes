...
2. $f: \RR^2 \to \RR$ is totally differentiable at $\vector x$ iff there is an open neighborhood of $\vector 0$ containing $\vector h$ and there exists a linear map $D: \RR^2 \to \RR$ such that $f(\vector x + \vector h)- f(\vector x) = \vector D(\vector x)\vector h + \vector \varepsilon(\vector h)$, where $\frac{\varepsilon(\vector h)}{\norm{\vector h}} \to 0$.
    ...
    We'll also use this form of the mean value theorem:
    $$
    f_x \text{ differentiable and continuous on } (0, t) \implies \exists A \in [0, t] \suchthat \\ f(t, y) - f(0, y) = f_x(A, y)(t - 0) = tf_x(A, y)
    $$
    ...
    &= f(h_1, h_2) - f(0, 0) - \nabla f(0, 0)\thevector{h_1, h_2}^T \\ \\
    &= f(h_1, h_2) - f(0, 0) - h_1 f_x(0, 0) -h_2 f_y(0,0) \\ \\
    &= (f(h_1, h_2) - f(h_1, 0)) + (f(h_1, 0) - f(0, 0)) - h_1 f_x(0, 0) -h_2 f_y(0,0) \\ \\
    &= h_2f_y(h_1, A) + h_1f_x(B, 0)- h_1 f_x(0, 0) -h_2 f_y(0,0) \\ 
    & \quad \quad \textit{for some } A \in [0, h_2], B \in [0, h_1] \textit{ by MVT }\\ \\
    &= h_2(f_y(h_1, A) - f_y(0, 0)) + h_1(f_x(B,0) - f_x(0, 0))
    \end{align*}$$

    Since $f_x$ is continuous at $(0, 0)$ and $B \in [0, h_2]$, we have $\abs{f_x(B,0) - f_x(0, 0)} \to 0$ as $h_2 \to 0$. So we can pick a $\delta_1$ such that this is bounded by $o(\varepsilon)$.

    > Explicitly, we have $B \in [0, h_2], so lim_{h_2\to 0} B = 0. Since $f_x$ is continuous, we can commute limits, so $\lim_{h_2\to 0} \abs{f_x(B,0) - f_x(0, 0)} = \abs{f_x(\lim_{h_2\to 0}  B,0) - f_x(0, 0)} = \abs{f(0,0) - f(0,0)} = 0$.

    Since $f_y$ isn't necessarily continuous, we can't do the same thing....
