...
1. Claim: this holds iff $f$ is injective iff $f$ has a left inverse.
   1. $\implies$: Suppose this holds, and let $x_1,x_2 \in X$. Then suppose $f(a) = f(b)$. We have $f^{-1}(f(\theset{a})) = \theset{a}$ and $f^{-1}(f(\theset{b})) = \theset{b}$ by assumption, but also $f(\theset{a}) = f(a) = f(b) = f(\theset{b})$, so this says that $f^{-1}f(a) = a$ and $f^{-1}f(b) = b$. Combining these, we have $a = f^{-1}f(a) = f^{-1}f(b) = b$, so $f$ is injective.
   2. $\Longleftarrow:$ Suppose $f$ is injective, so $f(a)=f(b) \implies a=b$.
