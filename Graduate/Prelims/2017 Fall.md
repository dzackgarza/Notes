...
1. 
   ...
   1. Note that $p\implies q \iff \neg q \vee p$, so we have $\neg(p \implies q) \iff \neg(q \vee \neg p) \iff p ~\&~ \neg q$.
$$
f: \RR \to \RR \text{ is continuous } \iff \\ 
\forall (x, y) \in \RR^2, ~\forall \varepsilon,~\exists \delta \suchthat d(x,y) < \delta \implies d(f(x), f(y)) < \varepsilon \iff \\ 
\forall (x, y) \in \RR^2, ~\forall \varepsilon,~\exists \delta \suchthat d(f(x), f(y)) < \varepsilon \quad \vee \quad d(x,y) \geq \delta,
$$
so
$$
f: \RR \to \RR \text{ is not continuous } \iff \\ \exists (x,y) \in \RR^2, \exists \varepsilon \suchthat ~\forall \delta, ~ d(x,y) < \delta ~\&~ d(f(x), f(y)) \geq \varepsilon. 
$$
...
1. $V = \theset{\vector v \in \RR^3 \suchthat \inner{\vector v}{\thevector{3,4,5}} = \vector 0}$
   ...
   $$
   2. Note $V$ defines a plane $P \cong \RR^2 \subset \RR^3$, so a projection onto $P^\perp = \thevector{3,4,5}$ will work:
   $$
   ...
   3. Yes, get two vectors in the plane using the "nullspace" trick, then take the orthogonal projection $A(A^TA)\inv A^T$
   4. No, by rank nullity: $\abs{\im A} + \abs{\ker A} = \abs{\mathrm{domain} A}$.
   
1. Induct on $n$, integrate by parts and use L'Hopital.
...
1. 
   ...
   \left(\begin{array}{rr|r}
      ...
      \spec(A) = [-1, 1, 1]
   $$
   3. Every symmetric matrix with real spectrum can be diagonalized over $\RR$ by the spectral theorem, so $A$ is similar to a diagonal matrix $D$ with the same eigenvalues ($\pm 1$). But $D^2_{ii} = (\pm 1)^2$ so $D^2 = I$. 
