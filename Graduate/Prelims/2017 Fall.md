...
1. Definitions:
  ...
  - Proof of actual statement
    ...
    $$
    but by the previous statement, $\dd{g}{t}(t_0) = 0$, which exacty says that the gradient of $f$ is orthogonal to $\gamma$ at $\vector p$. But $\vector p$ was an arbitrary point on the level surface, and $\gamma$ was an arbitrary curve through it. So $\nabla f$ is orthogonal to *every* curve through $\vector p$, and this orthogonal to the tangent plane at $\vector p$, and thus normal to the surface at $\vector p$. Since $\vector $p$ was an arbitrary point on the level set, this holds everywhere on the level set, and for arbitrary level sets. Thus $\nabla f$ is orthogonal to every level set. $\qed$
