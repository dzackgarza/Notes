...
\forall (x, y) \in \RR^2, ~\forall \varepsilon,~\exists \delta \suchthat d(x,y) < \delta \implies d(f(x), f(y)) < \varepsilon \\ 
\forall (x, y) \in \RR^2, ~\forall \varepsilon,~\exists \delta \suchthat d(f(x), f(y)) < \varepsilon \quad \vee \quad d(x,y) \geq \delta,
$$
so
$$
f: \RR \to \RR \text{ is not continuous } \iff \\ \exists (x,y) \in \RR^2, \exists \varepsilon \suchthat \quad\forall \delta, ~ d(x,y) < \delta ~\&~ d(f(x), f(y)) \geq \varepsilon. 
$$
