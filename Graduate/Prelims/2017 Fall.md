...
3. 
   ...
   \left(\begin{array}{rr|r}
      ...
      \spec(A) = [-1, 1, 1]
   $$
   1. Every symmetric matrix with real spectrum admits a real eigendecomposition $\Lambda D \Lambda\inv$, where $D$ is diagonal with entries ($\pm 1$), the eigenvalues of $A$. But $[D^2]_{ii} = (\pm 1)^2 = 1$ so $D^2 = I_n$. Thus we have $A^2 = (\Lambda D \Lambda\inv)^2 = \Lambda D^2 \Lambda\inv = \Lambda I_n \Lambda\inv = I_n$. 
