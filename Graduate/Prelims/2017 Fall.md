...
3. 
   ...
   \left(\begin{array}{rr|r}
      ...
      \spec(A) = [-1, 1, 1]
   $$
   1. Every symmetric matrix with $A$ real spectrum admits a real eigendecomposition $\Lambda D \Lambda^T$, where $D$ is diagonal with entries the eigenvalues of $A$ and $\Lambda$ are orthogonal. Here, we only need the fact that $A$ is diagonalizable by invertible matrices$. But here $[D^2]_{ii} = (\pm 1)^2 = 1$ so $D^2 = I_n$. Thus we have 
