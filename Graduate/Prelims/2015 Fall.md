...
$$
  ...
  $$
    ...
    Since the partial derivatives of $f$ exist and are continuous, $f$ is differentiable with derivative $Df = \nabla f$. Thus by the multivariate chain rule, $g$ is differentiable as well and can be computed as 
    $$
    g'(t) = Df(\gamma(t)) ~ D\gamma(t) = \inner{\nabla f(\gamma(t))}{\gamma'(t)}.
    $$ 

    Note that $\gamma(0) = \vector p$, and $\gamma$ is a linear function of $t$ that is differentiable, where $\gamma'(t) = \vector u$.

    From this, we can compute $g'(0) = \inner{\nabla f(\vector p)}{\vector u}$ in the above expression.

    Since $g$ is differentiable, we can write
    $$
    g'(0) = \restrictionof{g'(t)}{t=0} = \restrictionof{\lim_{h\to 0} \frac{g(t+h) - g(t)}{h}}{t=0} = g(h) - g(0)
    $$
  $$\begin{align*}
  j
  \end{align*}$$