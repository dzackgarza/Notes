...
$$
  ...
  $$
    ...
    We need to show that this exists for arbitrary $\vector u$. Since the partials exist and are continuous, $f$ is differentiable and $\nabla f$ exists. We can thus compute
  $$\begin{align*}
  \inner{\vector u}{\nabla f} &= af_x + bf_y \\
  &= a\lim_{h\to 0} \frac{f(x+h, y) - f(x,y)}{h} + b\lim_{h\to 0} \frac{f(x, y+h) - f(x,y)}{h} \\ 
  &= \lim_{h\to 0} \frac{af(x+h, y) + bf(x, y+h) - (a+b)f(x,y)}{h}
  \end{align*}$$