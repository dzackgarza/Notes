...
$$
  ...
  $$
    ...
    We need to show that this exists for arbitrary $\vector u$. Wlog, assume $\norm{\vector u} = 1$, and let $D_{\vector u}f$ denote the directional derivative of $f$ in the direction $\vector u$. Let $\gamma(t) = \vector x + t \vector u$

    Define 
    $$g: \RR \to \RR \\ t \mapsto f(\gamma(t)) $$
  $$\begin{align*}
  j
  \end{align*}$$