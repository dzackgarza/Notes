...
$$
  Definition: 
  $$
  \dd{}{x}f(x,y) = \lim_{h\to 0} \frac{f(x+h, y) - f(x,y)}{h}
  $$

    We need to show that this exists for arbitrary $\vector u$. We can generally compute this as $\inner{\nabla f}{\vector u} = f_xa + f_yb$