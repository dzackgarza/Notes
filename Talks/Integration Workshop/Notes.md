...
In all cases, we end up reducing the hard (or sometimes impossible) problem of computing a real integral using calculus in $\R$ to what are essentially algebra problems in $\C$. Although these techniques may seem quite involved at first glance, a bit of practice make these techniques fast and powerful enough to rival the standard set of single variable real calculus techniques.