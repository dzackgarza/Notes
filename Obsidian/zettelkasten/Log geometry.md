...
- Every log scheme has a locus where the log structure is trivial, $\ts{x\in X\st \OO_{X, x}\units = M_x}$, so [[stalk]] of $M$ are units. Think of this like the complement of the boundary.
