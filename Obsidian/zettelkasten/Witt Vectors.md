...
General idea: given a commutative ring $R$, a [Witt vectors](Witt%20vectors) $w$ will be an infinite sequence of elements of $R$. Taking $W(R)$ to be a collection of such $w$, we will equip $W(R)$ with its own ring structure.

One useful fact will be that $W(\FF_p) = \ZZ_p$, the ring of [p-adic integers](p-adic%20integers). The construction essentially mirrors a way of obtaining $\ZZ_p$ from the finite field $\FF_p$.
...
$$ 
the $(p-1)\dash$th roots of unity in $\ZZ_p$. This arises from the fact that $\FF_p^\times \cong \ZZ/(p-1)\ZZ$, and so every nonzero element of $\FF_p$ is a root of $x^{p-1}-1$. Using [Hensel's Lemma](Hensel's%20Lemma), these lift to representatives in $\ZZ_p$ satisfying the same equation.
