...
\begin{tikzcd}
	...
	\arrow[shorten <=7pt, dashed, two heads, from=3-1, to=5-3]
	\arrow[shorten >=7pt, dashed, hook, from=3-3, to=5-5]
\end{tikzcd}

> [Link to Diagram](https://q.uiver.app/?q=)
...
\begin{tikzcd}
	...
	{\operatorname{Bernoulli}(p)} && p & {p(1-p)} & {\begin{cases}p & x=1 \\ 1-p & x=0\end{cases}} \\
...
- $\EE[r(x)] \da \int r(x) f(x) \dx$ or $\sum_k r(k) P(x=k)$
- $\Var(x) = E[x^2] - E^2[x]$
- Marginal densities: if $f(x, y)$ is a distribution, $f_x(x) \da \int f(x, y) \dy$ and $P(x\times y \in A) = \iint_A f(x, y) \dx \dy$.
  - Independent if $f(x, y) = f_x(x) f_y(y)$.
- If $x$ has density $f_x$ and $y \da r(x)$ for a continuous increasing function, setting $s \da r\inv$, $y$ has density $g_y(y) = f_x(s(y)) s'(y)$.
