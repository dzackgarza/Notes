...
**Definition:** A principal bundle $F \to E \mapsvia{\pi} B$ is **universal** iff $E$ is [weakly contractible](weakly%20contractible.md).
...
# Examples
- Every [fiber bundle](fiber%20bundle.md) $F\to E\to B$ is a principal $\Aut(F)\dash$ fiber bundle. 
...
- Each $F_b$ is a [homogeneous space](homogeneous%20space.md).
- Although each fiber $F_b \cong G$, there is no preferred identity element in $F_b$. 
	...
	- So each fiber $F_b$ is a $G\dash$ [torsor](torsor.md).
