...
.\]
Up to the Quillen equivalence {Kan complexes} ~= {spaces}, geometric realization takes a (quasi-)category (or simplicial set) and inverts all the morphisms.
...
- Consider $\OO_n \da \WW(\FFpn) \gens{ s } / \gens{ sa = a^{ \varphi} S }$ where \( \varphi \) is the Frobenius on $\FFpn$. 
