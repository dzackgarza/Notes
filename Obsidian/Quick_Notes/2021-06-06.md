...
- Take smooth manifolds, take the Yoneda embedding to $\Presh(\sm\Mfd)$: these satisfy a Mayer-Vietoris gluing property, and homotopy invariance in the sense that $\Hom(\wait, X) \cong \Hom(\wait \cross I, X)$
  - Why the first argument: homotopy invariance *as a presheaf*

- AG setting: $\Presh(\sm\Sch_{/k})$, send to presheaves to define **motivic spaces**.
...
- Theorem (Lubin-Tate): there is a universal deformation for $(k, F)$ given by $\Wittvectors(k)\powerseries{\elts{u}{n-1}} \da E(k, F)_0$.

- For $k = \FF_p$, we have $\Wittvectors(k) = \ZZp$, and there is an action of $\Aut(F)$ and $\Gal(k)$.
...
- Example: for $n=2$, we know $\EO_2(G) = L_{K(2)} \tmf$ or a summand thereof.
...
- Consider $\OO_n \da \Wittvectors(\FFpn) \gens{ s } / \gens{ sa = a^{ \varphi} S }$ where \( \varphi \) is the Frobenius on $\FFpn$. 
  - It turns out that $\Aut_(F_{\Honda}) = \OO_n\units$
  This is the Dieudonne module for $F_\Honda$?
- Power series rings have a maximal ideal $\mfm$, so consider $\pi_{-2} E_n / \gens{ p}, \mfm^2$. 
...
- Devinats-Hopkins: up to associated graded, $\pi_* E_n \cong \Sym(\OO_n) \localize{\Delta}\complete{I}$.
