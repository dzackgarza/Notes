...
- Can get a derived version: take *non-abelian derived functors*, i.e. take a [simplicial resolution](simplicial%20resolution) by simplicial polynomial algebras and apply the functor to the resolution.
  - Equivalently a left [kan extension](../zettelkasten/Kan%20extension.md)?
- Define $\Ld \Omega^1 _{A/R}$ to be the [cotangent complex](../zettelkasten/Cotangent%20Complex.md) and take derived exterior powers for the other degrees.
...
- Fact: de Rham complex has a universal property, initial (strictly, so odd elements square to zero) commutative [DGA](../zettelkasten/differential%20graded%20algebras.md) receiving a map $A\to X^0$, so the initial way to turn an algebra into a DGA.
  Does the derived version have a similar property?

- [2021-04-26_hochschild_homology](../zettelkasten/Hochschild%20homology.md) is defined as $\HH(A/R) \da A^{\tensor_R S^1}$ the $S^1$ tensoring, take [Homotopy fixed points](Homotopy%20fixed%20points) to get [$\HC^-$](cyclic%20homology).
- Associated graded of $\HH$ recovers derived de Rham:

![image_2021-05-06-11-25-44](figures/image_2021-05-06-11-25-44.png)

  - Why does this happen? $\HH(A/R)$ is the initial [simplicial R-algebra](simplicial%20algebra) with an $S^1\dash$action receiving a map from $A$.
  - Analogies
    - [simplicial rings](../zettelkasten/simplicial%20ring.md) $\mapstofrom$ [CDGAs](../zettelkasten/differential%20graded%20algebras.md)
...
- Some relevant things about [Morava K-theory](../zettelkasten/Morava%20K-theory.md), which will be coefficients that we take.
...
- Produce a virtual fundamental class for flow categories.
  Moduli spaces of trajectories appearing will be "derived [2021-04-26_Stacks_and_Orbifolds](../zettelkasten/Orbifold.md)".
...
- Can express enrichments in terms of [lax functors](lax%20functor) from an indexing 2-category
...
- See the [Borel construction](Borel%20construction), the [Adams isomorphism](Adams%20isomorphism), [ambidexterity](../zettelkasten/ambidexterity.md).
...
- Technical issues with loop actions on categories, only get module structure on homotopy category?
  - Also issues with local-to-global coherence, need $\hocolim$ to be compatible with Kuranishi atlases?
    ...
    Need lots of [equivariant stable homotopy theory](equivariant%20stable%20homotopy%20theory).
...
- Can get the [Arnold conjecture](Arnold%20conjecture) from this homotopy type!
  ...
  $H$ is a [Hamiltonian](Hamiltonian).
  - Can split $H_*(M; \Lambda)$ off from $\HF_*(H; \Lambda)$.

  - Recover statement about $\HF_p$ cohomology using the [AHSS](../zettelkasten/Atiyah%20Hirzebruch%20spectral%20sequence.md).
...
- A lot of cool stuff for homotopy theorists to do here!
  ...
  - Work to build spectrally enriched [Fukaya categories](../zettelkasten/Fukaya%20category.md).
  ...
  Need strong [orientability](../zettelkasten/Orientability%20of%20spectra.md) conditions.
...
- Improvement over rational results when there is torsion.
  May not improve over numerical bounds for special classes of [Symplectic manifolds](../zettelkasten/Symplectic%20geometry.md).

- [Morava K-theory](../zettelkasten/Morava%20K-theory.md) is useful here because it behaves well with respect to finite groups
  - Want to dualize [classifying spaces](../zettelkasten/classifying%20space.md) of [orbifolds](../zettelkasten/Orbifold.md), use Poincaré duality for orbifolds.
...
- Theorem: from [Langlands](../zettelkasten/Langlands.md) philosophy.
  There is a [Hermitian form](Hermitian%20form) and a [cuspidal representation](cuspidal%20representation) in the same [L packet](L%20packet) where $P$ is nonvanishing.
