...
4. $\Rightarrow$
   Suppose $X$ is contractible. Then, unpacking definitions, the identity map on $X$ is null-homotopic. That is, there is some constant map $f_c: X \into X, f_c(x) = c$ such that (defining the identity map as $\text{Id}_X: x \mapsto x$) we have $f_c \homotopic \id_X$