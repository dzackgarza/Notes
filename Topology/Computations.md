...
- $\prod_S G = \bigoplus_S G$ in $\mathbf{Grp}$ when $\mid S \mid < \infty$, and otherwise the direct sum requires cofinitely many zero entries -- so here we always use $\prod$.
